...
#   speak "The time is $Time_Now" if time_cron '40,45,50 6 * * 1-5';
    speak "room=nick,zack $Time_Now. Kids leave in " . plural(45 - $Minute, 'minute') if time_cron '30,40,45,44,45 6 * * 1-5';
    
    if (time_cron '37,42 6 * * 1-5') {
#       or 9 == said $v_test1) {
...
speak "rooms=all Remember to turn the boys alarm clocks off" if time_cron '15,30 22 * * 5';
