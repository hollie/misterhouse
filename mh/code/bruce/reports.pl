...
if (said $v_report_weblog or time_now '5:05 AM') {
    ...
    set   $p_report_weblog "report_weblog -ignore none -outdir $config_parms{data_dir}/logs -runid mh_month $config_parms{data_dir}/logs/server.$Year_Month_Now.log";
...
if (done_now $p_report_weblog) {
    ...
    run "$config_parms{browser} http://misterhouse.net/report_weblog.html";
}
