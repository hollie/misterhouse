...
if ($MW and $Reload) {
    ...
    if (!$Startup and !file_change("$config_parms{code_dir}/tk_frames.pl")) {
        ...
        $Tk_objects{grid} = $Tk_objects{fl}->Frame->pack(qw/-side left -anchor n/); 
    }
        ...
        $Tk_objects{fll}  = $Tk_objects{fl}->Frame->pack(qw/-side left -fill both -expand 1/); 
        $Tk_objects{grid} = $Tk_objects{fl}->Frame->pack(qw/-side left -anchor n/); 
        ...
        $Tk_objects{msg_window} = $Tk_objects{fll}->
            Scrolled('Text', -height=> 5, -width => 30, -bg => 'white', -wrap => 'none', -scrollbars => 'se')->
