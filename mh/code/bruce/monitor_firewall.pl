...
# Monitor a usage log from the firewall ... keep the boys honest!

my $ipchain_file = '//misterhouse/projects/logs/ipchains.log';

$check_ip_log = new Voice_Cmd "{Check the ip log,Check Nicks computer time}", "Ok, checking";
$check_ip_log-> set_info('Check to see if Nick has spent too much time gaming for today');
...
if (said $check_ip_log or ($New_Minute and !($Minute % 15))) {
    ...
    for my $data (file_read $ipchain_file) {
        my ($time, $source, $port, $dest) = (split ' ', $data)[3,7,8,9];
#       print "db s=$source p=$port d=$dest\n";
        if ($source eq '200.200.200.4' and $time ne $time_prev) {
...
#   $time_nick = time_diff 0, $time_nick * 60; # Convert to a text string
    if ((($Save{nick_computer_time} < $time_nick) and $time_nick > 120) or
        said $check_ip_log) {
        $Save{nick_computer_time} = $time_nick;
        my $msg = sprintf("Notice, Nick has been on the computer for %2.1f hours today", $time_nick/60);
