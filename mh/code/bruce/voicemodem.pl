...
#
#$phone_modem = new Serial_Item ('AT#CID=1','init','serial_modem');
...
if ($VoiceModemMode eq "answermode" and expired $timer_masterinact
...
print_log "modem data: $Serial_Ports{serial_modem} {data}" if $Serial_Ports{serial_modem} {data};
if ($Serial_Ports{serial_modem} {data} =~ /(\d)/) {
    $VoiceModemStatus = $1;
...
if ($Serial_Ports{serial_modem} {data} =~ /b/) {
...
if ($Serial_Ports{serial_modem} {data} =~ /r/) {
