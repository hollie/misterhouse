# Category= Timers
				...
				# Do an hour timer
my %timer_reminder_hour_intervals = map {$_, 1} (1,2,5,10,20,30);
...
speak &plural($temp, "hour") . " left on the hour timer"  if 
    ($temp = hours_remaining_now $timer_hour) and $timer_reminder_hour_intervals{$temp};
speak "$temp minutes left on the hour timer" if
    ($temp = minutes_remaining_now $timer_hour) and $timer_reminder_minutes_intervals{$temp};
