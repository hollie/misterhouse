...
my %timer_reminder_minutes_intervals = map {$_, 1} (1,2,3,5,10,20,30,60);
...
speak &plural($temp, "minute") . " left on the timer"  if
    ($temp = minutes_remaining_now $timer_minute) and $timer_reminder_minutes_intervals{$temp};
