...
$v_tv_shows1 = new  Voice_Cmd('What TV shows are on channel [5,6,8,9,12] tonight');
...
if (time_cron('0,30 18-22 * * *')) {
    run qq[get_tv_info -times $Hour:$Minute -keys "$config_parms{favorite_tv_shows}"];
...
if ($state = changed $f_tv_file) {
    ...
    my $summary = read_head $f_tv_file, 1;
