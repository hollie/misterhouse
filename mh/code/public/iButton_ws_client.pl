...
#	4	peak_speedMS 	is peak wind speed in meters/sec
#	5	max_speedMS 	is today's hi in meters/sec
#	6	current_dir 		is the current wind direction
#	7	max_dir 			is the 10 minute average wind direction
...
if ($state = said $ibws_v)
{
	...
	{
		my $msg = "The Current temperature is $Weather{TempOutdoor}, with a high of $Weather{TempOutdoorHigh}, and low of $Weather{TempOutdoorLow}. ";
		$msg .= "Current Wind Speed is $Weather{WindSpeed} miles per hour, with gusts of $Weather{WindSpeedPeak}, and a high of $Weather{WindSpeedHigh}.  Wind Direction is $direction[$Weather{WindDir}] and is averaging $direction[$Weather{WindDirAvg}]. ";
		$msg .= "The Current Rainfall Rate is $Weather{RainRate} inches per hour.  Today's total rainfall is $Weather{RainToday} inches with $Weather{RainWeek} inches for the week, and $Weather{RainMonth} inches for the month.  ";
