...
if (my $packet = said $ISDN) {
  my $debug = $config_parms{debug} eq 'ISDN';
  ...
  while ($i < length($packet)) {
    ...
    if ($option == 7) {   # Call duration Channel 1
      $current{1}{calldur} = unpack('H*', substr($packet,$i,$length));
    ...
    if ($option == 27) {   # Call duration Channel 2
      $current{2}{calldur} = unpack('H*', substr($packet,$i,$length));
...
# Following code is oriented around detecting and processing voice calls
# that are new this pass.

  use vars '$old1_call', '$old2_call';

  $old1_call = '' if $current{1}{callflag} != 1; 
  if ( ($current{1}{callflag} == 1 ) and ($current{1}{callingnum} ne $old1_call) ) {  # New incoming voice call?
    $old1_call = $current{1}{callingnum};
    &checkcalled($current{1}{callednum});	
    &callerid($current{1}{callingnum});	
  }

  $old2_call = '' if $current{2}{callflag} != 1;
  if ( ($current{2}{callflag} == 1 ) and ($current{2}{callingnum} ne $old2_call) ) {  # New incoming voice call?
    $old2_call = $current{2}{callingnum};
    &checkcalled($current{2}{callednum});	
    &callerid($current{2}{callingnum});	
  }

# Detect if we've launched an outgoing data call to the internet; if so, update our DNS entry via TZO.

  use vars '$data1_call', '$data2_call';
  $p_set_ip_3com_tzo = new Process_Item("set_ip_3com_tzo");

  $data1_call = '' if $current{1}{callflag} != 4;
  if ( ($current{1}{callflag} == 4 ) and ($current{1}{destname} ne $data1_call) ) {  # New data call?
  ...
  $data2_call = '' if $current{2}{callflag} != 4;
  if ( ($current{2}{callflag} == 4 ) and ($current{2}{destname} ne $data2_call) ) {  # New data call?
...
# End of code that runs each time we receive a packet from the 3Com ISDN Lan Modem
...
sub checkcalled {
  my ($called) = @_;
  print_log "New ISDN Incoming Voice Call, called number = $called";
  
  # a call to my second number indicates a desire to bring up an internet connection.
  $p_ping = new Process_Item("ping www.tzo.com");
  start $p_ping if $called = '6683007';
}


# As of May 2000, Code below needs more work and better integration into 
...
sub callerid {
  ...
  $cid_speak_nmbr = substr($cid_speak_nmbr, 3, 8) if ($areacode eq $config_parms{local_area_code}); # Drop area code if same
