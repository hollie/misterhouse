...
$p_star_citystate = new Process_Item("get_url http://www.starnews.com/news/citystate/index.html $f_star_local_html");
...
if (said $v_star_citystate eq 'Get') {
    ...
    if (0 and -s $f_star_local_html > 10 and
        time_date_stamp(6, $f_star_local_html) eq time_date_stamp(6)) {
...
if (done_now $p_star_citystate) {
	...
	if (/From The Indianapolis Star/){}
      ...
      elsif ((/^([A-Z][A-Za-z0-9':,\$\s\-\.]+)/) and $start and $sdate <2 and $i <9){
            $text  .= "  $1\n";
	}
        elsif ((/html">([A-Za-z0-9':,\$\s\-\.]+)</) and $sdate < 2 and $i <10){
            $i++;
		$headline++;
            $text  .= "$i: $1\n";
        }
        elsif ((/(^[A-Za-z0-9':,\$\s\-\.]+)/) and $headline and $sdate < 2 and $i <9){
      	$headline = 0;
            $text  .= "      $1\n";
	  }
