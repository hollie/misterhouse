...
$v_tv_shows2 = new  Voice_Cmd('What favorite TV programs are on [tonight,late tonight,this afternoon,this morning,tomorrow,today and tomorrow,next 2 days,yesterday]');
...
if ($New_Minute) {
  if (&WaitForAnyWindow('WinTV', \$tv_window, 100, 100)) { $Save{TV_on} = 1; }
  else { $Save{TV_on} = 0; }
}
...
sub turn_on_tv_spare2 {
  my $temp_window;
  $Save{TV_on} = 0 if (&WaitForAnyWindow('WinTV', \$temp_window, 100, 100));
  print "$Save{TV_on}\n";
...
#            speak "There are no favorite shows are on tonight";
        ...
        speak "rooms=all Notice, $msg starting $start.  @data" if $show_count > 0;
    }
    else {
