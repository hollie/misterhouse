...
if (said $v_weather_forecast_page eq 'Get')	{
	...
	# Do this only if we the file has not already been updated today and it	is not empty
	if (0 and -s $f_weather_forecast_html >	10 and
		time_date_stamp(6, $f_weather_forecast_html) eq	time_date_stamp(6))	{
...
if (done_now $p_weather_page) {
	...
	foreach	$AirportLine (@airport)	{			# Run for each airport in array
		($AirportCode, $AirportPos,	$AirportName) =	(split(',',	$AirportLine))[0,1,2];	# Split	each line
		...
		if ($text =~ /$AirportCode /) {				# Check	if Airport is in text
			...
			foreach	$data (@data) {					# Process each field
				...
				if ($data =~ /\//) {					# Check	if Temp	and	Dew
					($data_temp, $data_dew)	= split	("/", $data);	# Split	temp and dew
...
#		print_log $data;
	if ($data =~ /KT/) {
	   ...
	   if ($wind_direction >=	0 && $wind_direction <=	 22) { $wind_direction = "North"; }
	   if ($wind_direction >=  23 && $wind_direction <=	 67) { $wind_direction = "NorthEast"; }
	   if ($wind_direction >=  68 && $wind_direction <=	112) { $wind_direction = "East"; }
	   if ($wind_direction >= 113 && $wind_direction <=	157) { $wind_direction = "SouthEast"; }
	   if ($wind_direction >= 158 && $wind_direction <=	202) { $wind_direction = "South"; }
	   if ($wind_direction >= 203 && $wind_direction <=	247) { $wind_direction = "SouthWest"; }
	   if ($wind_direction >= 248 && $wind_direction <=	292) { $wind_direction = "West"; }
	   if ($wind_direction >= 293 && $wind_direction <=	337) { $wind_direction = "NorthWest"; }
	   if ($wind_direction >= 338 && $wind_direction <=	360) { $wind_direction = "North"; }
