...
my ($PhoneModemString, $PhoneNumber, $PhoneName, $NameDone, $NumberDone, $i, $j);
my (@rejloglines, $NumofCalls);
my ($PhoneDate, $PhoneTime, @callloglines, $CallLogTempLine, $RejLogTempLine);
...
$request_phone_stuff = new X10_Item('B6');
...
if ((said $v_phone_lastcaller) || (state_now $request_phone_stuff eq 'on')) {
    ...
    foreach $CallLogTempLine (@callloglines) {
        ...
        if ($PhoneNameLog eq 'Out of the Area' and $PhoneDateLog ne $Date_Now) {
            speak "At $PhoneTimeLog on $PhoneDateLog, a person from out of the area called.";
        }
        if ($PhoneNameLog eq 'Out of the Area' and $PhoneDateLog eq $Date_Now) {
            speak "At $PhoneTimeLog, a person from out of the area called.";
        }
...
if ($PhoneModemString = said $phone_modem) {
    ...
    if ($NumberDone eq "yes" and $NameDone eq "yes") {
        ...
        if ($PhoneName eq "Out of the Area") {
            print_msg "PHONE: Out of area phone call.";
            speak "Out of area phone call.";
        }
        else {
