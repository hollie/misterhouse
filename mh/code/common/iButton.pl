...
$ib_temp2  = new iButton '1000000029b992';
...
speak voice => 'random', text => read_random $remark_bad if ON  eq state_now $ib_nick;
speak voice => 'random', text => read_next $f_deep_thoughts_ib if ON  eq state_now $ib_zack;
speak voice => 'random', text => read_next $remark_good  if ON  eq state_now $ib_bruce;
