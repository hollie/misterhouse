...
if ($state = said $v_moon_info1) {
    ...
    respond qq[The next $state moon is in $days, on $Moon{$state}];
}

if ($state = said $v_moon_info2) {
    my $days = &time_diff($Moon{"time_${state}_prev"}, $Time);
    respond qq[The last $state moon was $days ago, on $Moon{"${state}_prev"}];
}

if ($state = said $v_moon_info3) {
    respond qq[The moon is $Moon{phase}, $Moon{brightness}% bright, and $Moon{age} days old];
}
