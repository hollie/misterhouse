...
#@ delete triggers with <a href=/bin/triggers.pl>/bin/triggers.pl</a>.

use vars '%triggers';           # use vars so we can use in the web server

my ($trigger_write_code_flag);
...
sub trigger_write_code {
    ...
    foreach my $name (trigger_list()) {
        ...
        $script .= "\n# name=$name type=$type  triggered=$triggered\n";
    ...
    &file_write("$config_parms{code_dir}/triggers.mhp", $script);
...
sub trigger_expired {
    my $name = shift; 
    return ($triggers{$name}{triggered});
}
