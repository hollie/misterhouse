...
##
# use Ultimeter2000;
# $peet_weather=new Ultimeter2000;;
# $peet->set_time;

# mh.ini
##
# Ultimeter2000_serial_port=/dev/ttyR33

use strict;

package Ultimeter2000;

@Ultimeter2000::ISA = ('Serial_Item');

sub serial_startup {
	&main::serial_port_create('Ultimeter2000',$main::config_parms{Ultimeter2000_serial_port}, 2400, 'none','');
	&main::MainLoop_pre_add_hook(\&Ultimeter2000::update_weather, 1 );
	&::print_log("Ultimeter Serial Port initialized"); 
}

sub update_weather {
	my ($v,$vv,$vvv,$data,$record_type);
	# Don't want to overload $New_Minute
	# It seems to take 8 seconds to transmit a complete record
	# from ultimeter to pc
	if (($main::Second == 3) and ($main::New_Second)) {
		$main::Serial_Ports{Ultimeter2000}{object}->write(">H");
	} 
	&main::check_for_generic_serial_data('Ultimeter2000');
	$data = $::Serial_Ports{Ultimeter2000}{data_record};
	$::Serial_Ports{Ultimeter2000}{data_record}='';
	return unless $data;

	if ($main::config_parms{debug} =~/weather/) { 
	...
	# NOTE ($data) should contain 456 characters, without the header
	# it should contain 452 characters.  Other units might contain
	# 452 or 448 characters -- see FAQ on www.peetbros.com
...
sub set_time {
	...
	$main::Serial_Ports{Ultimeter2000}{object}->write($string);
}
