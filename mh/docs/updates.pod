...
- Andrew Drummond send in mh/lib/Acid.pm, so we can 
display callerid data from mh on one or more Audrey units running the acid program,
which is included in Pete's nifty Audrey image.
This is automatically enabled from Bill's new CID_Server code, which is called in common/callerid.pl.

- Added a  get_parm_file function, so we can tell if a parm was overridden or not.  This allowed us to 
fixed the ia5 weather page to not display the 'click to set parm' button if the weather parm has already been overridden.

- Bill Sobel updates:
Audiotron player:  Fixed for latest firmware, support password if set in .ini file
CID_Server, caller id server with initial YAC implementation (can add others here fairly easy)  Needs yacserver. and yacserver.bat in the MH directory.
Compool.pm fixes for solar power
DSC_Alarm: supports proxy config
http_server.pl: bug fixes
...
serial_item.pm: looping fixes
stargate.pm: Fixes, digital relay updates, ir support (initial)
stargate485.pm: LCD keypad digit support
telephony_item: base work for dialed ability (we now log outgoing calls as well if the CID hardware detects DTMF and reports them)
wakeonlan.pm: wakeonlan module
x10_items: x10 transmitter definition
xantech.pm: made some logging optional
mh.: serial port/proxy fixes
internet_weather.pl  added nws_city .ini parm nws_city= paramter which overrides city if set
- set_with_timer 
 - return_state option
 - state can be blank

- Bill Sobel created lib/WakeOnLan.pm.  See code/public/wakeup_on_lan.pl for more info.
...
to $format1, which includes City and State if out of City and/or State.  Also enabled RING for Telephone_Interface
and added some different 'Test callerid #' tests.
...
so now we can store nested data in %Save ( e.g. $Save{TEMPS} = { BASEMENT => 70, LIVING_ROOM => 40};  )
I also updated the save/restore code to allow for ~ characters in the state and state_log.
