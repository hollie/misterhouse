...
The default setup runs most the standard scripts in the mh/code/common directory.
...
Both are available from http://misterhouse.net:81/download .
...
http://misterhouse.net:81/public/unzip.exe .
...
Unfortunatly, the 3 packages we need are not yet avaliable from ActiveState for perl version 5.8,
but Randy Kobes has kindly made them available at his site. You can install them with the following:
   ...
   > ppm
   ppm > repository add http://theoryx5.uwinnipeg.ca/cgi-bin/ppmserver?urn:/PPMServer58
   ppm> install GD
   ppm> install DB_File
   ppm> install Tk-JPEG

If the DB_File post-install script doesn't run to install the included libdb.dll, you can extract the dll
from http://theoryx5.uwinnipeg.ca/ppms/x86/DB_File.tar.gz and copy it to a dir in your path.
...
from here: http://misterhouse.net:81/public/speech
...
Download the rpm, tar, or zip file from http://misterhouse.net:81/download .
...
compiling the current Tk interface with 5.004, so you may want to upgraded to 5.005 or higher.
...
The default (press enter) usually is good for most questions.
...
If on linux, you can pick up a copy (3 meg, 7 meg unziped) here: http://misterhouse.net:81/public/flite.gz
...
Now the only supplier we have found is from http://www.nextup.com/attnv.html, where additional voices are $35.
But this site only offers the Windows engine, although the voice data files can be used with either engine.
If you find a source for the linux engine, let me know.  
...
http://misterhouse.net:81/mh.html#List_of_supported_hardware_inter .
...
After mh is up and running, Use http://localhost:8080/bin/select_code.pl  menu
(avaliable on the web ia5 MrHouse menu 'Select Code' button) to select which of the 
standard code files you want to run.  You can add your own custom code files
to your private \misterhouse\code directory.
