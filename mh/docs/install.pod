...
and the unix OSes Linux, FreeBSD, and AIX.  In theory, much
...
and installing packages. Do this by logging on to the internet and typing:
   ...
   perl ppm.bat install Win32-API  (ppm.bat may be ppm or ppm.pl in older perls)
...
For $100, you can get 2 nice 5.1 compatable voices with AT&T Natural Voices TTS SDK here: http://www.naturalvoices.com.
...
After compiling, simply copy the flite binary to mh/bin and set voice_text = flite.
...
These instructions are here for future reference, in hopes that the modules 
will become available again.

The IBM ViaVoice TTS engine for Linux is available from
http://www-4.ibm.com/software/speech/dev/sdk_linux.html .
Here is the download link: http://www6.software.ibm.com/devcon/devcon/docs/vvsdkrt.htm

Looks like you can still get the TTS engine from here:
...
The Voice Recognition engines are available here:
http://www6.software.ibm.com/devcon/devcon/docs/vvsdkrt.htm. 
...
Documentation is in /usr/doc/ViaVoice and test/example code is in /usr/lib/ViaVoice/samples.
...
run mh/bin/viavoice_server_start, make sure mh/code/test/viavoice_control.pl is in your code dir, 
then start mh.  Use the Tk 'VR mode' button and/or the phrase(s) in viavoice_awake_phrase to 
enable the awake VR mode.
