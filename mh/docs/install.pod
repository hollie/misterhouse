...
=head2 Windows users
 ...
 - mh.bat (this will call mh.exe if it exists or 'perl mh')
...
The default setup runs only a few core scripts from mh/code/common directory
(listed in data_dir/code_select.txt). 
You can use http://localhost:8080/bin/code_select.pl  menu
(avaliable on the web ia5 MrHouse menu 'Select Code' button) to select other 
standard code files you want to run.  To quickly test many of the code files, run:
