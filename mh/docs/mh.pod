...
All other items that have states (e.g. X10_Item, Serial_Item, iButton, Voice_Cmd, Group) inherit all Generic_Item methods.
  ...
  Methods:
     ...
     set_with_timer      :  like set, but will toggle back on/off after the specified time
...
will be tested against all Text_Cmd items.  All items that match will fire their state_now methods.
  ...
  Examples:
   ...
   $tcmd2->tie_event('print_log "tcmd2 state: $state"');
   print_log "tcmd3 state: $state"' if $state = $state_now $tcmd3;
...
comments before and after the code that should preceed the Item, or add a # noloop comment to the 
end for the record.  For example:
