...
Europeans can get X10 stuff from  http://www.intellihome.be and
http://www.hwg-telekom.de
...
Used to send and Instant Message across the internet.
Currently only the AIM clients are supported. 
More information on AIM clients (available for various different platforms)
  ...
  Usage:
    net_im_send(option => value);
     ...
     These are the options:
        ...
        password    Default is mh.ini parm net_aim_password
        from        Default is mh.ini parm net_aim_name
        to          Default is mh.ini parm net_aim_name_send

        text        Message
        file        Message.  You can use the text and/or file options.

The first time you send a message, mh will pause for a few seconds while it logs onto AIM.  
...
If you want to have a user defined function called once perl loop, but you want it before or after all the other user code has been
called, use the &MainLoop_pre_add_hook and &MainLoop_post_add_hook functions to add callbacks to your functions.  For example:
