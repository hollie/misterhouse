  ...
  Examples:
    ...
    if ($state = said $v_iButton_connect) {
      if ($state eq 'Connect') {
        ...
        print_log &iButton::disconnect, 'ibutton_port2;
      }
    }
...
=item SendKeys WaitForAnyWindow SetFocus EnumChildWindows sendkeys_find_window 
...
These are the record types:
 ...
 R:  what Response to display.  last_response will pick up the last mh response
...
For cell phones, simply point your phone to http://your_domain/menu_wml.  You can test your
menus with a web based phone simulator at http://www.gelon.net , or download
a phone simlator from http://phone.com .  Other browsers are listed at http://www.palowireless.com/wap/browsers.asp
...
with the code= include directive or as a response to a /SET or /RUN command (see below).
...
SET, RUN, or SUB can use an optional response field to decide what html will be returned:
  ...
  h_response can be one of the following:
...
Here are some examples demonstrating these options:
 ...
 $Password_Allow{'&my_response'} = 1):
  ...
  <FORM ACTION="SET:&my_response">Test 1<INPUT SIZE=5 NAME="$Save{test_input1}"</form>
...
set the persistant flag by passing a 1 as a 2nd argument.  For example:
