...
<p>Paul Chamberlain has set up a 
<a href=http://tifster.tzo.com/wiki/bin/view/Misterhouse>TWiki web-based collaboration site</a>
...
prefers that you use their software (that's why they give it away), and if you want to look up data
on the web for an item, their software is probably best, as they have a large database of UPC codes.
...
you use their software, you must agree to a License that says you will only use the scanner with their
...
All other items that have states (e.g. X10_Item, Serial_Item, iButton, Voice_Cmd, Group) inherit all Generic_Item methods.
  ...
  Methods:
     ...
     set           :  places the value into the state field (e.g. set $light on)
...
You can use this object to group and operate on groups of items:
  ...
  Methods:
     ...
     member_changed:      Returns a member object whenever one changes
  ...
  Examples:
     ...
     if (my $member = member_changed $outside_lights) {
        print_log "Outside light $member->{object_name} changed"
     }
  ...
  Methods:
     new $type, $code : Creates a new Item.  
        ...
        Currently, $code can be any of he following:
           ...
           3digit  : 3 digits are transmitted for channels
...
pass in a model number of MS13.  For the id, you can use the 2 character, or 5 character X10 code.
Here are some examples:          
...
Without the MS13 model number, the light/dark codes will be ignored.
...
my/code/bruce/mh_watchdog.pl, mh/code/test/socket_test.pl, and mh/code/examples/socket_test.pl
...
=head2 Use distributed MisterHouse proxies
...
On your main mh box, change your mh.ini port parms to point to the address:port of the proxy box, like this:
 ...
 cm11_port           = proxy localhost:9001
 cm17_port           = proxy localhost:9001
 iButton_serial_port = proxy localhost:9001

You can start or restart either the main mh or mh_proxy before or after the other one and they should sync up.
So far, only the CM11, CM17 interfaces and iButton state_now and read_temp methods are proxy enabled.
Support for other iButton methods or serial interfaces can be added as needed.
...
Here are some small $400->$500 computers that might be good for distributed speech and/or ports:

 http://www.cappuccinopc.com/espressopc.asp  (6"x5"x1.5")  (0.9Lbs) .
 http://www.enpc.com/cgi-bin/enpc/ESPRESSO.html?mv_pc=Dealnews
