...
For a list of the hardware in Bruce's house, see mh/pod/faq.* quesion 6.8.
...
X10.com also sells a $50 IR transimiter that works through the CM17 interface.  
Info is at http://www.x10.com/products/ux17a_bj2.htm.  This is supported
via the mh IR_Item object.

Another nice interface is the $30 MR26A: http://www.x10.com/products/x10_mr26a.htm 
...
to transmit data over the powerline. 

Another supported X10 interface is the Marrick Lynx10-PLC controller: http://www.marrickltd.com/LynX105.htm
See mh/lib/Lynx10PLC.pm

A nice summary of the different X10 controlers is here:
http://www.geocities.com/ido_bartana/Which_controller.html

Europeans can get X10 stuff from  http://www.intellihome.be , 
http://www.hwg-telekom.de , http://www.digihouse.com.au , http://www.eon3.com, 
www.madomotique.com, www.letsautomate.com, and www.laser.com .
...
The CPU-XA, Ocelot, and Leopard controlers from Applied Digital Inc
...
In addition to the mh/code/public/irman.pl code, there there is free winamp,
...
Here are some urls and prices:
  ...
  http://www.aag.com.mx
    - Sell an ibutton weather station for $80
...
Both of these mikes are ment to be used when you are at the desktop.
...
Can process data from a keyboard simulator scanner, like the Cue Cat

The mh/code/test/barcode* members code will process data from the Cue Kat keyboard simulator
...
$Keyboard echos characters typed on the keyboard (currently windows only)
...
See mh/code/examples/test_tie.pl for more examples on how to tie/untie items/events.
...
(http://www.x10.com/commander.htm and/or ftp://ftp.x10.com/pub/applications/commander/)
...
On Unix systems, the fork function is used.  On either system, the following methods work in the same way:
  ...
  Methods:
     ...
     set_output $output_file : Program errata goes to $output_file
...
will show up as a popup window
                on the web interface, when the mouse hovers over the command text.
...
If you have a LM14 lamp module (or any other module that supports Preset Dims),
you can use the ##% preset dim states.  Set the 3rd X10_Item parm to 'preset' or 'LM14' to specify 
you are using a module that supports preset dims.  For example:
...
The older X10 modules can only be brightened/dimed relative to their current state.
If you specify a specific brightness level with ##% (e.g. 10% or 90%) when using an older
X10 module, mh will use what it thinks the current brightness level is to come up with a +-## 
...
this is useful as these commands can also be run from the Tk, web, telnet, and file interfaces.
  ...
  Methods:
     new($command, $response, $confirm, $vocabulary)
        ...
        is : "I heard $command" (on viavoice).
...
Calls up the web browser defined my the mh.ini browser parm
  ...
  Examples: 
    ...
    browser 'http://misterhouse.net:81';
...
Passes specified text to the TTS program.  There are 2 modes of calling it:
   ...
   Options are:
     ...
     These options are currently for the windows MS TTS engine only.
       ...
       voice  => 'name'
                 Example voices: Mike,Mary,Sam,random,male,female
       ...
       card   => d (d = 1,2,3...) Picks which sound card to used (V5 only)

       to_file     => xyz.  Saves speech to file xyz, then plays the file.
       compression => low,normal,high  How much to compress the to_file file.
...
=item net_jabber_send or net_im_send
...
The first time you send a message, mh will pause for a few seconds while it logs onto AIM or a jabber server.
Subsequent messages use the same signon, so are sent much faster.

Once you have been logged in, mh will also display incoming jabber messages.  This code has lots of
...
To reset the password, delete the mh/data/.password file.
...
You can use use any or all of these interfaces to walk through menus:
...
menu interface subroutines.  For the above menu.pl, you can point your WAP phone to:
  ...
  Either of these will get the 'default' test menu:
    http://misterhouse.net:8080/sub?menu_wml
    http://misterhouse.net:8080/sub?menu_wml(default)

  This will get the auto-generated 'mh' entry for all voice commands:
    http://misterhouse.net:8080/sub?menu_wml(mh)

Something like this could be used to create different menus for different LCD controlers:

  $menu_standard = read_file 'standard.menu';
  $menu_bedroom  = read_file 'bedroom.menu';
  $menu_living   = read_file 'living.menu';
  $menu_all      = read_file 'all.menu';
...
or download a phone simlator from http://phone.com .  
...
You can view the mh vxml menus by pointing a XML compatable browser (e.g. IE) to this address:
...
Then you can go to to my studio and enter your url:
...
In all of the above examples, .gif was used as an example, but any browser compatable graphics file can be used (e.g. .png and .jpg).
...
Important note:  If you do play with adding/deleteing/renaming icons, you will want to do a reload before 
...
One useful built in function is &dir_index, which can be used to create a directory
...
Here are some examples demonstrating these options:
 ...
 This will redirect the browser to mh page ia5/lights/main.shtml page:
  ...
  <a HREF="http://misterhouse.net:8080/SET;referer/ia5/lights/main.shtml?$camera_light=on">Light on</a>
...
Here is an example of using a subroutine to generate html:
   ...
   http://misterhouse.net:8080/SUB;tellme_menu(,,html)

You can also use a seperate web server (e.g. Apache or ISS) to serve pages that control MisterHouse. 
For example:
...
For more examples on code hooks, see mh/code/bruce/test_code_hooks.pl
...
mh can be download from http://misterhouse.net:81
...
The Circuit Cellar Ink article includes a diagram of how 
to set up a house PA system, so you can control which rooms MisterHouse talks to.

You can suscribe and/or view the mailing list at
...
#
# $Log$
# Revision 1.52  2002/03/31 18:50:36  winter
# - 2.66 release
#
