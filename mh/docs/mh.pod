...
If you do this, however, you need to also make sure you also make the tie* methode call 
...
Displays the specified text string, text file contents, or bitmap (.gif or .jpg).
  ...
  Simple mode:
     display $file_or_text, $time, $title, $font, $window_name;
...
Use this to add/delete an offset to a time value, using the same syntax as offsets allowed for in time_now.
   ...
   Examples:
    ...
    $Save{setback_off} = time_add "$Save{wakeup_time} - 1:20";
...
applicatino while it is running.
