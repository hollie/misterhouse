...
This new RF->X10 interface now supports all housecodes (older ones only support once housecode each):
...
Also, methods light and dark will return the current light/dark state.  For example:
...
and simply include the proxy code dir in you mh.ini code_dir parm.  For example:
...
On your main mh box, change your mh.ini port parms to point to the address:port of the proxy box, like this:
 ...
 cm11_port           = proxy localhost:8005
 cm17_port           = proxy localhost:8005
 iButton_serial_port = proxy localhost:8005
