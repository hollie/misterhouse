...
available at http://members.home.net/ncherry/common/cpuxad
...
In addition to the mh/code/bruce/irman.html code, there there is free winamp,
...
you can use the #% preset dim states.  For example:
...
By default time_cron only operates on the pass when a new minute starts.
This can be overridden if the parameter 1 is added - this may be useful if a
movement sensor is to trigger different events at different times.
...
Some examples of this can be found in the mh/web/test/*.pl files.    
