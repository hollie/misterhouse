...
module prints all events that DSC logs to the even buffer.  These events include
...
to be aware of events that DSC alarm systems log to their event buffers.
  ...
  numbers to the parms as in:
    ...
    DSC_Alarm:2_serial_port=COMy or /dev/ttysx
...
SET and RUN can use an optional response field to decide what html will be returned:
...
Activestate perl 5.6 build 613 and 616 leaks memory.  On my box about 30 Meg, once a day. 
