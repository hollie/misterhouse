...
Here is the command to check out the mh repository.  Do this someplace other than your main mh directory.  It does not include all the mh code (some of which came from CPAN and is maintained elsewhere), so you will not be able to run out of this directory.

cd \sourceforge
cvs -dusername@cvs.misterhouse.sourceforge.net:/cvsroot/misterhouse co mh
