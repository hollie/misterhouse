...
#   print "dbx1 m=$menu_group, m=$menus, m=$menu, i=$item, xy=$xy, x=$x\n";
    ...
    my ($menu_group, $menus);
    ($menu_group, $menus) = &get_menu_default($Socket_Ports{http}{client_ip_address});
    ($menu_group, $menus) = &get_menu_default('default') unless $menu_group;
    $menus .= "Top|$Menus{$menu_group}{_menu_list}[0]" if $menu_group and !$menus;
...
sub menu_list_top {
    ...
    for my $menu_group (sort keys %Menus) {
        next unless $Menus{$menu_group}{_menu_list} and @{$Menus{$menu_group}{_menu_list}} > 0; # Skip empty menus
        next if $menu_group eq 'menu_data';
        $html .= &menu_button($menu_group, "/bin/menu.pl?$menu_group");
        $html .= "&nbsp;&nbsp;&nbsp;&nbsp;" unless $menu_buttons;
    }
    $html .= 'No menus defined.  Enable menu.pl in the <a href="/bin/code_select.pl">common code selector</a>' unless %Menus > 1;
...
sub menu_list {
    ...
    for my $menu_prev (@menus) {
        if ($menus_prev1) {
            $html .= ($menu_buttons) ? "<img src='/graphics/arrow1.gif'>\n" : "&nbsp;&nbsp;&nbsp;&nbsp;";
            $menus_prev1 .= '|';
        }
...
#   $html .= "\n";
    $html .= "<hr>\n";
    $html .= &menu_states($menu_group, $menu_with_state, $item_with_state) if $menu;
    return &html_top . $html;
}
