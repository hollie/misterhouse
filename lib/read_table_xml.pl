...
$itemtype{"RF"} = {object=>"RF_Item",use_name=>1,
			min_comma=>2};
$itemtype{"COMPOOL"} = {object=>"COMPOOL_HAS_OPEN_ISSUES_UNCONVERTED",
			use_name=>1};
...
$itemtype{"MP3PLAYER"} = {object=>"Mp3Player",
			use_other=>0,
...
$itemtype{"WAKEONLAN"} = {object=>"WakeOnLan",
		   "init"=> "require 'WakeOnLan.pm';\n"};
$itemtype{"YACCLIENT"} = {object=>"CID_Server_YAC",
		   "init"=> "require 'CID_Server.pm';\n"};
$itemtype{"ZONE"} = {object=>"Sensor_Zone"};

$itemtype{"LIGHT"} = {object=>"Light_Item",
			"use_addr"=>"obj_ref",
		   "init"=> "require 'Light_Item.pm';\n"};
$itemtype{"DOOR"} = {object=>"Door_Item",
			"use_addr"=>"obj_ref",
		   "init"=> "require 'Door_Item.pm';\n"};
$itemtype{"MOTION"} = {object=>"Motion_Item",
			"use_addr"=>"obj_ref",
		   "init"=> "require 'Motion_Item.pm';\n"};
$itemtype{"PHOTOCELL"} = {object=>"Photocell_Item",
			"use_addr"=>"obj_ref",
...
$itemtype{"OCCUPANCY"} = {object=>"Occupancy_Monitor",
			"use_addr"=>0,
			"trailing_space_count"=>1,
		   "init"=> "require 'Occupancy_Monitor.pm';\n"};
$itemtype{"PRESENCE"} = {object=>"Presence_Monitor",
			"use_addr"=>"obj_ref",
...
sub construct_object{
	...
	if(exists $itemtype{$x_type}){
		...
		if($object_hash->{"use_addr"}  ## if address allowed for objecttype
		&& $q_address){   ## if "address" data exists, tack it on.
		...
		if($object_hash->{"use_name"}  ## rf,x10ms have address:name:other
		&& $x_name){   ## tack on the name, and quote it.
			$object .= ", '".$x_name . "'";
		}
		if($object_hash->{"use_occupancy"}  ## Presence monitor
		&& $x_occupancy){   ## tack on the name, and quote it.
			if($object_hash->{"use_occupancy"} eq "obj_ref"){
				$object .= ', $' . $x_occupancy;
			}
		}

		if($object_hash->{"use_other"}  ## if other allowed for objecttype
		&& $q_other_string){   ## if "other" data exists, tack it on.
...
#	print "testing for existence of group: [$group]\n";
   ...
   $code .= sprintf "\$%-35s -> add(\$%s);\n", $group, $x_name unless $groups{$group}{$x_name};
   $groups{$group}{$x_name}++;
