...
sub set
{
	...
	&::print_log("Motion_Item::set name: $name state: $p_state") if $main::Debug{occupancy};
	...
	if ($p_setby eq $$self{m_timerCheck}) { # Check timer expired
	    &::print_log("Motion_Item:: name: $name m_timerCheck") if $main::Debug{occupancy};
		...
		&::print_log("$$self{object_name}->Has not received motion in $hours hours") if $main::Debug{occupancy};
	    }
	    ...
	    &::print_log("Motion_Item:: name: $name m_timeout") if $main::Debug{occupancy};
	    $p_state='still';
	} else {
		...
		&::print_log("Motion_Item:: name: $name state: $p_state") if $main::Debug{occupancy};
		$$self{m_timeout}->set(2*60,$self);
		if ($$self{'inactivity_time'}) {
		    $$self{m_timerCheck}->set($$self{'inactivity_time'}, $self);
		}
...
sub set_inactivity_alarm($$$) {
   ...
   my $name = $$self{object_name};
   &::print_log("$name->set_inactivity_alarm for $time hours action->$action") if $main::Debug{occupancy};
}
