...
sub set {
    ...
    if ( ref $p_setby ) {
        if ( $p_setby->can('get_set_by') ) {
            &::print_log( "Light_Switch_Item($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by "
                  . $p_setby->get_set_by )
              if $main::Debug{occupancy};
        }
        else {
            &::print_log( "Light_Switch_Item($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name}" )
              if $main::Debug{occupancy};
        }
    }
