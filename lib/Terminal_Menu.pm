...
sub new {
    ...
    &::print_log("Terminal_Menu.pm: new \$p_menugroup=$p_menugroup \$self=$self") if $::Debug{'Terminal_Menu'};
...
sub set {
    ...
    &::print_log("Terminal_Menu.pm: set \$state=$state, \$setby=$setby)") if $::Debug{'Terminal_Menu'};
    ...
    if ($state =~ m/^SIZE\s+(\d+)\s+(\d+)\s*$/i) {
        ...
        $$self{border} = ($$self{useborder} and $$self{terminal_width} >= 22 and $$self{terminal_height} >= 18) ? 1 : 0;
        $updatescreen = 1;
    }
        ...
        my @colorcodes = qw(clear reset dark bold underline underscore blink reverse concealed black red green yellow blue magenta on_black on_red on_green on_yellow on_blue on_magenta on_cyan on_white); 
...
sub loop {
    ...
    if ($$self{last_response_loop} and $$self{last_response_loop} <= $main::Loop_Count) {
        $$self{last_response_loop} = 0;
...
sub _select_menu {
    ...
    &::print_log("Terminal_Menu.pm: _select_menu \$self=$self \$p_menu=$p_menu") if $::Debug{'Terminal_Menu'} > 1;
    ...
    return unless defined $$menus{$$self{menuCurrent}}; # Guard against bad menus
...
sub _select_item {
    ...
    &::print_log("Terminal_Menu.pm: _select_item \$self=$self \$p_item=$p_item") if $::Debug{'Terminal_Menu'} > 1;	
    ...
    if ($$item{A}) { 
        if ($$item{Dstates}) {
            ...
            my $l_item = $p_item - 1;
            my $l_itemPrevious = ${$$self{itemList}}[$$self{menuDepth}-1] - 1;
            my $l_menuPrevious = ${$$self{menuList}}[$$self{menuDepth}-1];
...
sub _exit_menu {
    ...
    &::print_log("Terminal_Menu.pm: _exit_menu \$self=$self") if $::Debug{'Terminal_Menu'} > 1;	
...
sub _response {
    ...
    &::print_log("Terminal_Menu.pm: _response \$self=$self \$response=$response") if $::Debug{'Terminal_Menu'} >= 2;
    ...
    if (defined $$self{response} and ($$self{response} =~ m/\n/s  or  length($$self{response}) > $$self{terminal_width})) {
        $$self{zoomed} = 1;
...
sub _handle_normal {
    ...
    if ($cmd eq "*") {
        ...
        if ($$self{menu_morepages}) {
            ...
            $$self{menu_offset} = 0 if ($$self{menu_offset} > (scalar(@{$$self{currentmenu}}) -2));
            $self->_draw_screen();
        }
    }
    elsif ($cmd eq "UP") {
...
sub _draw_response {
    ...
    $$self{zoomed} ? $self->_draw_zoomed_response() : $self->_draw_normal_response();
}
...
sub _draw_zoomed_response {
    ...
    foreach my $line (@lines) {
        ...
        last if ($$self{cursor_y} >= $$self{terminal_height} - 1 - ($$self{border} ? 1 : 0));
    }
...
sub DESTROY {
    my ($self) = @_;
    &::print_log("Terminal_Menu.pm: DESTROY \$self=$self)") if $::Debug{'Terminal_Menu'};	
}
