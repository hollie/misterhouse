...
sub startup {
    return if $started++;       # Allows us to call with $Reload or with xpl_module mh.ini parm
...
#              $o -> SUPER::set($state_value, 'xPL') if defined $state_value;
               if (defined $state_value and $state_value ne '') {
                  my $set_by_name = 'xPL';
                  $set_by_name .= " [$source]"; # no longer needed: if ($::config_parms{'xpl_use_to_target'});
...
sub send_xpl_heartbeat {
    ...
    if ($xpl_send) {
       ...
       print "Error in xPL_Item::send_heartbeat.  xPL send socket not available.\n";
