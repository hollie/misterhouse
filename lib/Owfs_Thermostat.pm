...
sub add_thermometer {
  ...
  &main::print_log ("Owfs_Thermostat::adding_thermometer: $thermometer zone: $zone") if $::Debug{owfs};
  $self->{thermometers}{$thermometer} = new Owfs_Item ( $thermometer, $location );
  ...
  if (exists $self->{cool_relay}) {
    ...
    if ( $current != $value ) {
      if ( !$value && $self->{t_on_timer}->active( )) {
        &main::print_log ( "Waiting for on_timer to expire ...") if $::Debug{owfs};
      } elsif ( $value && $self->{t_off_timer}->active( )) {
        &main::print_log ( "Waiting for off_timer to expire ...") if $::Debug{owfs};
      } else {
  ...
  if ( ($self->{index} % $numThermometers) == 0) {
    ...
    foreach my $zone (@{$self->{zone}}) {
      if (exists $zone->{name}) {
        ...
        my $zoneNum = exists ($zone->{temps}) ? @{$zone->{temps}} : ( );
        if ($zoneNum) {
          my $zoneSum = 0;
