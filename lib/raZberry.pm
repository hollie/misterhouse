...
#"http://192.168.0.155:8083/ZWaveAPI/Run/devices[x].data.isFailed.value"
  	my ($self,$device) = @_;
  	my ($devid,$instance,$class) = (split /-/,$device)[0,1,2];
  	&main::print_log("[raZberry] Checking $device ($devid)...") if ($self->{debug});
	my $cmd;
...
sub update_dev {
    ...
    $cmd = "%5B" . $devid . "%5D.instances%5B" . $instance . "%5D.commandClasses%5B" . $class ."%5D.Get()";
  	&main::print_log("[raZberry] Getting local state from $device ($devid)...") if ($self->{debug});
...
sub set {
   ...
   if ($p_setby eq 'poll') {
	   ...
	   if ((lc $p_state eq "up") or (lc $p_state eq "down") or (lc $p_state eq "stop")) {
           $$self{master_object}->set_dev($$self{devid},$p_state); 
       	#} elsif (($p_state eq "100%") or ($p_state =~ m/^\d{1,2}\%$/)) {
		#		my ($n_state) = ($p_state =~ /(\d+)%/);
		#	$$self{master_object}->set_dev($$self{devid},"level=$n_state");
