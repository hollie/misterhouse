...
sub process_data {
    ...
    for my $key ( keys %{ $self->{data}->{programs} } ) {
        ...
        if ( $previous ne $self->{data}->{programs}->{$key}->{status} ) {
            main::print_log( "[OpenSprinkler] Program $key changed from $previous to $self->{data}->{programs}->{$key}->{status}" )
              if ( $self->{loglevel} );
    ...
    if ( $self->{previous}->{info}->{state} ne $self->{data}->{info}->{state} ) {
        main::print_log( "[OpenSprinkler] State changed from $self->{previous}->{info}->{state} to $self->{data}->{info}->{state}" )
          if ( $self->{loglevel} );
