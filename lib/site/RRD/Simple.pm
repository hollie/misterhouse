...
$DEBUG ||= $ENV{DEBUG} ? 1 : 0;
$DEFAULT_DSTYPE ||= exists $ENV{DEFAULT_DSTYPE} ? $ENV{DEFAULT_DSTYPE} : 'GAUGE';
...
<<<<<<< Updated upstream
    ...
    cluck( 'Unrecognised parameters passed: ' . join( ', ', grep( !/^$validkeys$/, keys %{$stor} ) ) )
      if ( grep( !/^$validkeys$/, keys %{$stor} ) && $^W );

    $stor->{rrdtool} = _find_binary( exists $stor->{rrdtool} ? $stor->{rrdtool} : 'rrdtool' );
...
sub create {
    ...
    for my $ds ( sort keys %ds ) {
        $ds =~ s/[^a-zA-Z0-9_-]//g;
        push @def, sprintf( 'DS:%s:%s:%s:%s:%s', substr( $ds, 0, 19 ), uc( $ds{$ds} ), ( $rrdDef->{heartbeat} || 600 ), 'U', 'U' );
    }
    ...
    for my $cf ( sort keys %cf ) {
        for my $rra ( @{ $cf{$cf} } ) {
            push @def, sprintf( 'RRA:%s:%s:%s:%s', $cf, 0.5, $rra->{step}, $rra->{rows} );
        }
    }
...
sub update {
    ...
    unless ( -f $rrdfile ) {
        ...
        cluck( "RRD file '$rrdfile' does not exist; attempting to create it ", "using default DS type of '$default_dstype'" ) if $^W;
        my @args;
        for ( my $i = 0; $i < @_; $i++ ) {
    ...
    for my $ds ( sort keys %ds ) {
        ...
        if ( !grep( /^$ds$/, @sources ) ) {
            TRACE( "Supplied data source '$ds' does not exist in pre-existing " . "RRD data source list: " . join( ', ', @sources ) );

            # If someone got the case wrong, remind and correct them
            if ( grep( /^$ds$/i, @sources ) ) {
                cluck( "Data source '$ds' does not exist; automatically ", "correcting it to '", ( grep( /^$ds$/i, @sources ) )[0], "' instead" ) if $^W;
                ...
                if ( $stor->{on_missing_ds} eq 'ignore' ) {
                    ...
                    TRACE( "on_missing_ds = add (or not set at all/default); " . "automatically adding new data source '$ds'" );
...
sub add_source {
    ...
    my $heartbeat =
      $info->{ds}->{ ( sort { $info->{ds}->{$b}->{minimal_heartbeat} <=> $info->{ds}->{$b}->{minimal_heartbeat} } keys %{ $info->{ds} } )[0] }
      ->{minimal_heartbeat};
    ...
    # Add the data source
    my $new_rrdfile = '';
    eval { $new_rrdfile = _modify_source( $rrdfile, $stor, $ds, 'add', $dstype, $heartbeat, ); };
    ...
    unless ( -f $new_rrdfile ) {
        croak "Failed to add new data source '$ds' to RRD file '$rrdfile': ", "new RRD file '$new_rrdfile' does not exist";
    }

    # Barf is the new data source isn't in our new RRD file
    unless ( $TgtSources eq join( ',', sort( $self->sources($new_rrdfile) ) ) ) {
        croak "Failed to add new data source '$ds' to RRD file '$rrdfile': ", "new RRD file '$new_rrdfile' does not contain expected data ", "source names";
    }
...
sub heartbeat {
    ...
    my $rrdfile =
        @_ >= 3 ? shift
      : _isLegalDsName( $_[0] ) && $_[1] =~ /^[0-9]+$/ ? _guess_filename($stor)
      :                                                  shift;
    croak "RRD file '$rrdfile' does not exist" unless -f $rrdfile;
    TRACE("Using filename: $rrdfile");
...
sub last_values {
    ...
    my @def = ( 'LAST', '-s', $lastUpdated - ( $largestHeartbeat * 2 ), '-e', $lastUpdated );
...
sub _create_graph {
    ...
    my $basename =
      defined $param{'basename'} && $param{'basename'} =~ /^[0-9a-z_\.-]+$/i
      ? $param{'basename'}
      : ( fileparse( $rrdfile, '\.[^\.]+' ) )[0];
    delete $param{'basename'};

    # Define where to write the image
    my $image = sprintf( '%s-%s.%s', $basename, _alt_graph_name($type), lc( $param{'imgformat'} ) );
    ...
    my $timestamp =
      !defined $param{'timestamp'} || $param{'timestamp'} !~ /^(graph|rrd|both|none)$/i
      ? 'graph'
      : lc( $param{'timestamp'} );
    ...
    my $line_thickness = defined $param{'line-thickness'}
      && $param{'line-thickness'} =~ /^[123]$/ ? $param{'line-thickness'} : 1;
    ...
    my @ds          = !exists $param{'sources'}
      ...
      #? @{$param{'sources'}}
      : defined $param{'sources'} ? _convert_to_array( $param{'sources'} )
      :                             ();
    ...
    if ( defined $param{'source-labels'} ) {
        if ( ref( $param{'source-labels'} ) eq 'HASH' ) {
            ...
            if ( defined $param{'sources'} && ref( $param{'sources'} ) eq 'ARRAY' ) {
                ...
                carp "source_labels may only be an array if sources is also " . "an specified and valid array";
            }
        }
    }
    ...
    if ( defined $param{'source-drawtypes'} ) {
        if ( ref( $param{'source-drawtypes'} ) eq 'HASH' ) {
            ...
            if ( defined $param{'sources'} && ref( $param{'sources'} ) eq 'ARRAY' ) {
                ...
                carp "source_drawtypes may only be an array if sources is " . "also an specified and valid array";
            }
        }
        ...
        while ( my ( $k, $v ) = each %source_drawtypes ) {
            if ( $v !~ /^(LINE[1-9]?|STACK|AREA)$/ ) {
                ...
                carp "source_drawtypes may be LINE, LINEn, AREA or STACK " . "only; value '$v' is not valid" if $^W;
            }
    ...
    if ($extended_legend) {
        for my $ds (@ds) {
            my $len = length( defined $source_labels{$ds} ? $source_labels{$ds} : $ds );
...
<<<<<<< Updated upstream
    ...
    for my $ds (@ds) {
        ...
        push @cmd,
          sprintf( $fmt,
            ...
            ( defined $source_labels{$ds} ? $source_labels{$ds} : $ds ), $stack );
    ...
    if ( $timestamp ne 'none' ) {
        ...
        if ( $timestamp eq 'rrd' || $timestamp eq 'both' ) {
            my $time = sprintf( 'RRD last updated: %s\r', strftime( $timefmt, localtime( ( stat($rrdfile) )[9] ) ) );
            $time =~ s/:/\\:/g if $RRDs::VERSION >= 1.2;    # Only escape for 1.2
            push @cmd, "COMMENT:$time";
        }

        if ( $timestamp eq 'graph' || $timestamp eq 'both' ) {
            my $time = sprintf( 'Graph last updated: %s\r', strftime( $timefmt, localtime(time) ) );
            $time =~ s/:/\\:/g if $RRDs::VERSION >= 1.2;    # Only escape for 1.2
            push @cmd, "COMMENT:$time";
        }
    }
...
<<<<<<< Updated upstream
    ...
    if ( $type =~ /^(week|month|year|3years)$/i ) {
        ...
        push @{ $rra->{rra} },
          {
            step => int( 120 / $step ),
            rows => int( _minutes_in( $type eq 'week' ? 'week' : 'month', 125 ) / int( 120 / $step ) )
          };    # 2 hour average
    }
...
<<<<<<< Updated upstream
    ...
    if ( $cmd =~ /^([\/\.\_\-a-zA-Z0-9 >]+)$/ ) {
        ...
        if ( $? == -1 ) {
            ...
            croak(
                sprintf( "While executing command '%s', child died " . "with signal %d, %s coredump\n", $cmd, ( $? & 127 ), ( $? & 128 ) ? 'with' : 'without' )
            );
        }
        my $exit_value = $? >> 8;
...
sub _find_binary {
    ...
    push @paths, File::Spec->catdir( $rrds_path, File::Spec->updir(), File::Spec->updir(), 'bin' );
