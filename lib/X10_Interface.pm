...
sub processData {
	...
	&main::print_log("X10: Outgoing data=$data") if $main::config_parms{x10_errata} >= 4;
	...
	while ($data) {
		if ($data =~ /^([A-P]STATUS_OFF)(\S*)/ or
			$data =~ /^([A-P]STATUS_ON)(\S*)/ or
			$data =~ /^([A-P]STATUS)(\S*)/ or
			$data =~ /^([A-P]ALL_LIGHTS_OFF)(\S*)/ or
			$data =~ /^([A-P]EXTENDED_CODE)(\S*)/ or
			$data =~ /^([A-P]EXTENDED_DATA)(\S*)/ or
			$data =~ /^([A-P]HAIL_REQUEST)(\S*)/ or
			$data =~ /^([A-P]HAIL_ACK)(\S*)/ or
			$data =~ /^([A-P]PRESET_DIM1)(\S*)/ or
			$data =~ /^([A-P]PRESET_DIM2)(\S*)/ or
			$data =~ /^([A-P][1][0-6])(\S*)/ or
			$data =~ /^([A-P][1-9A-W])(\S*)/ or
			$data =~ /^([A-P]\&P\d+)(\S*)/ or         # extended direct dim cmd
			$data =~ /^([A-P]Z\S*)/ or                # Extended Code cmd with arbitrary extended bytes
