...
sub check_for_data {
    ...
    foreach $NewCmd (@NewCmds) {
        ...
        if ( ( length($NewCmd) ) eq "20" ) {
            ...
            if ( $AckMsg eq '' ) {
                $AckMsg = ( $CmdAck{ ( substr( $NewCmd, 0, 18 ) ) } );
            }    # try stripping the checksum
            if ( $AckMsg eq '' ) {
                $AckMsg = ( $CmdAck{ ( substr( $NewCmd, 0, 16 ) ) } );
            }    # strip last 2 for vol caculations
            &main::print_log("[HARMON] - Ack $AckMsg - Hex $NewCmd ");
            $AckMsg = &GetAckMsg( $AckMsg, $NewCmd )
              if &GetAckMsg( $AckMsg, $NewCmd );
            my @AckMsgs;
...
sub set_child_state {
    ...
    if ( $object_type eq 'mute_object' ) {
        ...
        if ( $$child{mute_cmd} eq 'ON' and $msg eq 'OFF' ) {
            $child->set( 'Z' . $zone_num . '_MUTE' );
            $$child{mute_cmd} = 0;
        }
        elsif ( $$child{mute_cmd} eq 'OFF' and $msg eq 'ON' ) {
            $child->set( 'Z' . $zone_num . '_MUTE' );
            $$child{mute_cmd} = 0;
        }
    }
...
sub new {
    ...
    $$self{zone}     = $zone;
    @{ $$self{states} } = (
        'SAT',     'BLURAY', 'BRIDGE', 'DVR',   'SIRIUS', 'FM',
        'AM',      'TV',     'GAME',   'MEDIA', 'AUX',    'INET-RADIO',
        'NETWORK', 'SRC-A',  'SRC-B',  'SRC-C', 'SRC-D'
    );
    return $self;
}
...
sub set {
    my ( $self, $p_state, $p_setby, $p_response ) = @_;
    ::print_log(
        "[HARMON::Control] Received request " . $p_state . " for " . $self->get_object_name . " for zone " . $$self{zone} );
    if ( $p_state =~ /^Z\d_(.*)/ ) {
        $$self{receiver}->set($p_state);
