...
&triggers_read if -e $trigger_file;
&trigger_write_code;

sub triggers_loop {
    &triggers_save if $trigger_write_code_flag or ($New_Hour and %triggers);
...
sub triggers_save {
    ...
    if ($data) {
     	print_log "triggers_save: $i2 expired, $i1 saved" if $i2 or $Debug{'trigger'};
    	$data1 = '#
...
sub trigger_write_code {
    ...
    if ($script) {
    	$script = "#
# You shouldn't edit this file.  This file is auto-generated by mh/lib/trigger_code.pl.
# If there are syntax errors here, you should delete this file and edit
# $::config_parms{data_dir}/triggers.current.  This file will be recreated when
# Misterhouse is next started.
#
" . $script;
    	print_log "trigger_write_code: this sub was called, but triggers not changed", return if $script eq $prev_script;
    	$prev_script = $script;
    	&file_write($script_file, $script);
                                # Replace (faster) or reload (if there was no file previously)
    	if ($main::Run_Members{'triggers_table'}) {
   	    print_log "trigger_write_code: trigger script $script_file written, running do_user_file" if $Debug{'trigger'};
            &do_user_file("$::Code_Dirs[0]/triggers.mhp");
    	}
    	else {
                                # Must be done before the user code eval
   	    print_log "trigger_write_code: trigger script $script_file written, running read_code" if $Debug{'trigger'};
