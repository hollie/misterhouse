...
" . $script;
    	print_log "trigger_write_code: this sub was called, but triggers"
          . " not changed", return if $script eq $prev_script;
    	$prev_script = $script;
    	...
    	if ($main::Run_Members{'triggers_table'}) {
   	    print_log "trigger_write_code: trigger script $script_file"
              . " written, running do_user_file" if $Debug{'trigger'};
...
sub trigger_expire {
    my $name = shift;
    return unless exists $triggers{$name} and $triggers{$name}{type}
      eq 'OneShot';
    my $trigger   = $triggers{$name}{trigger};
    my $code      = $triggers{$name}{code};
    my $type      = 'Expired';
...
sub trigger_active {
    ...
    return (exists $triggers{$name} and
      ($triggers{$name}{type} eq 'NoExpire' or $triggers{$name}{type}
      eq 'OneShot') and (not exists $triggers{$name}{'trigger_error'})
    );
}
