    ...
    if (@candidate_devices) {
        foreach (@candidate_devices) {
                ...
                and !( $candidate_object->isa('Insteon::InterfaceController') ) )
            {
                ...
                &main::print_log(
                    "[Scan all linktables] INFO: !!! " . $candidate_object->get_object_name . " is NOT a candidate for scanning." );
            }
        }
    }
    else {
...
sub check_all_aldb_versions {
    ...
    foreach my $ALDB_device (@ALDB_devices) {
        ...
        {
            main::print_log(
                "[Insteon] DEBUG4 Checking aldb version for " . $ALDB_device->get_object_name() . " ($count of $ALDB_cnt)" )
              if ( $ALDB_device->debuglevel( 4, 'insteon' ) );
            $ALDB_device->check_aldb_version();
        }
        else {
            main::print_log(
                "[Insteon] DEBUG4 " . $ALDB_device->get_object_name . " does not have its own aldb ($count of $ALDB_cnt)" )
              if ( $ALDB_device->debuglevel( 4, 'insteon' ) );
        }
    }
