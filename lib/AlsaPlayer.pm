            ...
            if ( $key eq 'name' ) {
                ...
                if ( $sessions[$id] ) {
                        ...
                        and ( not @{ $sessions[$id]->{'queue'} } ) )
                    {
                        ...
                        and ( not @{ $sessions[$id]->{'queue'} } ) )
                    {
                        &::print_log( "AlsaPlayer($sessions[$id]->{session_name}): Pausing because it is playing but shouldn't be" )
                          if $main::Debug{alsaplayer};
...
sub _reconnect {
    my ($self) = @_;
    &::print_log( "AlsaPlayer($$self{session_name}): _reconnect(): session=$$self{'session'}, pending=$$self{'pending'}" )
      if $main::Debug{alsaplayer};
...
sub _died {
    my ($self) = @_;
    &::print_log( "AlsaPlayer($$self{session_name}): _died(): session=$$self{'session'}, pending=$$self{'pending'}" )
      if $main::Debug{alsaplayer};
    if ( $$self{'session'} >= 0 ) {
...
sub _remove_playlist_files {
    ...
    foreach ( keys %{ $$self{'playlist'} } ) {
        ...
        if ( $$self{'playlist'}->{$_} > 0 ) {
            push @{ $$self{'pending_playlist'} }, $_;
            &::print_log("AlsaPlayer($$self{session_name}): keeping '$_' in playlist")
              if $main::Debug{alsaplayer};
        }
        else {
...
sub _scan_sessions() {
    for ( my $i = 0; $i < MAX_SESSIONS; $i++ ) {
        if ( $sessions[$i] ) {
            ...
            {
                &::print_log( "AlsaPlayer($sessions[$i]->{session_name}): Replace=$sessions[$i]->{'replace'}" )
                  if $main::Debug{alsaplayer};
                ...
                and $sessions[$i]->{'control'}->done() )
            {
                ...
                while ( $song = shift @{ $sessions[$i]->{'pending_playlist'} } ) {
                    ...
                    if (   ( length $songs > 4000 )
                        or ( ( length $songs > 500 ) and ( $cmd eq 'replace' ) ) )
                    {
                        last;
                    }
                }
                ...
                if ($songs) {
                    ...
                    $sessions[$i]->{'control'}
                      ->set( "$::config_parms{alsaplayer_binary} -n '$sessions[$i]->{'session'}' --$cmd $songs" );
        ...
        unless ( $sessions[$i] ) {
            if ( @pending or @check_once ) {
                if ( $label = &_get_status($i) ) {
                    &::print_log("AlsaPlayer: scanned id $i and found label: $label")
                      if $main::Debug{alsaplayer};
                    if (@pending) {
...
sub unpause {
    ...
    $$self{'paused'} = 0;
    &::print_log("AlsaPlayer($$self{session_name}): setting paused to 0 in unpause()")
      if $main::Debug{alsaplayer};
}
...
sub pause {
    ...
    unless ( $$self{'paused'} ) {
        ...
        $$self{'paused'} = 1;
        &::print_log("AlsaPlayer($$self{session_name}): setting paused to 1 in pause()")
          if $main::Debug{alsaplayer};
    }
}
...
sub pause_toggle {
    ...
    &::print_log( "AlsaPlayer($$self{session_name}): setting paused to $$self{paused} in pause_toggle()" )
      if $main::Debug{alsaplayer};
}
...
sub start {
    my ($self) = @_;
    &::print_log( "AlsaPlayer($$self{session_name}): start(): session=$$self{'session'}, pending=$$self{'pending'}" )
      if $main::Debug{alsaplayer};
    if ( ( $$self{'session'} >= 0 ) or ( $$self{'pending'} ) ) {
        ...
        $self->_queue_cmd('start');
        $$self{'paused'} = 0;
        &::print_log("AlsaPlayer($$self{session_name}): setting paused to 0 in start()")
          if $main::Debug{alsaplayer};
        return;
    }
    $$self{'halted'} = 0;
    for ( my $k = 0; $k <= $#check_once; $k++ ) {
        if ( $self eq $check_once[$k] ) {
            ...
            &::print_log( "AlsaPlayer($$self{session_name}): waiting to see if process is already running..." )
              if $main::Debug{alsaplayer};
            $$self{'pending'} = 1;
            return;
        }
    ...
    &::print_log("AlsaPlayer($$self{session_name}): About to execute: '$program'")
      if $main::Debug{alsaplayer};
    ...
    $$self{'pending'} = 1;
    $$self{'replace'} = 1;
    &::print_log("AlsaPlayer($$self{session_name}): setting paused to 1 in start()")
      if $main::Debug{alsaplayer};
