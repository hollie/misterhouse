...
sub mqtt_reconnect() {
    ...
    &main::print_log( "*** mqtt $$self{instance} mqtt_connect Socket ($$self{host}:$$self{port},$$self{keep_alive_timer}) " )
    ...
    &main::print_log( "*** mqtt $$self{instance} Socket check #1 ($$self{keep_alive_timer}) [ $! ]: "
    ...
    &main::print_log( "*** mqtt $$self{instance} Socket check #2 ($$self{keep_alive_timer}) [ $! ]: "
          . ( $self->isConnected() ? "Connected" : "Failed" ) )
      if ( $main::Debug{mqtt} );
...
sub mqtt_connect() {
    ...
    &main::print_log( "*** mqtt mqtt_connect Socket ($$self{host}:$$self{port},$$self{keep_alive_timer}) " )
      if ( $main::Debug{mqtt} || 1 );
    ...
    ### 3) Check for ACK or fail
    &main::print_log(
        "*** mqtt Socket check ($$self{keep_alive_timer}) [ $! ]: " . ( $self->isConnected() ? "Connected" : "Failed" ) )
      if ( $main::Debug{mqtt} );
...
sub read_mqtt_msg {
    ...
    do {
        ...
        unless ($bytes) {
            &main::print_log( "*** mqtt $$self{instance}: read_mqtt_msg Socket closed "
    ...
    if ( defined($p_setby) && $p_setby eq $self->interface() ) {
        ...
        if ( $main::Debug{mqtt} ) {
            if ( defined( $self->get_object_name() ) ) {
                &::print_log(
                    "*** mqtt mqtt_Item nom to MH to MQTT (" . $self->get_object_name() . ') no p_setby ::set($msg, $p_setby)' );
            }
            else {
