...
sub main::ia7_update_schedule {
    ...
    &main::print_log( "Updating Schedule for object $object, schedule size is "
          . scalar(@schedules) );
    ...
    for ( my $i = 1; $i <= ( scalar(@schedules) / 3 ); $i++ ) {
        ...
        &main::print_log( "Adding Schedule (id="
              . $schedules[ $i * 3 - 3 ]
              . " cron="
              . $schedules[ $i * 3 - 2 ]
              . " label="
...
sub ia7_update_collection {
    ...
    for my $file (@collection_files) {
        if ( -e $file ) {
            &main::print_log(
                "[IA7_Collection_Updater] : Reviewing $file to current version $ia7_coll_current_ver"
            );
            my $json_data;
            my $file_data;
            eval {
                $file_data = &main::file_read($file);
                $json_data = decode_json($file_data)
                  ;    #HP, wrap this in eval to prevent MH crashes
            };

            if ($@) {
                &main::print_log(
                    "[IA7_Collection_Updater] : WARNING: decode_json failed for $file. Please check this file!"
                );
            }
            else {
                my $updated = 0;
                ...
                {    #IA7 v1.2 required change
                    ...
                    &main::print_log(
                        "[IA7_Collection_Updater] : Updating $file to version 1.2"
                    );
                    $updated = 1;
                }
                if ($updated) {
                    my $json_newdata =
                      to_json( $json_data, { utf8 => 1, pretty => 1 } );
                    my $backup_file =
                        $file . ".t"
                      . int( ::get_tickcount() / 1000 )
                      . ".backup";
                    &main::file_write( $backup_file, $file_data );
                    &main::print_log( "[IA7_Collection_Updater] : Saved backup "
                          . $file . ".t"
                          . int( ::get_tickcount() / 1000 )
                          . ".backup" );
