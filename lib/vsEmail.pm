...
sub ValidAddress {
    ...
    my $address = shift;
    if (   ( $address =~ /(@.*@)|(\.\.)|(@\.)|(\.@)|(^\.)/ )
        || ( $address !~ /^.+\@(\[?)[a-zA-Z0-9\-\.]+\.([a-zA-Z]{2,3}|[0-9]{1,3})(\]?)$/ ) )
    {
        return 0;
    }
    else {
...
sub time_to_date {
    ...
    return join( " ", ( $wdays[$wday] . ',' ), $mday, $months[$mon], $year + 1900, sprintf( "%02d", $hour ) . ":" . sprintf( "%02d", $min ), $TZ );
...
sub sendmail {
    ...
    my ( %mail, $k, $smtp, $server, $port, $connected, $localhost, $message, $fromaddr, $recip, @recipients, $to, $header, );
