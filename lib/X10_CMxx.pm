...
sub check_for_data {
    ...
    foreach my $line ( split( /\n/, $buffer ) ) {
        ...
        if ( $duplicate_count == $duplicate_threshold ) {
            ...
            &::print_log( "CMxx: Ignoring first send of X10RF data from mochad (looking for confirmation resend): $line" )
              if $main::Debug{cmxx};
        }
        else {
            &::print_log( "CMxx: Ignoring duplicate X10RF data from mochad (dupe cnt >= $duplicate_count): $line" )
              if $main::Debug{cmxx};
        }
    }
}
