...
sub process_incoming_data {
        ...
        and $tedchars == $tedcount )
    {
        ...
        $main::Electric{DlrTdySum} =
          ( ( $pkt[159] * 256 * 256 * 256 ) + ( $pkt[158] * 256 * 256 ) + ( $pkt[157] * 256 ) + ( $pkt[156] ) );
        $main::Electric{DlrTdy} = $main::Electric{DlrTdySum} / 600000;
        $main::Electric{WattTdySum} =
          ( ( $pkt[163] * 256 * 256 * 256 ) + ( $pkt[162] * 256 * 256 ) + ( $pkt[161] * 256 ) + ( $pkt[160] ) );
        $main::Electric{KwTdy} = $main::Electric{WattTdySum} / 60000;
        $main::Electric{KwhMtdCnt} =
          ( ( $pkt[167] * 256 * 256 * 256 ) + ( $pkt[166] * 256 * 256 ) + ( $pkt[165] * 256 ) + ( $pkt[164] ) );
        $main::Electric{KwhMtdSum} =
          ( ( $pkt[171] * 256 * 256 * 256 ) + ( $pkt[170] * 256 * 256 ) + ( $pkt[169] * 256 ) + ( $pkt[168] ) ) / 60000;
        $main::Electric{DlrMtdSum} =
          ( ( $pkt[175] * 256 * 256 * 256 ) + ( $pkt[174] * 256 * 256 ) + ( $pkt[173] * 256 ) + ( $pkt[172] ) ) / 600000;
        ...
        $main::Electric{VRmsNow_1} =
          ( ( $pkt[273] * 256 * 256 * 256 ) + ( $pkt[272] * 256 * 256 ) + ( $pkt[271] * 256 ) + ( $pkt[270] ) );
        $main::Electric{VRmsNow_2} =
          ( ( $pkt[277] * 256 * 256 * 256 ) + ( $pkt[276] * 256 * 256 ) + ( $pkt[275] * 256 ) + ( $pkt[274] ) );
