        ...
        unless ( $VTxt_version eq 'msv4' ) {
            if ( my $test = Win32::OLE->new('Sapi.SpVoice') ) {
                ...
                if ( defined $VTxt_stream1 ) {
                    ...
                    {
                        my $desc =
                          $VTxt_stream1->GetVoices->Item($VoiceCnt)
                          ->GetDescription;
...
sub speak_text {
    ...
    if ( $speak_engine =~ /NaturalVoiceWine/i ) {
        ...
        my $nv_path =
          $main::config_parms{voice_text_naturalvoice}; # DOS path to NatVox stuff
        $speak_pgm =
    ...
    if ( $speak_engine =~ /^&/ ) {
        ...
        my $fork = 1
          unless $parms{to_file}
          and
          !$parms{async}; # Must wait for to_file requests, so http requests work
