...
Description:
	...
	- scene_switch            (invented to map to something in HA that has on/off but no state)
	                          (MH scenes can have off states -- use scene_switch if that is the case)
...
#     }
    ...
    if( $self->{mqtt_type} eq 'scene_switch' ) {
...
sub add_listen_topic {
    my ($self, $topic) = @_;

    push @{$self->{topic}}, $topic;
}

sub set_discovery_names {
    my ($self, $full_name, $mqtt_name) = @_;
    my $area_name;
    my $device_name;
    my $friendly_name;
    my $device_id;
    ...
    my ($part1, $part2, $part3) = split(/:/,$full_name);
    if( defined( $part3 ) ) {
	$friendly_name = $part3;
	$area_name = $part1;
	$device_name = $part2;
...
=item C<make_topic( $topicpattern, @parms )>

    Make a topic based on a topic pattern and '+' wildcard replacement values.
    Extra parameters are appended to the end.

    Ex:

        make_topic( '/mh/shop/+/+', 'command', 'power')	# returns '/mh/shop/command/power'
        make_topic( '/aa/+/bb/+/cc', '1','2','3','4/5')	# returns 'aa/1/bb/2/cc/3/4/5'


=cut

sub make_topic {
    my ( $self, $topicpattern, @parms ) = @_;
    my $Caller = (caller())[3];

    my (@topic_parts) = split( "/", $topicpattern );
    my $wildcard_count = 0;
    for( my $i=0; $i <= $#topic_parts; $i += 1 ) {
        last if ($wildcard_count > $#parms);	# out of parms - must be done.
    ...
    # Add any extra parms to the end. This allows for flexibility in .mht files, e.g.:
    # a topic prefix of '/ha/mh/+/+' is the same as '/ha/mh'. Also allows sub-topics
    # in the second format to be specified as 'command/power' instead of 'command:power',
    # which is more intuitive for some.
    push @topic_parts, @parms[$wildcard_count..$#parms]
        if ($wildcard_count <= $#parms);

    my $NewTopic = join( '/', @topic_parts );

    # Help diagnose mis-constructed topics. Otherwise, the server just hangs up on us
    # and we don't know why.
    if ($NewTopic =~ /\+/) {
        &main::print_log(		# Don't want to use print_log, but no $self available.
               "[MQTT] Error: make_topic called by $Caller  with insufficient parms. "
             . "Pattern=$topicpattern, parms="
             . join(', ',@parms)
             . "\n"
        );
        $NewTopic =~ s/\+/_UNKN_/g;	# This is the wrong topic now, but the MQTT server won't hang up at least.
    }
...
sub process_template {
    ...
    # This is an attempt to convert simple Jinja templates to perl code

    # remove leading {{ and trailing }}
    $template =~ s/^\s*\{\s*\{\s*//;
    $template =~ s/\s*\}\s*\}\s*$//;

    # convert value_json.<element> to $value_json->{<element>}
    for( my $i=1; $i<4; ++$i ) {
	$template =~ s/value_json\.([a-zA-Z\-_]+)/\$value_json->\{\1\}/;
    }

    # convert value_json['<element1>']['<element2>] to $value_json->{<element1>}->{<element2>}
    for( my $i=1; $i<10; ++$i ) {
	$template =~ s/\[\s*\'?([a-zA-Z\-_]+)\'?\s*\]/->\{\1\}/;
    }

    if( $template =~ m/^\$value_json->\{[^\}]+\}$/ ) {
	...
	my ($true_val, $expr, $false_val) = $template =~ m/^[\{\s]*([^\s]+)\s+if\s+(.*)else\s+([^\s]+)[\s\}]*$/;
	if( $true_val ) {
	    $template = "\$value = $expr ? $true_val : $false_val";
	} else {
	    $self->debug( 1, "unable to process template $template" );
	    return;
	}
    }
    $self->debug( 2, "fishing template value out of json with '$template'" );
    eval "$template";
    if( $@  || !$value_json ) {
...
sub encode_mqtt_payload {
    ...
    $setval =~ s/%$//;	# Strip % if present; make things simpler all around.
    $brightness_scale = $self->{disc_info}->{brightness_scale} || 255;
    my $level;
    if( $self->{mqtt_type} eq 'light' ) {
	($level) = $setval =~ /^(\d|\d\d|100)$/;	# 0-100. Old regex allowed 0-199%.
    }
	...
	$self->error( "$self->{mqtt_name}: Unknown set value '$setval' for mqtt type '$self->{mqtt_type}'" );
	return;
    }
	...
	if( $self->{disc_info}->{schema} eq 'json' ) {
	    ...
	    if( $topic eq $self->{disc_info}->{brightness_command_topic} ) {
		$payload = $brightness;
...
sub create_discovery_message {
    ...
    &mqtt_BaseItem::normalize_discovery_info( $self->{disc} );

    # Use original topic and discovery message for mqtt_DiscoveredItem
    if( !$self->isa('mqtt_DiscoveredItem') ) {
	my $disc_topic;
	my $disc_msg;
	...
	$disc_msg = $json_obj->encode( $self->{disc} );
...
sub publish_discovery_message {
	...
	$self->debug( 2, "Skipped: ".  $self->{mqtt_name} . ' -- interface has no discovery prefix.' );
	return 0;
    }
	...
	$self->debug( 2, "Skipped: ".  $self->{mqtt_name} . ' -- discovery flag not set.' );
	return 0;
    }
...
sub new {     ### mqtt_LocalItem
    ...
    $unit_of_m = $local_object->{mqttlocalitem}->{unit_of_measurement} if ((defined $local_object->{mqttlocalitem}->{unit_of_measurement} ) and (!$unit_of_m));

    $interface->debug( 1,
        "New mqtt_LocalItem( $interface->{instance}, '$mqtt_name', '$type', '$local_object', "
    ...
    $topicpattern =~ s"/[+#]?$"";	# Remove trailing slash and wildcard, if present.
    my (@topic_parts) = split( "/", $topicpattern, 2 );
    if( $#topic_parts == 1 ) {
	$node_id = $topic_parts[0];
	$mqtt_name = $topic_parts[1];
	$topic_prefix = "$node_id/$mqtt_name";
    } else {
	$mqtt_name = $topic_parts[0];
    ...
    if( $interface->{topic_prefix} ) {
	$topic_prefix = "$interface->{topic_prefix}/$topic_prefix";
    }
    ...
    $self->set_discovery_names( $friendly_name, $mqtt_name );
	...
	delete $self->{disc_info}->{command_topic};
...
sub set {    ### LocalItem
	...
	# when it starts up.
	# For scenes and buttons, do not set the retain flag.

	my $retain = 1;
	if( $self->{mqtt_type} eq 'scene_switch'
	||  $self->{disc_type} eq 'scene'
	||  $self->{disc_type} eq 'button'
	) {
	    $retain = 0;
	}
	$self->debug( 1, "MH to MQTT LocalItem ${obj_name} set($setval) publishing state message '$payload' to mqtt" );
	$self->{has_published_state} = 1;
	$self->transmit_mqtt_message( $topic, $payload, $retain );
    } else {
	$self->debug( 1, "MH to MQTT LocalItem ${obj_name} -- set($setval) -- no state topic or payload, not sending mqtt message" );
    }
...
sub new {   ### mqtt_BaseRemoteItem
	...
	$self->set_states( "off", "on", "offline" );
    } elsif( $self->{mqtt_type} eq 'scene_switch' ) {
	$self->set_states( "off", "on", "offline" );
...
=item C<new(mqtt_interface, name, type, topicpattern, discoverable, friendly_name, statetopic, cmndtopic)>
    ...
    Note: statetopic and cmndtopic are all optional MQTT topics that have existing defaults.
...
sub new {      ### mqtt_RemoteItem
	...
	$cmndtopic       ||= 'cmnd:POWER';	# Default is Tasmota norms to maintain backward compatibility.
	$statetopic      ||= 'stat:POWER';	# Default is Tasmota norms to maintain backward compatibility.
	...
	$self->{disc_info}->{brightness_state_topic} = $self->make_topic( $listen_topic, 'tele', 'STATE' );
	$self->{disc_info}->{brightness_value_template} = '{{value_json.Dimmer}}';
	$self->{disc_info}->{brightness_scale} = 100;
	$self->{disc_info}->{brightness_command_topic} = $self->make_topic( $listen_topic, 'cmnd', 'Dimmer' );
...
sub new {      ### mqtt_InstMqttItem
    ...
    if( !grep( /$base_type/, ('light','switch','binary_sensor','sensor','scene', 'scene_switch', 'button', 'cover' ) ) ) {
	$interface->error( "Invalid InstMqttItem type '$type'" );
	return;
    }

    my $work = $topicpattern;
    my $mqtt_name;
    my $node_id;
    my $topic_prefix;
    my $topic_prefix_start;
    ($work, $mqtt_name) = $work =~ m|^(.*)/([^/\+]+)/?\+?$|;
    ($topic_prefix_start, $node_id) = $work =~ m|^(.*/)([^/]+)$|;
    if( !$node_id ) {
	$node_id = $work;
	$topic_prefix_start = '';
    }
    $topic_prefix_start .= "$node_id";
    $topic_prefix = "$topic_prefix_start/$mqtt_name";
	...
	$self->{disc_info}->{command_topic} = "$topic_prefix_start/modem/scene";
	$self->add_listen_topic( $self->{disc_info}->{command_topic} );
	...
	$self->{disc_info}->{command_topic} = "$topic_prefix/press";
	$self->{disc_info}->{optimistic} = 'true';
    } else {
