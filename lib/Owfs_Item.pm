...
sub process_read_response {
    ...
    if ( defined $response ) {
        ...
        &main::print_log( "Owfs_Item::process_read_response device: $device location: $location token: $token response: $response" )
          if $debug;
...
sub process_dir_response {
    ...
    &main::print_log( "Owfs_Item::process_dir_response family: $family id: $id level: $level path: $path tokens: @tokens" )
      if $main::Debug{owfs};

    while ( scalar( @{ $self->{dir_tokens} } ) ) {
        my $token = shift @{ $self->{dir_tokens} };
        &main::print_log( "Owfs_Item::process_dir_response family: $family id: $id level: $level path: $path token: $token" )
          if $main::Debug{owfs};
...
sub _lost {
    ...
    my $id       = $self->{id};
    my $path     = $self->{path};
    &main::print_log( "Owfs_Item::DEVICE_LOST: device: $device location: $location family: $family id: $id path: $path" )
      ;    # if $main::Debug{owfs};
    $self->{root}    = undef;
    $self->{path}    = undef;
    $self->{present} = 0;
...
sub _ToServer {
    ...
    my $message =
      pack( $f, $self->{VER}, $payload_length, $msg_type, $self->{SG} | $self->{PERSIST}, $size, $offset, $payload_data );
    &main::print_log(
    ...
    while ( active $socket) {
        ...
        if ( $socket_state == 0 ) {
            do {
                ...
                if ( !defined $r ) {
                    &main::print_log("Owfs_Item::_FromServer Trouble getting header")
                      if $main::Debug{owfs};
                    return;
                }
                ...
                if ( $return_status > 66000 ) {
                    &main::print_log("Owfs_Item::_FromServer Trouble getting payload")
                      if $main::Debug{owfs};
    ...
    if ( $msg_type eq $msg_read ) {
        if ( my $record = said $socket) {
            ...
            if ( !@response ) {
                ...
                if ( $response[2] > 66000 ) {
                    &main::print_log( "Owfs_Item::_run_loop read msg_type: $msg_type path: $path ERROR response: $response[2]" )
                      ;    # if $main::Debug{owfs};
                    $self->{failcnt}++;
                    if ( $self->{failcnt} >= 5 ) {
                        $self->_lost();
                    ...
                    # process response
                    &main::print_log( "Owfs_Item::_run_loop msg_type: $msg_type path: $path response: $response[6]" )
                      if $main::Debug{owfs};
                    $self->{failcnt} = 0;
    ...
    # Write Response
        ...
        if ( my $record = said $socket) {
            ...
            if ( !@response ) {
                ...
                if ( $response[2] > 66000 ) {
                    &main::print_log( "Owfs_Item::_run_loop write msg_type: $msg_type path: $path ERROR response: $response[2]" )
                      ;    # if $main::Debug{owfs};
                    $self->{failcnt}++;
                    if ( $self->{failcnt} >= 5 ) {
                        $self->_lost();
                    ...
                    # process response
                    &main::print_log( "Owfs_Item::_run_loop msg_type: $msg_type path: $path response: $response[2]" )
                      if $main::Debug{owfs};
                    $self->{failcnt} = 0;
    ...
    # Dirall Response
        ...
        if ( my $record = said $socket) {
            ...
            if ( !@response ) {
                ...
                if ( $response[2] > 66000 ) {
                    &main::print_log( "Owfs_Item::_run_loop dirall msg_type: $msg_type path: $path ERROR response: $response[2]" )
                      ;    # if $main::Debug{owfs};
                    $self->{failcnt}++;
                    if ( $self->{failcnt} >= 5 ) {
                        $self->_lost();
    ...
    # Dir response
        ...
        if ( my $record = said $socket) {
            ...
            if ( !@response ) {
                ...
                if ( $response[2] > 66000 ) {
                    &main::print_log( "Owfs_Item::_run_loop msg_dir msg_type: $msg_type path: $path ERROR response: $response[2]" )
                      ;    # if $main::Debug{owfs};
                    $self->{failcnt}++;
                    if ( $self->{failcnt} >= 5 ) {
                        $self->_lost();
    ...
    # Handle Unknown response
        ...
        if ( my $record = said $socket) {
            ...
            if ( !@response ) {
                ...
                if ( $response[2] > 66000 ) {
                    &main::print_log( "Owfs_Item::_run_loop unknown msg_type: $msg_type path: $path ERROR response: $response[2]" )
                      ;    # if $main::Debug{owfs};
                }
                else {
...
sub set {
    ...
    if ( $mode == $PIO ) {
        ...
        if ( ( $state eq $ON ) || ( $state eq $OFF ) ) {
            &main::print_log( "Owfs_Switch::set mode: $mode device: $device location: $location channel: $channel state: $state" )
              if $debug;
...
sub process_read_response {
    ...
    if ( defined $temperature ) {
        ...
        if ( $temperature =~ /^[-]?\d+(?:[.]\d+)?$/ ) {
            ...
            if ( $main::Debug{owfs} ) {
                &main::print_log( "Owfs_DS18S20::process_read_response $device $location temperature: $temperature" )
                  if $main::Debug{owfs};
            }
        }
    }
...
sub process_write_response {
    ...
    my $device   = $self->{device};
    my $location = $self->{location};
    &main::print_log( "Owfs_DS18S20::process_write_response $device $location response: $response token: $token value: $value" )
      if $main::Debug{owfs};
}
...
sub process_write_response {
    ...
    my $device   = $self->{device};
    my $location = $self->{location};
    &main::print_log( "Owfs_DS2450::process_write_response $device $location response: $response token: $token value: $value" )
      if $main::Debug{owfs};
}
...
sub run_loop {
    ...
    if ( $index == 0 ) {
        ...
        my $active   = $self->{active};
        my $present  = $self->{present};
        &main::print_log( "Owfs_DS2450::run_loop $index $device $location channel: $channel present: $present active: $active" )
          if $main::Debug{owfs};
        if ( ( $self->{active} == 0 ) && $self->{present} ) {
