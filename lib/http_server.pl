        ...
        or $get_req =~ /\/SET(_VAR)?[\:\;](\S*)$/i )
    {
        ...
        unless ($Authorized) {
            for my $temp ( split( '&&', $get_arg ) ) {
                ...
                my $set_authority =
                  eval qq[$item->get_authority if $item and ref($item) and UNIVERSAL::isa($item, 'Generic_Item');];
...
sub html_command_table {
    ...
    {
        my $msagent_file =
          file_read "$config_parms{'html_dir' . $Http{format}}/$config_parms{'html_msagent_script_vr' . $Http{format}}";
    ...
    return $html
      . &table_it(
        ...
        $main::config_parms{ 'html_category_cellsp' . $Http{format} }, @htmls
      );
}
