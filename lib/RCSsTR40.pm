...
sub new {
   ...
   push(@{$$self{states}}, 'temp_change', 'outside_temp_change', 
         'heat_sp_change', 'cool_sp_change', 'off', 'heat', 'cool', 'auto', 
         'emerg_heat', 'invalid', 'fan_on', 'fan_auto', 'hold', 'run', 
         'vacation', 'no_vacation');
   $self->_get_times();
   $self->_poll();
   return $self;
}
...
sub _check_auto_outside_temp {
   ...
   if (ref $$self{'auto_outside_temp_ref'}) {
      if (${$$self{'auto_outside_temp_ref'}} != $$self{'last_auto_outside_temp'}) {
         $$self{'last_auto_outside_temp'} = ${$$self{'auto_outside_temp_ref'}};
   ...
   while ($data =~ s/^\s*(\S+)=(\S+)\s*//) {
      ...
      if ($name eq 'A') {
            ...
            if (($$self{'mode'} eq 'heat') or ($$self{'mode'} eq 'auto')) {
               ...
               unless ($$self{'heat_start'}) {
                  ...
                  if (($$self{'heat_end'} + $$self{mot}) < $::Time) {
                     ...
                     $$self{'heat_start'} = ($$self{'heat_end'} + $$self{mot});
                  }
                  $$self{'heat_end'} = 0;
               }
            }
         ...
         if ($$self{'temp'} > $$self{'cool_sp'}) {
            if (($$self{'mode'} eq 'cool') or ($$self{'mode'} eq 'auto')) {
               if ($$self{'cool_end'} > $::Time) {
                  # It was scheduled to turn off, so cancel that
                  $$self{'cool_end'} = 0;
               }
               unless ($$self{'cool_start'}) {
                  ...
                  if (($$self{'cool_end'} + $$self{mot}) < $::Time) {
                     ...
                     # It will start in the future...
                     $$self{'cool_start'} = ($$self{'cool_end'} + $$self{mot});
                  }
                  $$self{'cool_end'} = 0;
               }
            }
         ...
         if (($$self{'temp'} >= $$self{'heat_sp'}) and (($$self{'temp'} <= $$self{'cool_sp'})) or ($$self{'mode'} eq 'off')) {
            if ($$self{'heat_start'}) {
               ...
               if ($$self{'cool_start'} > $::Time) {
                  # Was going to start, so cancel that...
                  $$self{'cool_end'} = ($$self{'cool_start'} - $$self{mot});
                  $$self{'cool_start'} = 0;
               } elsif (not $$self{'cool_end'}) {
                  # Is running, schedule stop time
                  if (($$self{'cool_start'} + $$self{mrt}) > $::Time) {
                     # Schedule future stop
                     $$self{'cool_end'} = ($$self{'cool_start'} + $$self{mrt});
                  } else {
                     # Stop now...
                     $$self{'cool_end'} = $::Time;
                  }
         ...
         $val = ($val == 0 ? 'hold' : 'run');
         if ($$self{'schedule_mode'} and ($$self{'schedule_mode'} ne $val)) {
