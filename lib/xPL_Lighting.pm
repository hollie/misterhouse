...
sub ignore_message {
	my ($self, $p_data) = @_;
        my $ignore_message = 0;
        if (!((defined($$p_data{'lighting.devinfo'}) and $$p_data{'lighting.devinfo'}{'device'} eq $self->id)
        	 or (defined($$p_data{'lighting.device'}) and $$p_data{'lighting.device'}{'device'} eq $self->id)))        {
            $ignore_message = 1;
        }
        return $ignore_message;
}
...
sub default_setstate
{
    ...
    if ($set_by =~ /^xpl/i) {
        ...
        &::print_log("[xPL_Light] Request light: " . $self->get_object_name
                	. " turn " . (($level eq '0') ? 'off' : "on at a level of $level")
                        ) if $main::Debug{xpl_light};
        my $cmd_block;
