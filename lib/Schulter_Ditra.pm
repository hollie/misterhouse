...
#TODO
#
# x print serial number that doesn't have a child object
# - voice commands? generate new token, enable/disable module?
# - print initial config when received
#setpoint item? temperature item and Heating item
...
@Schulter_Ditra::ISA = ('Generic_Item');
...
sub new {
    ...
    if ($self->{config}->{username} eq "" or $self->{config}->{password} eq "") {
        main::print_log( "[Schulter_Ditra] ERROR, username or password undefined" );
        $self->{enabled} = 0;
    }            
...
#how to store and restore a token
    ...
    if ($self->{enabled}) {
        ...
        main::print_log( "[Schulter_Ditra] module disabled, will not fetch data" );    
    }
}
...
sub start_timer {
    ...
    if ( defined $self->{timer} ) {
        $self->{timer}->set( $self->{config}->{poll_seconds}, sub { &Schulter_Ditra::get_data($self) }, -1 );
    }
    else {
        main::print_log( "[Schulter_Ditra] Warning, start_timer called but timer undefined" );
    }
}
...
sub get_data {
    ...
    main::print_log( "[Schulter_Ditra] get_data initiated" ) if ( $self->{debug} );
...
sub get_token {
    my ($self) = @_;
    main::print_log( "[Schulter_Ditra] Sending Authentication request for token..." );
...
sub poll {
    ...
    main::print_log( "[Schulter_Ditra] Background Polling initiated" ) if ( $self->{debug} );
...
sub process_check {
    ...
    if ( $self->{poll_process}->done_now() ) {
        ...
        main::print_log( "[Schulter_Ditra] Background poll process completed" ) if ( $self->{debug} );
        ...
        if ( $file_data =~ m/401 unauthorized/i ) {
            main::print_log( "[Schulter_Ditra] Authentication request denied, requesting new token" );
            $self->{auth_try}++;
            $self->get_token();
            return;
        }
        ...
        unless ( ($file_data) and ($json_data) ) {
            main::print_log( "[Schulter_Ditra] ERROR! bad data returned by poll" );
            main::print_log( "[Schulter_Ditra] ERROR! file data is [$file_data]. json data is [$json_data]" );
            return;
        }
        my $data;
        ...
        # catch crashes:
        if ($@) {
            main::print_log( "[Schulter_Ditra] ERROR! JSON file parser crashed! $@\n" );
            $com_status = "offline";
        }
        else {
            if ( keys %{$data} ) {
                ...
                main::print_log( "[Schulter_Ditra] ERROR! Returned data not structured! Not processing..." );
                $com_status = "offline";
            }
        }
...
#TODO if auth responds back yes, reset auth tries.
    ...
    if ( $self->{cmd_process}->done_now() ) {
        main::print_log( "[Schulter_Ditra] Background Command " . $self->{cmd_process_mode} . " process completed" ) if ( $self->{debug} );
                    ...
                    if ( keys %{$data} ) {
                        ...
                        #ErrorCode = 1, bad username, ErrorCode = 2 bad password
                            if ( defined $data->{SessionId} ) {
                                main::print_log( "[Schulter_Ditra] authentication token returned" );
                                $self->{token} = $data->{SessionId};
                            } else {
                                my $err = "Bad Username";
                                $err = "Bad Password" if ($data->{ErrorCode} == 2);
                                main::print_log( "[Schulter_Ditra] Error, could not request token. " . $err . "ErrorCode = " . $data->{ErrorCode} . " Email = " . $data->{Email} );                            
                            } 
                        }
                        if ( $self->{cmd_process_mode} eq "set" ) {
                            main::print_log( "[Schulter_Ditra] set returned" );
                            #RESPONSECODE:200
...
sub _get_JSON_data {
    my ( $self) = @_;
    if ($self->{token} eq "") {
        main::print_log( "[Schulter_Ditra] Token blank, requesting first one" ) if ( $self->{debug} );
        $self->get_token();
    } else {
        ...
        if ( $self->{poll_process}->done() ) {
            ...
            main::print_log( "[Schulter_Ditra] Backgrounding " . $self->{poll_process}->pid() . " command $cmd" ) if ( $self->{debug} );
        }
        else {
            if ( scalar @{ $self->{poll_queue} } < $self->{max_poll_queue} ) {
                main::print_log( "[Schulter_Ditra] Queue is " . scalar @{ $self->{poll_queue} } . ". Queing command $cmd" ) if ( $self->{debug} );
...
sub _push_JSON_data {
    ...
    if ( $self->{cmd_process}->done() ) {
        ...
        main::print_log( "[Schulter_Ditra] Backgrounding " . $self->{cmd_process}->pid() . " command $mode, $cmd" ) if ( $self->{debug} );
    }
    else {
        if ( scalar @{ $self->{cmd_queue} } < $self->{max_cmd_queue} ) {
            main::print_log( "[Schulter_Ditra] Queue is " . scalar @{ $self->{cmd_queue} } . ". Queing command $mode, $cmd" )
              if ( $self->{debug} );
            push @{ $self->{cmd_queue} }, "$cmd";
        }
        else {
            main::print_log( "[Schulter_Ditra] WARNING. Queue has grown past " . $self->{max_cmd_queue} . ". Command discarded." );
            if ( defined $self->{child_object}->{comm} ) {
...
sub stop_timer {
    ...
    if ( defined $self->{timer} ) {
        ...
        main::print_log( "[Schulter_Ditra] Warning, stop_timer called but timer undefined" );
    }
}
    ...
    foreach my $group (@{ $self->{data}->{Groups} }) {
        ...
        foreach my $stat (@{ $group->{Thermostats} }) {
            if ($serial eq "" or $stat->{SerialNumber} eq $serial) {
                ...
                main::print_log( "[Schulter_Ditra] MinTemp:              " . $stat->{MinTemp});
                main::print_log( "[Schulter_Ditra] MaxTemp:              " . $stat->{MaxTemp});
...
sub process_data {
    ...
    main::print_log( "[Schulter_Ditra] Processing Data..." ) if ( $self->{debug} );

    foreach my $group (@{ $self->{data}->{Groups} }) {
        main::print_log( "[Schulter_Ditra] Processing Data for Group " . $group->{GroupName} . "..." ) if ( $self->{debug} );
        ...
        foreach my $stat (@{ $group->{Thermostats} }) {
            if ($self->{child_object}->{$stat->{SerialNumber}}) {
                ...
                main::print_log( "[Schulter_Ditra] Setting stat " . $stat->{Room} . " with serial #" . $stat->{SerialNumber} . " to mode " . $heating ) if ( $self->{debug} );
                
            } else {
                main::print_log( "[Schulter_Ditra] Warning: No child object defined for Stat serial #" . $stat->{SerialNumber} );
            }
...
sub set_stat {
    ...
    main::print_log( "[Schulter_Ditra] DB set_mode, in master set, serial=$serial, p_setby=$serial" ) if ( $self->{debug} );
...
sub register {
    ...
    if ( lc $mode eq 'comm' ) {
        &main::print_log("[Schulter_Ditra]: Registering Communication object to controller");
        ...
        main::print_log( "[Schulter_Ditra] Warning: child object with mode [$mode] not recognized" );
    }
}
