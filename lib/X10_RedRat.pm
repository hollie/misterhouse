...
sub X10RR_send {
    ...
    $buffer[0] =
      ( 0x60, 0x70, 0x40, 0x50, 0x80, 0x90, 0xA0, 0xB0, 0xE0, 0xF0, 0xC0, 0xD0, 0x00, 0x10, 0x20, 0x30 )[ ord($house) - ord('A') ];
    if ( $unit > 8 ) {
        $buffer[0] |= 4;
        $unit -= 8;
    }
    ...
    if ( $func eq "N" ) {    # send ON
        if ( not $X10RR{ "$house$fullunit" . "ON" } ) {
            $X10RR{ "$house$fullunit" . "ON" } =
              gen_X10RR( sprintf "%08b%08b%08b%08b", $buffer[0], $buffer[0] ^ 0xff, $buffer[1], $buffer[1] ^ 0xff );
