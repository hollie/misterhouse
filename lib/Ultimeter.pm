...
####Start Data Log Record
                ...
                #3 Outdoor Temp
                $v=substr($data,12,4); $vv=hex($v); $vvv=$vv/10.0;
                $vvv = &adjust_temp( $vvv );
                $main::Weather{CurrentOutdoorTemp}=$vvv;

                #4 Long Term Rain Total (0.01")
                $v=substr($data,16,4); $vv=hex($v); $vvv=$vv/100.0;
        ...
        #       $main::Weather{DATE}=$vv;
                ...
                $v=substr($data,40,4); $vv=hex($v);
        #       $main::Weather{TIME}=$vv;

                #11 Rain Today
                $v=substr($data,44,4); $vv=hex($v); $vvv=$vv/100.0;
                $main::Weather{CurrentRain}=$vvv;

                #12 1 Minute Wind Speed Average (0.1kph)
                $v=substr($data,48,4); $vv=hex($v); $vvv=$vv/10*0.6;
                $main::Weather{OneMinWindSpeedAverage}=sprintf("%.1f",$vvv);
...
####Start Packet Record
                ...
                $v=substr($data,5,4); $vv=hex($v); $vvv=$vv/10*0.6;
                $main::Weather{FiveMinPeakWindSpeed}=sprintf("%.1f",$vvv);

                #2 Long Term Wind Speed Date
                $v=substr($data,9,4); $vv=hex($v);
                $main::Weather{LongtermWindSpeedPeakDate}=$vv;

                #3 Outdoor Temp
                $v=substr($data,13,4); $vv=hex($v); $vvv=$vv/10.0;
                $vvv = &adjust_temp( $vvv );
                $main::Weather{CurrentOutdoorTemp}=$vvv;

                #4 Long Term Rain Total (0.01")
                $v=substr($data,17,4); $vv=hex($v); $vvv=$vv/100.0;
                $main::Weather{LongtermRainTotal}=$vvv;

                #5 Current Barometer (millibars)
                $v=substr($data,21,4); $vv=hex($v);$vvv=$vv/10.0;
                $main::Weather{CurrentBarometer}=sprintf("%.2f", $vvv * .02953) ;

                #6 Barometer Delta (millibars)
                $v=substr($data,25,4); $vv=hex($v);
                if ($vv>32767) {$vv=$vv-hex("10000");}
                $vvv=$vv/10.0;
                $main::Weather{ThreeHourBarometerChange}=$vvv;

                #7 Barometric correction factor lsw
                $v=substr($data,29,4); $vv=hex($v); $vvv=$vv/10.0;

                #8 Barometric correction factor msw
                $v=substr($data,33,4); $vv=hex($v); $vvv=$vv/10.0;
                ...
                $v=substr($data,41,4); $vv=hex($v);
        #       $main::Weather{DATE}=$vv;

                #11 Time (minute of day)
                $v=substr($data,45,4); $vv=hex($v);
        #       $main::Weather{TIME}=$vv;

                #12 Rain Today
                $v=substr($data,49,4); $vv=hex($v); $vvv=$vv/100.0;
                $main::Weather{CurrentRain}=$vvv;

                #13 1 Minute Wind Speed Average (0.1kph)
                $v=substr($data,53,4); $vv=hex($v); $vvv=$vv/10*0.6;
                $main::Weather{OneMinWindSpeedAverage}=sprintf("%.1f",$vvv);
