...
sub _CheckCmd {
    ...
    if ( _IsChecksumOK($CmdStr) ) {
        ...
        if ( $cmd == 500 ) {    # System acknowledgement
            my $CmdName = "Unknown";
        ...
        elsif ( $cmd == 563 ) {    # Thermostats Set Point (untested; no escort/thermostat)
            my $TstatNum = substr( $data, 0, 1 );
            my $CoolSP   = substr( $data, 1, 3 );
            my $HeatSP   = substr( $data, 4, 3 );
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} Thermostat:$TstatNum; Cool Setpoint:$CoolSP; Heat Setpoint:$HeatSP" )
              if $main::config_parms{DSC_temp_log};
        }
        elsif ( $cmd == 570 ) {    # Broadcast Labels
            my $lblNum = substr( $data, 0, 3 );
            my $lbl    = substr( $data, 3, 32 );
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} Label#:$lblNum; Label:$lbl" );
        }
        elsif ( $cmd == 580 ) {    # Baud Rate Set
        ...
        elsif ( $cmd == 620 ) {    # duress alarm
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} ($data)" );
        }
        elsif ( $cmd == 621 ) {    # Fire Key Alarm
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 622 ) {    # Fire Key Alarm Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 623 ) {    # Auxiliary Key Alarm
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 624 ) {    # Auxiliary Key Alarm Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 625 ) {    # Panic Key Alarm
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 626 ) {    # Panic Key Alarm Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 631 ) {    # Auxiliary Input Alarm
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 632 ) {    # Auxiliary Input Alarm Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif (( $cmd == 650 )
            or ( $cmd == 651 ) )
          ...
          )                        # Partition Disarmed
        {
        ...
        elsif ( $cmd == 653 ) {    # Force partition to Arm
            ...
            ( $cmd == 656 )        # Exit Delay in Progress
            or ( $cmd == 657 )
          )                        # Entry Delay in Progress
        {                          # the data portion only contains the partition number
            $$self{'partitions'}[$data]->_entryExitDelay($cmd);
        }
        elsif ( $cmd == 658 ) {    # Keypad Lockout
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        }
        elsif ( $cmd == 659 ) {    # Keypad blanking
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        }
        elsif ( $cmd == 660 ) {    # Command Output In Progress
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        }
        elsif ( $cmd == 670 ) {    # Invalid Access Code
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        }
        elsif ( $cmd == 671 ) {    # Function Not Available
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        }
        elsif ( $cmd == 672 ) {    # Failed To Arm
            my $PartName = my $PartNum = $data;
            $PartName = $main::config_parms{"DSC_part_${data}"} if exists $main::config_parms{"DSC_part_${data}"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} partition $PartName ($PartNum)" );
        ...
        #    }
        #    =cut
        elsif ( $cmd == 800 ) {    # Panel Battery Trouble
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "panel trouble";
        }
        elsif ( $cmd == 801 ) {    # Panel Battery Trouble Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "panel trouble restore";
        }
        elsif ( $cmd == 802 ) {    # Panel AC Trouble
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "panel AC trouble";
        }
        elsif ( $cmd == 803 ) {    # Panel AC Trouble Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "panel AC trouble restore";
        }
        elsif ( $cmd == 806 ) {    # System Bell Trouble
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "system bell trouble";
        }
        elsif ( $cmd == 807 ) {    # System Bell Trouble restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        ...
        elsif ( $cmd == 814 ) {    # FTC Trouble
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
            set $self "ftc trouble";
        }
        elsif ( $cmd == 816 ) {    # Buffer Near Full
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd}" );
        }
        elsif ( $cmd == 821 ) {    # General Device Low Battery
        ...
        elsif ( $cmd == 822 ) {    # General Device Low Battery REstore
            my $ZoneNum = my $ZoneName = $data;
            $ZoneName = $main::config_parms{"DSC_zone_$ZoneNum"} if exists $main::config_parms{"DSC_zone_$ZoneNum"};
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} $ZoneName($ZoneNum)" );
        }
        elsif ( $cmd == 825 ) {    # Wireless Key Low Battery
            my $KeyNum = $data;
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} ($KeyNum)" );
        }
        elsif ( $cmd == 826 ) {    # Wireless Key Low Battery Restore
            my $KeyNum = $data;
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} ($KeyNum)" );
        }
        elsif ( $cmd == 827 ) {    # HandheldKey Low Battery
            my $KeyNum = $data;
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} ($KeyNum)" );
        }
        elsif ( $cmd == 828 ) {    # Handheld Key Low Battery Restore
            my $KeyNum = $data;
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} ($KeyNum)" );
        }
        elsif ( $cmd == 829 ) {    # General System Tamper
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        elsif ( $cmd == 830 ) {    # General System Tamper Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        ...
        elsif ( $cmd == 842 ) {    # Fire Trouble
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        elsif ( $cmd == 843 ) {    # Fire Trouble  Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        elsif ( $cmd == 896 ) {    # Keybus Fault
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        elsif ( $cmd == 897 ) {    # Keybus Fault Restore
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
        }
        elsif ( $cmd == 900 ) {    # user code required
            &_LocalLogit( $self, "$cmd $EventMsg{$cmd} " );
            $self->cmd( "CodeSend", $self->{houseCode} . "00" );
        }
        elsif ( $cmd == 908 ) {    # softwre version
...
sub new {
    ...
    if ( 1 == scalar( keys %DSC_Systems ) ) {    # Add hooks on first call only
    ...
    if ( defined $::config_parms{DSC_time_log} ) {         # enable/disable time broadcasts if configured
    ...
    if ( defined $::config_parms{DSC_temp_log} ) {         # enable/disable temperature broadcasts if configured
        select( undef, undef, undef, 0.250 );              # wait 250 millseconds to avoid overrunning RS-232 receive buffer on panel
        _cmd( $self, 'TemperatureBroadcastControl', $::config_parms{DSC_temp_log} );
    }
    if ( $type =~ /5401/ ) {                               # enable/disable VerboseArmingControl
        select( undef, undef, undef, 0.250 );              # wait 250 millseconds to avoid overrunning RS-232 receive buffer on panel
