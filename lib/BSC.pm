...
sub send_cmd {
   ...
   if ($$self{pending_device_state}) {
	 ...
	 $bsc_block->{'ID'} = $key;
         ...
         $bsc_block->{'Level'} = $pending_state->{'Level'} if $pending_state->{'Level'};
         $bsc_block->{'Text'} = $pending_state->{'Text'} if $pending_state->{'Text'};
...
sub commit_pending_state {
   my ($self, $id) = @_;
   $self->{device_state}{$id}{State}    = $self->{pending_device_state}{$id}{State};
   $self->{device_state}{$id}{Level}    = $self->{pending_device_state}{$id}{Level};
...
sub _handle_x10_device {
   ...
   if (defined($mh_obj)) {
      ...
      if (($msg_type eq 'send-event') or ($msg_type eq 'set-info')) {
         my $mh_state = $mh_obj->state_now() if ($msg_type eq 'send-event');
         $mh_state = $mh_obj->state() if ($msg_type eq 'set-info'); # probably need to parse out stacked state vals if they exist
