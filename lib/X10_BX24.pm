...
sub Stanley {
   ...
   &main::print_log("X10_BX24.pm: Stanley Code receive ->  $StanleyCode") if $main::config_parms{debug} eq 'BX24';
   if ($StanleyCode !~ /^23/) {
...
sub SendX10 {
   ...
   my $HouseUnit;
   if (scalar(@_) != 1) {
      &main::print_log("X10_BX24.pm: call to CM11a Invalid parameter [@_]") if $main::config_parms{debug} eq 'BX24';
   ...
   if (uc($House) !~ /[A-P]/) {
      &main::print_log("X10_BX24.pm: invalid house code [$House]") if $main::config_parms{debug} eq 'BX24';
   ...
   if ($X10Type =~ /[1-9A-GOP]/) {
      ...
      &main::print_log("X10_BX24.pm: SendX10 Complete X10 command received [$Cmd]") if $main::config_parms{debug} eq 'BX24';
      $prev_X10 = '';
   } else {
      &main::print_log("X10_BX24.pm: Invalid CM11a command [@_]") if $main::config_parms{debug} eq 'BX24';
      &::logit("$::config_parms{data_dir}/logs/BX24.$::Year_Month_Now.log", "BX24 invalid CM11a command [@_]");
      $prev_X10 = '';
      return;
   }

   &main::print_log("X10_BX24.pm: Sending CM11a command [$Cmd]") if $main::config_parms{debug} eq 'BX24';
...
sub check_for_data {
   ...
   &main::print_log("X10_BX24.pm: Receive [$RcvStr]") if $main::config_parms{debug} eq 'BX24';
   foreach (@CharStr) {
   ...
   &main::print_log("X10_BX24.pm: GotCR=$GotCR LineOK=$LineOK [$data] EOF") if $main::config_parms{debug} eq 'BX24';
   if (!$LineOK) {
      $prev_data = $data;

      &main::print_log("X10_BX24.pm: Incomplete command line [$data]") if $main::config_parms{debug} eq 'BX24';
      return;
   }
   ...
   if ($data =~ /RX:/) {
      ...
      &main::print_log("X10_BX24.pm: RX command received") if $main::config_parms{debug} eq 'BX24';
      ...
      if (scalar(@RX) == 1) {               # Only unit command
         ...
         &main::print_log("X10_BX24.pm: Keeping RX Unit [$prev_rx]") if $main::config_parms{debug} eq 'BX24';
         ...
         &main::print_log("X10_BX24.pm: Receive RX action [ $prev_rx $RX[1] ]") if $main::config_parms{debug} eq 'BX24';
         $prev_rx = '';
      }
   }
   ...
   &main::print_log("BX24 Data: $data") if $main::config_parms{debug} eq 'BX24';
   ...
   if (my ($X10BX24) = $data =~ /.*\((.*)\)/) {
      ...
      &main::print_log("BX24 Code: $X10BX24   X10 Code:$X10Code") if $main::config_parms{debug} eq 'BX24';
      ...
      chomp $Barometer;
      &main::print_log("BX24 Code: Barometer data $Barometer") if $main::config_parms{debug} eq 'BX24';
      ...
      foreach (@SecurityItems) {
         ...
         if (lc($RF_ID) eq lc($$obj{rf_id})) {
            ...
            ::print_log "X10_BX24.pm: The security sensor $SensorName is now $CurrentStatus" if $main::config_parms{debug} eq 'BX24';
