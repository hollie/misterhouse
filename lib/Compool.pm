      ...
      )
    {
        ...
        if ( unpack( 'C', substr( $Compool_Data{$serial_port}{Last_Basic_Acknowledgement_Packet}, $PacketOffset, 1 ) ) == 0 ) {
            ...
            my $temp = int(
                (
                    (
                        unpack( 'C', substr( $Compool_Data{$serial_port}{Last_Basic_Acknowledgement_Packet}, $PacketOffset, 1 ) ) /
                          $divisor
                    ) * 1.8
                ) + 32
            );
...
sub set_device {
    ...
    ( $targetprimary == 8 )
      ? return queue_command( $serial_port,
        "\x00\x00" . pack( "C", $targetbit ) . "\x00\x00\x00\x00\x00" . pack( "C", $byteuseenable ),
        $targetdevice, $targetstate == 1 ? "on" : "off" )
      : return queue_command( $serial_port,
        "\x00\x00\x00" . pack( "C", $targetbit ) . "\x00\x00\x00\x00" . pack( "C", $byteuseenable ),
        $targetdevice, $targetstate == 1 ? "on" : "off" );
}
...
sub get_device {
    ...
    if (
        $nowstate
        and (
            int( unpack( 'C', substr( $Compool_Data{$serial_port}{Last_Basic_Acknowledgement_Packet}, $targetprimary, 1 ) ) ) &
            $targetbit ) == (
            int( unpack( 'C', substr( $Compool_Data{$serial_port}{Now_Basic_Acknowledgement_Packet}, $targetprimary, 1 ) ) ) &
              $targetbit
            )
      )
    {
...
sub get_delay {
        ...
        == ( int( unpack( 'C', substr( $Compool_Data{$serial_port}{Now_Basic_Acknowledgement_Packet}, 10, 1 ) ) ) & $targetbit ) )
    {
        ...
        ( unpack( 'C', substr( $Compool_Data{$serial_port}{Last_Basic_Acknowledgement_Packet}, 10, 1 ) ) & $targetbit )
          ? return "on"
          : return "off";
    }
}
...
sub get_solar_present {
    ...
    ( unpack( 'C', substr( $Compool_Data{$serial_port}{Last_Basic_Acknowledgement_Packet}, 10, 1 ) ) & 8 )
      ? return "yes"
      : return "no";
}
