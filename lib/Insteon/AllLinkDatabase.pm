...
sub restore_string {
    ...
    if ( $$self{aldb} ) {
        ...
        $restore_string .=
          ...
          . "->_aldb;\n";
        $restore_string .=
          $$self{device}->get_object_name . "->_aldb->restore_aldb(q~$aldb~) if " . $$self{device}->get_object_name . "->_aldb;\n";
    }
...
sub delete_link {
        ...
        && ( !$$self{device}->isa('Insteon_PLM') ) )
    {
        ...
        $$self{_aldb_unchanged_callback} =
          '&Insteon::AllLinkDatabase::delete_link(' . $$self{device}->{object_name} . "->_aldb, 'ok')";
        $$self{_aldb_changed_callback} =
          '&Insteon::AllLinkDatabase::delete_link(' . $$self{device}->{object_name} . "->_aldb, 'fail')";
        ...
        if ( $link_parms{callback} ) {
            ...
            &::print_log(
                "[Insteon::AllLinkDatabase] failure occurred in callback eval for " . $$self{device}->get_object_name . ":" . $@ )
              if $@ and $self->{device}->debuglevel( 1, 'insteon' );
...
sub delete_orphan_links {
    ...
    LINKKEY: for my $linkkey ( keys %{ $$self{aldb} } ) {
        ...
        my (
            $linked_device, $plm_scene,   $controller_object, $link_defined, $controller_id,
            $responder_id,  $recip_data3, $group_object,      $data3_object
        );
        ...
        if ( $linked_device->is_deaf ) {
            $delete_req{skip} =
              "$selfname -- Skipping check for reciprocal links on deaf device " . $linked_device->get_object_name;
            next LINKKEY;
        }
...
sub get_first_empty_address {
    ...
    if ( !($first_address) ) {
        ...
        main::print_log(
            "[Insteon::AllLinkDatabase] DEBUG4: No empty link entries; using next lowest link address [" . $first_address . "]" )
...
sub add_link {
        ...
        && ( !$$self{device}->isa('Insteon_PLM') ) )
    {
        ...
        $self->{callback_parms} = \%link_parms;
        $$self{_aldb_unchanged_callback} =
          '&Insteon::AllLinkDatabase::add_link(' . $$self{device}->{object_name} . "->_aldb, 'ok')";
        $$self{_aldb_changed_callback} =
          '&Insteon::AllLinkDatabase::add_link(' . $$self{device}->{object_name} . "->_aldb, 'fail')";
        $self->query_aldb_delta("check");
    }
    elsif ( $link_parms{aldb_check} eq "fail" ) {
...
sub update_link {
        ...
        && ( !$$self{device}->isa('Insteon_PLM') ) )
    {
        ...
        $self->{callback_parms} = \%link_parms;
        $$self{_aldb_unchanged_callback} =
          '&Insteon::AllLinkDatabase::update_link(' . $$self{device}->{object_name} . "->_aldb, 'ok')";
        $$self{_aldb_changed_callback} =
          '&Insteon::AllLinkDatabase::update_link(' . $$self{device}->{object_name} . "->_aldb, 'fail')";
        $self->query_aldb_delta("check");
    }
    elsif ( $link_parms{aldb_check} eq "fail" ) {
        ...
        or ( $$self{_mem_activity} eq 'add' ) )
    {
        if ( $$self{_mem_action} eq 'aldb_flag' ) {
            ...
            my $aldbkey = $self->get_linkkey(
                $$self{pending_aldb}{deviceid},      $$self{pending_aldb}{group},
                $$self{pending_aldb}{is_controller}, $$self{pending_aldb}{data3}
            );
...
sub _on_peek {
    ...
    if ( $msg{is_extended} ) {
        ...
        if ( $$self{_mem_action} eq 'aldb_peek' ) {
            ...
            ::print_log(
                "[Insteon::ALDB_i1] " . $$self{device}->get_object_name . ": unhandled _mem_action=" . $$self{_mem_action} )
              if $self->{device}->debuglevel( 1, 'insteon' );
        }
    }
}
...
sub update_local_properties {
    ...
    if ( defined($aldb_check) ) {
        ...
        $$self{_aldb_unchanged_callback} =
          '&Insteon::ALDB_i1::update_local_properties(' . $$self{device}->{object_name} . "->_aldb, 1)";
        $$self{_aldb_changed_callback} =
          '&Insteon::ALDB_i1::update_local_properties(' . $$self{device}->{object_name} . "->_aldb, 1)";
        $self->query_aldb_delta("check");
    }
...
sub update_flags {
    ...
    if ( defined($aldb_check) ) {
        ...
        $$self{_aldb_unchanged_callback} =
          '&Insteon::ALDB_i1::update_flags(' . $$self{device}->{object_name} . "->_aldb, '$flags', 1)";
        $$self{_aldb_changed_callback} =
          '&Insteon::ALDB_i1::update_flags(' . $$self{device}->{object_name} . "->_aldb, '$flags', 1)";
        $self->query_aldb_delta("check");
    }
...
sub update_intradevice_links {
    ...
    if ( defined($aldb_check) ) {
        ...
        $$self{_aldb_unchanged_callback} =
          '&Insteon::ALDB_i1::update_intradevice_links(' . $$self{device}->{object_name} . "->_aldb, '', 1)";
        $$self{_aldb_changed_callback} =
          '&Insteon::ALDB_i1::update_intradevice_links(' . $$self{device}->{object_name} . "->_aldb, '', 1)";
        $self->query_aldb_delta("check");
    }
...
sub on_read_write_aldb {
    ...
    if ( $$self{_mem_action} eq 'aldb_i2read' ) {
        ...
        main::print_log(
            "[Insteon::ALDB_i2] " . $$self{device}->get_object_name . ": unhandled _mem_action=" . $$self{_mem_action} )
          if $self->{device}->debuglevel( 1, 'insteon' );
        $clear_message = 0;
    }
...
sub _write_delete {
    ...
    if ($address) {
        ...
        &::print_log(
            "[Insteon::ALDB_i2] WARN: " . $$self{device}->get_object_name . " write_delete failure: no address available" );
        if ( $$self{_success_callback} ) {
    ...
    if ($num_in_queue) {
        ...
        if ( $delete_req{'root_object'} ) {
            ...
            $delete_req{'root_object'}
              ->delete_orphan_links( ( $delete_req{'audit_mode'} ) ? 1 : 0, $failure_callback, $is_batch_mode );
        }
        else {
