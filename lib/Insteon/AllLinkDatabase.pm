...
sub delete_orphan_links {
        ...
        || ( $$self{device}->is_deaf && $is_batch_mode ) )
    {
        ...
        if ( $$self{device}->is_deaf ) {
            ...
            ::print_log(
                "[Insteon::AllLinkDatabase] Delete orphan links: skipping $selfname because health: "
                  . $self->health
...
sub update_link {
        ...
        && ( !$$self{device}->isa('Insteon_PLM') ) )
    {
        ...
        $self->_write_link( $address, $deviceid, $group, $is_controller, $data1,
            $data2, $data3 );
    }
    else {
        &::print_log(
...
sub log_alllink_table {
    ...
    if ( $self->health eq 'good' ) {
        ...
        main::print_log( "[Insteon::AllLinkDatabase] ALDB is "
              . $self->health
...
sub _on_poke {
        ...
        or ( $$self{_mem_activity} eq 'add' ) )
    {
        ...
        if ( $$self{_mem_lsb} eq '51' ) {
            ...
            $message =
              new Insteon::InsteonMessage( 'insteon_send',
...
sub has_link {
    my ( $self, $insteon_object, $group, $is_controller, $data3 ) = @_;
    my $key =
      $self->get_linkkey( $insteon_object->device_id,
