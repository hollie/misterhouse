			...
			if (!($device))
                        {
                        	...
                        	if ($audit_mode)
                                {
					&::print_log("[Insteon::ALDB_i1] (AUDIT) " . $$self{device}->get_object_name . " now deleting orphaned link w/ details: "
				...
				if (!($device->has_link($$self{device},$group,1)))
						...
						&::print_log("[Insteon::ALDB_i1] (AUDIT) Now deleting orphaned responder link in "
                                        ...
                                        {
				       		my %delete_req = (deviceid => $deviceid, group => $group, is_controller => $is_controller,
							callback => "$selfname->_aldb->_process_delete_queue()", object => $device, data3 => $data3,
					...
					if ($plm_link)
						...
						if ($is_invalid) {
								...
								&::print_log("[Insteon::ALDB_i1] (AUDIT) Delete orphan responder link from "
                                                        ...
                                                        {
								my %delete_req = (deviceid => $deviceid, group => $group, is_controller => $is_controller,
									callback => "$selfname->_aldb->_process_delete_queue()", object => $device,
								...
								$num_deleted++;
                                        	...
                                        	if ($audit_mode)
                                                {
							&::print_log("[Insteon::ALDB_i1] (AUDIT) Delete orphan because no PLM link defined to: "
								. (($is_controller) ? "controller" : "responder")
								. "=$selfname" . "($group), data=$data3");
                                                }
                                                else
                                                {
                        ...
                        {
                        	if ($device->isa('Insteon::RemoteLinc') or $device->isa('Insteon::MotionSensor'))
                                {
        			...
        			elsif ($device->_aldb->health ne 'good')
       				{
        				&::print_log("[Insteon::ALDB_i1] Delete orphan links: skipping check for reciprocal links from "
                				...
                				. $device->_aldb->health . ". Please rescan this device!!")
                				if ($device->_aldb->health ne 'empty');
        			}
                                else
                                {
							...
							foreach my $member_ref (keys %{$$link{members}})
								...
								if ($member->isa('Insteon::BaseDevice') && !($member->is_root))
									...
									last;
                                                        		...
                                                        		&& ($member->device_id eq $device->device_id))
                                                		{
									$is_invalid = 0;
									last;
								}
...
sub delete_orphan_links
{
	...
	foreach my $linkkey (keys %{$$self{aldb}}) {
		...
		if (!($device)) {
                	if ($audit_mode)
                        {
                        &::print_log("[Insteon::ALDB_PLM] (AUDIT) Delete Orphan Link to non-existant device " .
                        	" device: " .
                                $deviceid . "; group: $group; is_controller: $is_controller, data: $data3")
                                if $main::Debug{insteon};
                        }
                        else
                        {
				...
				if (!($link)) {
					...
					if ($is_invalid) {
						...
						if ($device->has_link($self,$group,0, $data3)) {
                                                	if ($audit_mode)
                                                        {
                                                                &::print_log("[Insteon::ALDB_i1] (AUDIT) Delete orphan controller link from PLM to "
                                                                	. $device->get_object_name()
