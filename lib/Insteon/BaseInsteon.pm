...
sub default_hop_count {
    ...
    if ( defined($hop_count) ) {
        ::print_log(
            "[Insteon::BaseObject] DEBUG3: Adding hop count of " . $hop_count . " to hop_array of " . $self->get_object_name )
    ...
    if ( $self->_is_valid_state($p_state) ) {
        ...
        if ( ref $p_setby and $p_setby eq $self ) {    #If set by device, update MH state,
            ...
            if ( $self->is_responder ) {
                ...
                ::print_log(
                    "[Insteon::BaseObject] " . $self->get_object_name() . " is not a responder and cannot be set to a state." );
            }
        }
...
sub _process_message {
    ...
    if ( $msg{is_ack} ) {
        ...
        if ( $p_setby->active_message->failure_callback ) {
            main::print_log(
                "[Insteon::BaseObject] WARN: Now calling message failure callback: " . $p_setby->active_message->failure_callback )
              if $self->debuglevel( 1, 'insteon' );
...
sub new {
    ...
    $self->restore_data(
        'devcat',         'firmware',           'level',              'retry_count_log',
        'fail_count_log', 'outgoing_count_log', 'incoming_count_log', 'corrupt_count_log',
        'dupe_count_log', 'hops_left_count',    'max_hops_count',     'outgoing_hop_count',
        'awake_time'
    );
...
sub link_to_interface {
    ...
    if ( $step == 0 ) {    #If NAK on get_engine, then this is an I2CS device
        ...
        if ( ref $$self{surrogate} ) {
            ...
            ::print_log(
                '[Insteon::BaseInsteon] Link_To_Interface successfully completed' . ' for device ' . $self->get_object_name );
        }
    }
...
sub unlink_to_interface {
    ...
    my $success_callback        = "";
    my $failure_callback =
      "::print_log('[Insteon::BaseInsteon] ERROR: Unlink_To_Interface " . "failed for device: " . $self->get_object_name . "')";
    $step = 0 if ( $step eq '' );
    if ( $step == 0 ) {    #Delete link on the device
        ...
        if ( ref $$self{surrogate} ) {
            ...
            ::print_log(
                "[Insteon::BaseInsteon] Unlink_To_Interface" . " successfully completed for device " . $self->get_object_name );
        }
    }
...
sub set_operating_flag {
    ...
    if ( $self->is_root and !( $self->isa('Insteon::InterfaceController') ) ) {
        ...
        &::print_log(
            "[Insteon::BaseDevice] " . $self->get_object_name . " is either not a root device or is a plm controlled scene" );
        return;
    }
}
