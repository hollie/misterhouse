...
sub _process_message {
        ...
        && $self->_is_info_request( $pending_cmd, $ack_setby, %msg ) )
    {
            ...
            || substr( $msg{extra}, 0, 6 ) eq "000101" )
        {
            ...
            main::print_log(
                "[Insteon::MotionSensor] WARN: Unknown Extended " . "Set/Get Data Message Received for " . $self->get_object_name )
              if $self->debuglevel( 1, 'insteon' );
        }
    }
    else {
...
sub _process_message {
        ...
        && $self->_is_info_request( $pending_cmd, $ack_setby, %msg ) )
    {
        ...
        if ( substr( $msg{extra}, 0, 6 ) eq "000001" ) {
            ...
            main::print_log(
                "[Insteon::TriggerLinc] The awake seconds " . "for device " . $self->get_object_name . " is set to: " . $awake );
            $clear_message = 1;
            $self->_process_command_stack(%msg);
        }
        else {
            main::print_log(
                "[Insteon::TriggerLinc] WARN: Corrupt Extended " . "Set/Get Data Received for " . $self->get_object_name )
              if $self->debuglevel( 1, 'insteon' );
        }
    }
    else {
