...
sub set {
    ...
    if ( ref $p_setby && $p_setby->can('equals') && $p_setby->equals($self) ) {
        ...
        {
            ::print_log(
                "[Insteon::IOLinc] Received duplicate " . $self->get_object_name . " sensor " . $p_state . " message, ignoring." )
...
sub set_momentary_time {
    ...
    if ( $momentary_time <= 255 ) {
        ...
        ::print_log(
            "[Insteon::IOLinc] Setting Momentary Time to $momentary_time " . "tenths of a second for " . $self->get_object_name )
          if $self->debuglevel( 1, 'insteon' );
    }
    else {
