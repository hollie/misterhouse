        ...
        if ( $FSM == 0 ) {
            ...
            if ( $plm_cmd_id eq '0250' ) {
                ...
                $plm_message .=
                  ...
                  . substr( $plm_string, 12, 2 ) . "\n";
                $plm_message .=
                  sprintf( "%20s: ", 'Device Category' ) . substr( $plm_string, 14, 2 ) . ":" . substr( $plm_string, 16, 2 ) . "\n";
            ...
            if ( $plm_cmd_id eq '0260' ) {
                ...
                $plm_message .=
                  sprintf( "%20s: ", 'Device Category' ) . substr( $plm_string, 4, 2 ) . ":" . substr( $plm_string, 6, 2 ) . "\n";
                ...
                my $flags = hex( substr( $plm_string, 4, 2 ) );
                $plm_message .=
                  sprintf( "%20s: Automatic Linking ", 'Bit 7' ) . ( $flags & 0b10000000 ? 'Disabled' : 'Enabled' ) . "\n";
                $plm_message .= sprintf( "%20s: Monitor Mode ", 'Bit 6' ) . ( $flags & 0b01000000 ? 'Enabled' : 'Disabled' ) . "\n";
                $plm_message .=
                  sprintf( "%20s: Automatic LED ", 'Bit 5' ) . ( $flags & 0b00100000 ? 'Disabled' : 'Enabled' ) . "\n";
                $plm_message .=
                  sprintf( "%20s: Deadman Feature ", 'Bit 4' ) . ( $flags & 0b00010000 ? 'Disabled' : 'Enabled' ) . "\n";
                $plm_ack_pos = 6;
            }
                ...
                if ( length($plm_string) > 4 ) {
                    ...
                    $plm_message .=
                      sprintf( "%20s: Automatic Linking ", 'Bit 7' ) . ( $flags & 0b10000000 ? 'Disabled' : 'Enabled' ) . "\n";
                    $plm_message .=
                      sprintf( "%20s: Monitor Mode ", 'Bit 6' ) . ( $flags & 0b01000000 ? 'Enabled' : 'Disabled' ) . "\n";
                    $plm_message .=
                      sprintf( "%20s: Automatic LED ", 'Bit 5' ) . ( $flags & 0b00100000 ? 'Disabled' : 'Enabled' ) . "\n";
                    $plm_message .=
                      sprintf( "%20s: Deadman Feature ", 'Bit 4' ) . ( $flags & 0b00010000 ? 'Disabled' : 'Enabled' ) . "\n";
...
sub insteon_message_flags_decode {
    ...
    $flags_message .=
      sprintf( "%28s: (%01b) ", 'Message Length', $flag_ext ) . ( $flag_ext ? 'Extended Length' : 'Standard Length' ) . "\n";
