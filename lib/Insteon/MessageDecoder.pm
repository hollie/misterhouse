    ...
    while ( !$abort and !$finished ) {
        if ( $FSM == 0 ) {
            ...
            if ( $plm_cmd_id eq '0250' ) {
                ...
                $plm_message .=
                  sprintf( "%20s: ", 'All-Link Flags' )
                  . substr( $plm_string, 4, 2 ) . "\n";
                my $flags = hex( substr( $plm_string, 4, 2 ) );
                $plm_message .=
                  sprintf( "%20s: Record is ", 'Bit 7' )
                  . ( $flags & 0b10000000 ? 'in use' : 'available' ) . "\n";
                $plm_message .=
                  sprintf( "%20s: PLM is ", 'Bit 6' )
                ...
                $plm_message .=
                  sprintf( "%20s: ", 'All-Link Command1' )
                  . substr( $plm_string, 14, 2 ) . "\n";
                $plm_message .=
                  sprintf( "%20s: ", 'All-Link Command2' )
                  . substr( $plm_string, 16, 2 ) . "\n";
                $plm_message .=
                  sprintf( "%20s: ", 'All-Link Data' )
                  . substr( $plm_string, 18, 2 ) . "\n";
                ...
                $plm_message .=
                  sprintf( "%20s: (", 'Undefined Cmd Data' )
                  . substr( $plm_string, 4 ) . ")\n";
            }
            $finished++;
        }
            ...
            if ( $plm_cmd_id eq '0260' ) {
                ...
                $plm_message .=
                  sprintf( "%20s: Monitor Mode ", 'Bit 6' )
                  . ( $flags & 0b01000000 ? 'Enabled' : 'Disabled' ) . "\n";
                $plm_message .=
                  sprintf( "%20s: Automatic LED ", 'Bit 5' )
                  . ( $flags & 0b00100000 ? 'Disabled' : 'Enabled' ) . "\n";
                $plm_message .=
                  sprintf( "%20s: Deadman Feature ", 'Bit 4' )
...
sub insteon_message_flags_decode {
    ...
    $flags_message .=
      sprintf( "%28s: (%03b) ", 'Message Type', $flag_msg )
      . $message_string{$flag_msg} . "\n";
    $flags_message .=
      sprintf( "%28s: (%01b) ", 'Message Length', $flag_ext )
