...
sub sync_intradevice_links {
    ...
    ::print_log(
        '[Insteon::KeyPadLinc] ' . $self->get_object_name . 'will be ' . 'programmed with the following IntraDevice Links:' );
        ...
        if ( ref $subgroup_object ) {
            ...
            foreach my $member_ref ( keys %{ $$subgroup_object{members} } ) {
                ...
                if ( $member_root eq $self ) {
                    ...
                    ::print_log(
                        "[Insteon::KeyPadLinc] Group $member_group will " . "$link_type when Group $dec_group is pressed." );
                }
            }
        }
    }
...
sub _write_intradevice_links {
    ...
    my $failure_callback =
      "::print_log('[Insteon::KeyPadLinc] ERROR - Syncing" . "IntraDevice Links to " . $self->get_object_name . " failed.')";
    if ( $group == 8 && $sub == 3 ) {
        $success_callback =
          "::print_log('[Insteon::KeyPadLinc] Successfully wrote " . "IntraDevice links for " . $self->get_object_name . "')";
    }
