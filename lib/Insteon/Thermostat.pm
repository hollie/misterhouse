...
sub _is_info_request {
    ...
    if ($is_info_request) {
        my $val = $msg{extra};
        main::print_log( "[Insteon::Thermostat] Processing is_info_request for $cmd with value: $val" )
          if $self->debuglevel( 1, 'insteon' );
...
sub _process_message {
        ...
        && $self->_is_info_request( $pending_cmd, $ack_setby, %msg ) )
    {
        ...
        main::print_log( "[Insteon::Thermostat] Processing data for $msg{command} with value: $msg{extra}" )
          if $self->debuglevel( 1, 'insteon' );
...
sub _is_info_request {
    ...
    if ( $cmd eq 'thermostat_control' && $$self{_control_action} eq "mode" ) {
        my $val = $msg{extra};
        main::print_log( "[Insteon::Thermo_i1] Processing is_info_request for $cmd with value: $val" )
          if $self->debuglevel( 1, 'insteon' );
        if ( $val eq '00' ) {
            $self->_mode('off');
        }
...
sub _process_message {
        ...
        && $self->_is_info_request( $pending_cmd, $ack_setby, %msg ) )
    {
        ...
        if ( substr( $msg{extra}, 0, 4 ) eq "0201" ) {
            ...
            main::print_log(
                "[Insteon::Thermo_i2CS] WARN: Unknown Extended " . "Set/Get Data Received for " . $self->get_object_name )
              if $self->debuglevel( 1, 'insteon' );
        }
    }
...
sub _is_info_request {
    ...
    if ( $cmd eq 'thermostat_control' && $$self{_control_action} eq "mode" ) {
        my $val = $msg{extra};
        main::print_log( "[Insteon::Thermo_i2CS] Processing is_info_request for $cmd with value: $val" )
          if $self->debuglevel( 1, 'insteon' );
        if ( $val eq '09' ) {
            $self->_mode('Off');
        }
...
sub set {
    ...
    if ($found_state) {
        ::print_log(
            "[Insteon::Thermo_i2CS] Received request to set heat setpoint " . $p_state . " for device " . $self->get_object_name );
...
sub set {
    ...
    if ($found_state) {
        ::print_log(
            "[Insteon::Thermo_i2CS] Received request to set cool setpoint " . $p_state . " for device " . $self->get_object_name );
        if ( lc($p_state) eq 'cooler' ) {
