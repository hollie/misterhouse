...
@source_number_to_name = (
    '',       'CD',       'AUX',      'TAPE',    'TUNER',   'TUNER2',   'AM',    'FM',
    'MP3',    'BLUES',    'CHILDREN', 'CLASSIC', 'COUNTRY', 'DAD',      'DANCE', 'DVD',
    'LIGHTS', 'INTERNET', 'JAZZ',     'REQUEST', 'MOM',     'XM RADIO', 'POP',   'R&B',
    'RAP',    'HD RADIO', 'ROCK',     'SAT',     'SAT2',    'SOUL',     'WESTERN'
);

@source_number_to_name_30419 = (
    '',         'CD',      'AUX',     'TAPE',   'TUNER',   'TUNER2',  'AM',     'FM',       'MP3',  'BLUES',
    'CHILDREN', 'CLASSIC', 'COUNTRY', 'DAD',    'DANCE',   'DVD',     'LIGHTS', 'INTERNET', 'JAZZ', 'REQUEST',
    'MOM',      'XMRADIO', 'POP',     'R&B',    'RAP',     'RADIO',   'ROCK',   'SAT',      'SAT2', 'SOUL',
    'CD2',      'TALK',    'NEWS',    'SIRIUS', 'TRAFFIC', 'WEATHER', 'SPORTS', 'NPR',      'DSS',  'M SERVER',
    'DISH',     ''
);
...
@button_number_to_name =
  ( '', 'pause', 'stop', 'play', 'rewind', 'up', 'forward', 'left', 'down', 'right', 'previous', 'power', 'next' );

@button_number_to_name_40822 =
  ( '', 'pause', 'stop', 'play', 'shuffle', '', 'repeat', 'left', '', 'right', 'previous', 'mode', 'next' );
        ...
        if ( ( $Musica_Systems{$port_name}{'last_data_received'} + MAX_RESPONSE_WAIT ) < $::Time ) {
            ...
            if ( $Musica_Systems{$port_name}{'object'}->{'queue'}->[0] ) {
                    ...
                    or ( $Musica_Systems{$port_name}{'object'}->{'queue'}->[0] =~ /^StatVer/ ) )
                {
                    ...
                    $Musica_Systems{$port_name}{'object'}->_critical_error( "Re-sent command ["
                          . $Musica_Systems{$port_name}{'object'}->{'queue'}->[0]
                    ...
                    if ( ( $Musica_Systems{$port_name}{'object'}->{'zones'}[$i]->{'on_time'} + MAX_ZONE_ON_DELAY ) <= $::Time ) {
                        if ( $Musica_Systems{$port_name}{'object'}->{'on_repeat_count'} < 3 ) {
                            ...
                            $Musica_Systems{$port_name}{'object'}
                              ->_report_error( "Turning zone $i on again since it did not turn on before" );
                            ...
                            $Musica_Systems{$port_name}{'object'}
                              ->_critical_error( "Turning zone $i on again since it did not turn on before" );
                        }
                    }
                }
            }
    ...
    if ( $$self{'zones'}[$zone] and ( $source ne 'X' ) ) {
        ...
        unless ( $currsrc eq $source ) {
            ...
            if ( $currsrc eq '0' ) {
                ...
                &::print_log( "$self->{'port_name'}: _store_zone_source($zone,$source): Zone just turned on" )
                  if $main::Debug{musica};
                if (   ( $source eq '1' )
                    or ( $source eq '2' )
                    or ( $source eq '3' )
                ...
                if ( $source eq '0' ) {
                    if ( defined $currsrc ) {
                        &::print_log( "$self->{'port_name'}: _store_zone_source($zone,$source): Zone just turned off" )
                          if $main::Debug{musica};
                    ...
                    &::print_log( "$self->{'port_name'}: _store_zone_source($zone,$source): Zone source changed" )
                      if $main::Debug{musica};
...
sub _parse_data {
    ...
    if ( $cmd eq 'StatVer' ) {
            ...
            if ( $button > 12 ) {
                ...
                if ( ( $main::Time - $$self{'last_held_time'} ) <= IGNORE_BUTTON_PRESSED_AFTER_HELD ) {
                    if ( $$self{'last_held_button'} == $button ) {
                        &::print_log( "$self->{'port_name'}: ignoring button press because of recent button hold" )
                          if $main::Debug{musica};
                        $ignore = 1;
                    }
                }
        ...
        my (
            $zone,    $volume,     $bass,      $treble, $balance, $loudness, $mute,
            $blcolor, $brightness, $audioport, $amp,    $locked,  $overheat
        ) = split /\//, $value;
        if ( $$self{'zones'}[$zone] ) {
            ...
            if ( $$self{'zones'}[$zone]->{'just_turned_on'} ) {
                &::print_log( "$self->{'port_name'}: Got first EventData after zone $zone was turned on, setting zone_on state" )
                  if $main::Debug{musica};
                ...
                if ( $$self{'waiting_for_zone'} == $zone ) {
                    &::print_log( "$self->{'port_name'}: Queue was waiting on this zone, sending next message" )
                      if $main::Debug{musica};
    ...
    if ( $$self{'queue'}->[0] ) {
        ...
        if ( ( $cmd, $zone, $value ) = ( $$self{'queue'}->[0] =~ /^([^\/]+)\/(\d+)\/(\d+)/ ) ) {
            ...
            if ( $$self{'zones'}[$zone] ) {
                ...
                if ( $$self{'zones'}[$zone]->{'just_turned_on'} ) {
                    &::print_log( "$self->{'port_name'}: just_turned_on=$$self{'zones'}[$zone]->{'just_turned_on'} (zone $zone)" )
                      if $main::Debug{musica};
                    my $moved = '';
    ...
    if ( $zone == 0 ) {
        for ( my $i = 1; $i <= MAX_ZONES; $i++ ) {
            if ( $$self{'zones'}[$i] and $$self{'zones'}[$i]->{'present'} ) {
                $self->_store_zone_data( $zone, $member,
                    &Musica::_calc_zone_nudge( $$self{'zones'}[$i]->{$member}, $value, $min, $max ), $newstate );
            }
        }
    }
    else {
