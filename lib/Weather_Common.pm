...
sub weather_updated {
            ...
            and $$w{WindGustSpeed} > $$w{WindAvgSpeed} )
        {
            if ( $$w{WindGustSpeed} < 1 ) {
                ...
                $shortWindText = sprintf( '%s %.0f (%.0f) %s',
                    $windDirName, $$w{WindAvgSpeed}, $$w{WindGustSpeed}, $main::config_parms{weather_uom_wind} );
                if ( $$w{WindAvgSpeed} >= 1 ) {
                    ...
                    $longWindText = sprintf( '%serly gusts of %.0f %s',
                        $windDirNameLong, $$w{WindGustSpeed}, $main::config_parms{weather_uom_wind} );
                }
            }
        }
    }
    ...
    if ( $apparentTempText ne '' ) {
        $$w{Summary_Long} .=
          '  Apparent Temperature: ' . $$w{TempOutdoorApparent} . '&deg;' . $main::config_parms{weather_uom_temp};
    }
