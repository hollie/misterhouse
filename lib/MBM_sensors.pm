...
my $MBMSharedIndexPattern = 
   'C'.   # iType
   'N';   # iCount 
my $MBMSharedIndexLen = length pack $MBMSharedIndexPattern;
   $MBMSharedIndexLen += 4-($MBMSharedIndexLen % 4) if ($MBMSharedIndexLen % 4);

my $MBMSharedSensorPattern = 
   'C'.   # ssType
   'A12'. # ssName
   'x3'.  # ssPad
   'd'.   # ssCurrent value
   ...
   'x4'.  # ssPad2
   'H20'. # ssTotal - Note: Perl unpack has no equivalent to Delphi "Extended" 10 byte floating; So return ssTotal in hex for further processing in the parser
   'x6'.  # ssPad3
...
$MBMSharedSensorLen += 4-($MBMSharedSensorLen % 4) if ($MBMSharedSensorLen % 4);

my $MBMSharedInfoPattern = 
   'S'.    # siSMB_Base SMBus base address
...
my $MBMSharedInfoLen = length pack $MBMSharedInfoPattern;
   $MBMSharedInfoLen += 4-($MBMSharedInfoLen % 4) if ($MBMSharedInfoLen % 4);

my $MBMSharedMemPattern = 
   'd'.   # sdVersion
   $MBMSharedIndexPattern  x  10 .
   $MBMSharedSensorPattern x 100 .
   $MBMSharedInfoPattern         .
   'A41'. # sdStart    Start Time
...
my $MBMSharedMemLen = 
    ...
    (length pack 'A41') +
    (length pack 'A256');
$MBMSharedMemLen += 4-($MBMSharedMemLen % 4) if ($MBMSharedMemLen % 4);
...
for ($i =0 ; $i < $SensorTotalCount ; $i++) {
  my ($ssType, $ssName, $ssCurrent, $ssLow, $ssHigh, $ssCount, $ssTotal, $ssAlarm1, $ssAlarm2 ) = unpack "x$offset$MBMSharedSensorPattern", $MBMSharedMem;
#print "MBM_sensors.pm: $ssType, $ssName, $ssCurrent, $ssLow, $ssHigh, $ssCount, $ssTotal, $ssAlarm1, $ssAlarm2\n";
  ...
  $MBM_sensors{$ssType}{total}[$j] = $ssTotal;  # Need more work for this extended float
...
my ($siSMB_Base, $siSMB_Type, $siSMBCode, $siSMB_Addr, $siSMB_Name, $siISA_Base, $siChipType, $siVoltageSubType) = unpack "x$offset$MBMSharedInfoPattern", $MBMSharedMem;
