...
sub main::get_imap {
    ...
    my $service		= "local";
    $service		= $main::config_parms{"net_mail_${mhaccount}_service"} if (defined $main::config_parms{"net_mail_${mhaccount}_service"});
    my $ssl		= 0;
    ...
    my $size		= 0;
    $size 		= $main::config_parms{"net_mail_scan_size"} if (defined $main::config_parms{"net_mail_scan_size"});
    #allow local overrides...
    $size 		= $main::config_parms{"net_mail_${mhaccount}_scan_size"} if (defined $main::config_parms{"net_mail_${mhaccount}_scan_size"});
    $size = $size * 1024; #make it in K

    $server  = $main::config_parms{"net_mail_${mhaccount}_server"}  unless $server;
    $server = 'imap.gmail.com' if (lc $main::config_parms{"net_mail_${mhaccount}_service"} eq "gmail" and !$server);
...
if ($client->IsAuthenticated()) {
   ...
   $mailbox_size = $mailbox_size * 1024 if (defined $mailbox_size); #make it into bytes
   ...
   foreach my $msgid ( @msgs ){
	...
	next if ($age and !_check_age($date,$local_offset,$isdst,$age));
	#check_age($date);
	$processed_count++;
...
#	decode("MIME-Header", $from) if ($from =~ m/=\?/);
	...
	if ($size) {
	   ...
	   $body = $client->body_string($msgid) unless (defined $body); #try again for large messages
	}
	unless (defined $body) {
