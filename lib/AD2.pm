    ...
    if ( $status_type->{unknown} ) {
        ...
        foreach my $partition (@partitions) {
                ...
                && ( ( $zone_no_pad - $self->{zone_last_num}{$partition} ) != 1 ) )
            {
                ...
                for (
                    my $i = $start; ( $y <= $$self{max_zones} ) && ( ( !$reverse && $i <= $end )
...
sub GetStatusType {
    ...
    if ( $AdemcoStr =~ /(!KPM:)?\[([\d-]*)\],(\d{3}),\[(.*)\],\"(.*)\"/ ) {
        ...
        my @flags = (
            'ready_flag',    'armed_away_flag', 'armed_home_flag', 'backlight_flag',  'programming_flag', 'beep_count',
            'bypassed_flag', 'ac_flag',         'chime_flag',      'alarm_past_flag', 'alarm_now_flag',   'battery_low_flag',
...
sub set {
    ...
    if ( ref $p_setby and $p_setby->can('get_set_by') ) {
        ::print_log( "AD2_Output($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by " . $p_setby->get_set_by )
          if $main::Debug{AD2};
    }
    else {
