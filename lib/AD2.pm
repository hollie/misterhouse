...
sub new {
    ...
    $$self{max_zones} =
      250;    #The current max zones by any panel, can be increased
    ...
    $$self{CmdMsgRev} =
      { reverse %{ $$self{CmdMsg} } };    #DeRef Hash, Rev, Conv to Ref
...
sub restore_string {
    ...
    for my $partition ( keys %{ $$self{partition_address} } ) {
        for my $zone ( keys %{ $$self{$partition}{zone_status} } ) {
            ...
            $restore_string .=
              $self->{object_name} . "->zone_bypassed($zone, $bypass);\n";
        }
    }
...
sub serial_startup {
    ...
    if (    $::config_parms{ $instance . '_serial_port' }
        and $::config_parms{ $instance . '_serial_port' } ne '/dev/none' )
    {
          ...
          )
        {
            ...
            ::print_log(
                "[AD2] initializing $instance on port $port at $BaudRate baud")
              if $main::Debug{'AD2'};
            ::MainLoop_pre_add_hook(
                sub { AD2::check_for_data( $instance, 'serial' ); }, 1 )
...
sub server_startup {
    ...
    ::print_log(
        "  AD2.pm initializing $instance TCP session with $ip on port $port")
      if $main::Debug{'AD2'};
    ...
    ::MainLoop_pre_add_hook( sub { AD2::check_for_data( $instance, 'tcp' ); },
        1 );
}
...
sub check_for_data {
    ...
    foreach my $rf_key ( keys %{ $$self{wireless} } ) {
        if ( $rf_key =~ /.*\..*\.k/i ) {
            $self->ChangeZoneState( int( $$self{wireless}{$rf_key} ),
                "ready", 1 );
        }
    }
    ...
    if ( $connecttype eq 'tcp' ) {
        if ( $Socket_Items{$instance}{'socket'}->active ) {
            ...
            if ( $Socket_Items{$instance}{recon_timer}->inactive ) {
                ::print_log(
                    "Connection to $instance instance of AD2 was lost, I will try to reconnect in $$self{reconnect_time} seconds"
                );
...
sub CheckCmd {
    ...
    if ( $status_type->{unknown} ) {
        ...
        foreach my $partition (@partitions) {
            ...
            if ( $zone_no_pad < $self->{zone_last_num}{$partition} ) {
                ...
                if ( $self->{zone_last_num}{$partition} ==
                    $self->{highest_zone}{$partition} )
                {
              ...
              )
            {
                ...
                for (
                    my $i = $start;
                    ( $y <= $$self{max_zones} ) && ( ( !$reverse && $i <= $end )
                  ...
                  )
                {
                    ...
                    $y++;
                    $i = 0
                      if ( $i == $$self{max_zones} && $reverse );   #loop around
                }
            }
        ...
        $self->debug_log( "WIRELESS: rf_id("
              . $rf_id
              . ") status("
              . $status_type->{rf_status}
              . ") loop1("
              . $status_type->{rf_loop_fault_1}
              . ") loop2("
              . $status_type->{rf_loop_fault_2}
              . ") loop3("
              . $status_type->{rf_loop_fault_3}
              . ") loop4("
        ...
        $self->debug_log( "WIRELESS: rf_id("
              ...
              . $status_type->{rf_status}
              . ") low_batt("
              . $status_type->{rf_low_batt}
              . ") supervised("
              . $status_type->{rf_supervised}
              . ")" );
        ...
        $self->debug_log(
            "EXPANDER: exp_id($exp_id) input($input_id) status($status)");
        ...
        $self->debug_log(
            "RELAY: rel_id($rel_id) input($rel_input_id) status($rel_status)");
    ...
    if ( $status_type->{keypad} ) {
        ...
        if ( $status_type->{alarm_now_flag} ) {
            ...
            $self->debug_log( "ALARM IS SOUNDING - Zone $zone_no_pad ("
                  . $self->zone_name($zone_no_pad)
                  . ")" );
...
sub GetStatusType {
    ...
    if ( $AdemcoStr =~ /(!KPM:)?\[([\d-]*)\],(\d{3}),\[(.*)\],\"(.*)\"/ ) {
        ...
        my @flags = (
            'ready_flag',       'armed_away_flag',
            'armed_home_flag',  'backlight_flag',
            'programming_flag', 'beep_count',
            'bypassed_flag',    'ac_flag',
            'chime_flag',       'alarm_past_flag',
            'alarm_now_flag',   'battery_low_flag',
            'no_delay_flag',    'fire_flag',
            'zone_issue_flag',  'perimeter_only_flag'
        );
        for ( my $i = 0; $i <= 15; $i++ ) {
        ...
        $message{rf_unknown_1} =
          ( ( hex( substr( $message{rf_status}, 1, 1 ) ) & 1 ) == 1 ) ? 1 : 0;
        $message{rf_low_batt} =
          ( ( hex( substr( $message{rf_status}, 1, 1 ) ) & 2 ) == 2 ) ? 1 : 0;
        $message{rf_supervised} =
          ( ( hex( substr( $message{rf_status}, 1, 1 ) ) & 4 ) == 4 ) ? 1 : 0;
        $message{rf_unknown_8} =
          ( ( hex( substr( $message{rf_status}, 1, 1 ) ) & 8 ) == 8 ) ? 1 : 0;

        $message{rf_loop_fault_1} =
          ( ( hex( substr( $message{rf_status}, 0, 1 ) ) & 8 ) == 8 ) ? 1 : 0;
        $message{rf_loop_fault_2} =
          ( ( hex( substr( $message{rf_status}, 0, 1 ) ) & 2 ) == 2 ) ? 1 : 0;
        $message{rf_loop_fault_3} =
          ( ( hex( substr( $message{rf_status}, 0, 1 ) ) & 1 ) == 1 ) ? 1 : 0;
        $message{rf_loop_fault_4} =
          ( ( hex( substr( $message{rf_status}, 0, 1 ) ) & 4 ) == 4 ) ? 1 : 0;
...
sub ChangeZoneState {
    ...
    if ( $self->status_zone($zone) ne $new_status ) {
        ...
        #  Set the new state
        $$self{ $self->zone_partition($zone) }{zone_status}{$zone} =
          $new_status;
        ...
        $$self{partition_object}{$zone_partition}
          ->set_receive( $partition_status, $$self{zone_object}{"$zone"} )
          if defined $$self{partition_object}{$zone_partition};

        #  Log everything if requested
        if ( $log == 1 ) {
            my $ZoneNumPadded = sprintf( "%03d", $zone );
            $self->debug_log( "Zone $zone ("
                  . $self->zone_name($zone)
...
sub update_child_object {
    ...
    {
        $$self{zone_object}{"$zone"}
          ->set( $status, $$self{zone_object}{"$zone"} );
    }
}
...
sub DefineCmdMsg {
    ...
    my %Return_Hash = (
        ...
        "ArmStayInstant" => $Configuration{ $instance . "_user_master_code" }
          . "7",
    ...
    foreach my $key ( keys %Configuration ) {
        ...
        if ( $key =~ /^${instance}_output_(\D+)_(\d+)$/ ) {
            if ( $1 eq 'co' ) {
                $Return_Hash{ $Configuration{$key} . "c" } =
                  $Configuration{ $instance . "_user_master_code" } . "#70$2";
                $Return_Hash{ $Configuration{$key} . "o" } =
                  $Configuration{ $instance . "_user_master_code" } . "#80$2";
            }
            elsif ( $1 eq 'oc' ) {
                $Return_Hash{ $Configuration{$key} . "o" } =
                  $Configuration{ $instance . "_user_master_code" } . "#80$2";
        ...
        #Create Commands for Zone Expanders
            ...
            $two_digit_zone =
              substr( $Configuration{$key}, 1 );    #Trim leading zero
            $Return_Hash{ "exp" . $Configuration{$key} . "c" } =
              "L$two_digit_zone" . "0";
            $Return_Hash{ "exp" . $Configuration{$key} . "f" } =
              "L$two_digit_zone" . "1";
            $Return_Hash{ "exp" . $Configuration{$key} . "p" } =
              "L$two_digit_zone" . "2";
        }
    }
...
sub debug_log {
    my ( $self, $text ) = @_;
    my $instance = $$self{instance};
    ::logit( $$self{log_file}, $text )
      unless ( $Configuration{ $instance . '_debug_log' } == 0 );
}
...
sub cmd {
    ...
    if (   ( $password ne $Configuration{ $instance . '_user_master_code' } )
        && ( $CmdName ne "ShowFaults" ) )
    {
        ::logit("Invalid password for command $CmdName ($password)");
        return;
    }
    $self->debug_log(
        ">>> Sending to ADEMCO panel              $CmdName ($cmd)");
    ...
    if ( defined $Socket_Items{$instance} ) {
        if ( $Socket_Items{ $instance . '_sender' }{'socket'}->active ) {
            ...
            if ( $Socket_Items{$instance}{recon_timer}->inactive ) {
                ::print_log(
                    "Connection to $instance sending instance of AD2 was lost, I will try to reconnect in $$self{reconnect_time} seconds"
                );
                $Socket_Items{$instance}{recon_timer}->set(
                    $$self{reconnect_time},
                    sub {
...
sub set {
    ...
    $p_state = lc($p_state);
    my $cmd =
      ( exists $self->{CmdMsg}->{$p_state} )
      ? $self->{CmdMsg}->{$p_state}
      : $p_state;
    $self->debug_log(
        ">>> Sending to ADEMCO panel              $p_state ($cmd)");
...
sub new {
    my ( $class, $type, $interface, $zone, $partition, $expander, $relay,
        $wireless )
      = @_;
...
sub set {
    ...
    if ( ref $p_setby and $p_setby->can('get_set_by') ) {
        ::print_log(
            "AD2_Item($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by "
              . $p_setby->get_set_by )
          if $main::Debug{AD2};
    }
    else {
        ::print_log("AD2_Item($$self{object_name})::set($p_state, $p_setby)")
          if $main::Debug{AD2};
    }
...
sub new {
    ...
    @{ $$self{states} } = (
        'Disarm', 'ArmAway', 'ArmStay',        'ArmAwayMax',
        'Test',   'Bypass',  'ArmStayInstant', 'Code',
        'Chime',  'ToggleVoice'
    );
    return $self;
}
...
sub set {
    ...
    if ( ref $p_setby and $p_setby->can('get_set_by') ) {
        ::print_log(
            "AD2_Output($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by "
              . $p_setby->get_set_by )
          if $main::Debug{AD2};
    }
    else {
        ::print_log("AD2_Output($$self{object_name})::set($p_state, $p_setby)")
          if $main::Debug{AD2};
    }
    my $reported_state;
