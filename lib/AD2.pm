...
sub serial_startup {
        ...
        and $::config_parms{ $instance . '_serial_port' } ne '/dev/none' )
    {
        ...
        if ( &main::serial_port_create( $instance, $port, $BaudRate, 'none', 'raw' ) ) {
            ...
            ::print_log("[AD2] initializing $instance on port $port at $BaudRate baud")
              if $main::Debug{'AD2'};
            ::MainLoop_pre_add_hook( sub { AD2::check_for_data( $instance, 'serial' ); }, 1 )
...
sub server_startup {
    ...
    ::print_log("  AD2.pm initializing $instance TCP session with $ip on port $port")
      if $main::Debug{'AD2'};
    $Socket_Items{"$instance"}{'socket'} =
      new Socket_Item( $instance, undef, "$ip:$port", $instance, 'tcp', 'raw' );
    $Socket_Items{ "$instance" . '_sender' }{'socket'} =
      new Socket_Item( $instance . '_sender', undef, "$ip:$port", $instance . '_sender', 'tcp', 'rawout' );
...
sub CheckCmd {
    ...
    if ( $status_type->{keypad} ) {
        ...
        # ARMED HOME
        $self->debug_log( "armed_home_flag = "
              . $status_type->{armed_home_flag}
              . " Alpha status = "
              . $status_type->{alphanumeric}
              . "| stay = "
              . index( $status_type->{alphanumeric}, "***STAY***" )
              . " exit delay = "
              . index( $status_type->{alphanumeric}, "You may exit now" )
              . "entry delay = "
              . index( $status_type->{alphanumeric}, "or alarm occurs" ) );
...
sub GetStatusType {
    ...
    if ( $AdemcoStr =~ /(!KPM:)?\[([\d-]*)\],(\d{3}),\[(.*)\],\"(.*)\"/ ) {
        ...
        my @flags = (
            'ready_flag',       'armed_away_flag', 'armed_home_flag', 'backlight_flag',
            'programming_flag', 'beep_count',      'bypassed_flag',   'ac_flag',
            'chime_flag',       'alarm_past_flag', 'alarm_now_flag',  'battery_low_flag',
            'no_delay_flag',    'fire_flag',       'zone_issue_flag', 'perimeter_only_flag'
        );
        for ( my $i = 0; $i <= 15; $i++ ) {
...
sub DefineCmdMsg {
    ...
    foreach my $key ( keys %::config_parms ) {
        ...
        if ( $key =~ /^${instance}_output_(\D+)_(\d+)$/ ) {
            if ( $1 eq 'co' ) {
                $Return_Hash{ $::config_parms{$key} . "c" } =
                  $::config_parms{ $instance . "_user_master_code" } . "#70$2";
                $Return_Hash{ $::config_parms{$key} . "o" } =
                  $::config_parms{ $instance . "_user_master_code" } . "#80$2";
            }
            elsif ( $1 eq 'oc' ) {
                $Return_Hash{ $::config_parms{$key} . "o" } =
                  $::config_parms{ $instance . "_user_master_code" } . "#80$2";
...
sub set {
    ...
    if ( ref $p_setby and $p_setby->can('get_set_by') ) {
        ::print_log(
            "AD2_Item($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by " . $p_setby->get_set_by )
          if $main::Debug{AD2};
    }
    else {
...
sub new {
    ...
    @{ $$self{states} } =
      ( 'Disarm', 'ArmAway', 'ArmStay', 'ArmAwayMax', 'Test', 'Bypass', 'ArmStayInstant', 'Code', 'Chime', 'ToggleVoice' );
    return $self;
}
...
sub set {
    ...
    if ( ref $p_setby and $p_setby->can('get_set_by') ) {
        ::print_log(
            "AD2_Output($$self{object_name})::set($p_state, $p_setby): $$p_setby{object_name} was set by " . $p_setby->get_set_by )
          if $main::Debug{AD2};
    }
    else {
