...
our ($barometric_trend, $next_rec, $barometric_press, $air_temp_inside, $humidity_inside,
    $air_temp, $wind_speed, $wind_speed_10min_ave, $wind_direction, $relative_humidity,
    $rain_rate, $uv, $solar, $rain_storm, $storm_date, $rain_day, $rain_month, $rain_year,
    $day_ET, $month_ET, $year_ET, $alarms_inside, $alarms_rain, $alarms_outside,
    $batt_xmit, $batt_cons, $forecast_icon, $forecast_rule, $sunrise, $sunset, $dew_point_inside,
    $dew_point_outside, $crc, $crc_calc, $wptr, $data);
...
sub wake_up{
  ...
  foreach (1..3)
  {
    ...
    if ($str eq "\n\r" ) 
	{ 
      &::print_log ("davisvantageproii: success on wakeup") if $::Debug{weather};
   	return 1; 
	}
	 
	&::print_log ("davisvantageproii: no wakeup response") if $::Debug{weather}; 
	
	sleep 1; # As per page 5 of VantagePro Doc 
  }

  &::print_log ("davisvantageproii: could not wake up unit") if $::Debug{weather}; 
  return 1; # fail
}

# called by trigger every minute
sub requestData {
	 &::print_log ("davisvantageproii: requesting new data from station") if $::Debug{weather};
      #!#!#!#! &wake_up;
      $DavisVP_port->set($wakeupCommand);
...
sub process{
  ...
  if ($loo eq 'LOO') {
     &::print_log ("davisvantageproii: found proper LOO header") if $::Debug{weather};
  }
  else {
     &::print_log ("davisvantageproii: proper LOO header not found") if $::Debug{weather};
     return '';
  }
...
## ?	}
	...
	&::print_log ("davisvantageproii: found a header with the right checksum") if $::Debug{weather};
...
#     $dew_point_outside		= $air_temp_inside - ( (100 - $humidity_inside)/5 );	
     ...
     if ($humidity_inside ne undef) {$dew_point_inside=&::convert_humidity_to_dewpoint($humidity_inside,&::convert_f2c($air_temp_inside))};
     if ($relative_humidity ne undef) {$dew_point_outside=&::convert_humidity_to_dewpoint($relative_humidity,&::convert_f2c($air_temp))};
