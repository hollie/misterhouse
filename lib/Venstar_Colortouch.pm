  ...
  if ($isSuccessResponse1) {
    ...
    if (($stat->{api_ver} > 3) and ($stat->{type} eq "residential")) {
      ...
      if ($self->poll()) {
    	...
    	foreach my $key1 (keys $self->{data}->{info}) { $self->{previous}->{info}->{$key1} = $self->{data}->{info}->{$key1}; }
...
sub poll {
  ...
  main::print_log("[Venstar Colortouch] Polling initiated") if ($self->{debug});
...
sub _push_JSON_data {
  ...
  my ($cooltemp) = $params =~ /cooltemp=(\d+)/;
    ...
    my ($isSuccessResponse,$cmode,$cfan,$cheattemp,$ccooltemp,$setpointdelta,$minheat,$maxheat,$mincool,$maxcool) = $self->_get_control_params;
...
sub _get_control_params {
  ...
  return ($isSuccessResponse,$info->{mode},$info->{fan},$info->{heattemp},$info->{cooltemp},$info->{setpointdelta},$info->{heattempmin},$info->{heattempmax},$info->{cooltempmin},$info->{cooltempmax});
}
...
sub print_info {
	...
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Device " . $self->{data}->{name} . " is " . $type . " Thermostat");
	...
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Heat:\t" . $self->{data}->{info}->{heattemp} . "$unit\t\t" . $self->{data}->{info}->{heattempmin} . "$unit\t" . $self->{data}->{info}->{heattempmax} . "$unit");
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Cool:\t" . $self->{data}->{info}->{cooltemp} . "$unit\t\t" . $self->{data}->{info}->{cooltempmin} . "$unit\t" . $self->{data}->{info}->{cooltempmax} . "$unit");
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Humidity:\t" . $self->{data}->{info}->{hum_setpoint} . "%") unless ($self->{data}->{info}->{hum_setpoint} == 99);
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Dehumidity:" . $self->{data}->{info}->{dehum_setpoint} . "%") unless ($self->{data}->{info}->{dehum_setpoint} == 99);

	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Setpoint Delta: " . $self->{data}->{info}->{setpointdelta} . "$unit");
		
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Current Thermostat Sensor Temperature:" . $self->{data}->{sensors}->{sensors}[0]->{temp} . "$unit");
	main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Current Thermostat Sensor Humidity:" . $self->{data}->{sensors}->{sensors}[0]->{hum} . "%");
	if ($self->{data}->{sensors}->{sensors}[1]->{temp} != -39) {
		main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Current Outdoor Sensor Temperature:" . $self->{data}->{sensors}->{sensors}[1]->{temp});
	} else {
...
sub process_data {
	...
	if ($self->{previous}->{tempunits} != $self->{data}->{tempunits}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Temperature Units changed from $self->{previous}->{tempunits} to $self->{data}->{tempunits}");
	...
	if ($self->{previous}->{info}->{availablemodes} != $self->{data}->{info}->{availablemodes}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Available Modes changed from $self->{previous}->{info}->{availablemodes} to $self->{data}->{info}->{availablemodes}");
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] This really isn't a regular operation. Should check Thermostat to confirm");
	  $self->{previous}->{info}->{availablemodes} = $self->{data}->{info}->{availablemodes};
	}
	
	if ($self->{previous}->{info}->{fan} != $self->{data}->{info}->{fan}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Fan changed from $fan[$self->{previous}->{info}->{fan}] to $fan[$self->{data}->{info}->{fan}]") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{fanstate} != $self->{data}->{info}->{fanstate}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Fan state changed from $fanstate[$self->{previous}->{info}->{fanstate}] to $fanstate[$self->{data}->{info}->{fanstate}]") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{mode} != $self->{data}->{info}->{mode}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Mode changed from $mode[$self->{previous}->{info}->{mode}] to $mode[$self->{data}->{info}->{mode}]") if ($self->{loglevel});
	  $self->{previous}->{info}->{mode} = $self->{data}->{info}->{mode};
	}

	if ($self->{previous}->{info}->{state} != $self->{data}->{info}->{state}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat state changed from $state[$self->{previous}->{info}->{state}] to $state[$self->{data}->{info}->{state}]") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{schedule} != $self->{data}->{info}->{schedule}) {
	  ...
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat state changed to" . $sch . "on a schedule") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{schedulepart} != $self->{data}->{info}->{schedulepart}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat schedule changed from $schedule[$self->{previous}->{info}->{schedulepart}] to $schedule[$self->{data}->{info}->{schedulepart}]") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{away} != $self->{data}->{info}->{away}) {
	  my $away = "home mode";
	  $away = "away mode" if ($self->{data}->{info}->{away});
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat occupency changed to" . $away) if ($self->{loglevel});
	  $self->{previous}->{info}->{away} = $self->{data}->{info}->{away};
	}

	if ($self->{previous}->{info}->{spacetemp} != $self->{data}->{info}->{spacetemp}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat temperature changed from $self->{previous}->{info}->{spacetemp} to $self->{data}->{info}->{spacetemp}") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{heattemp} != $self->{data}->{info}->{heattemp}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat heating setpoint changed from $self->{previous}->{info}->{heattemp} to $self->{data}->{info}->{heattemp}") if ($self->{loglevel});
	  $self->{previous}->{info}->{heattemp} = $self->{data}->{info}->{heattemp};
	}	

	if ($self->{previous}->{info}->{heattempmin} != $self->{data}->{info}->{heattempmin}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat heating setpoint minimum changed from $self->{previous}->{info}->{heattempmin} to $self->{data}->{info}->{heattempmin}") if ($self->{loglevel});
	  $self->{previous}->{info}->{heattempmin} = $self->{data}->{info}->{heattempmin};
	}	
	
	if ($self->{previous}->{info}->{heattempmax} != $self->{data}->{info}->{heattempmax}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat heating setpoint maximum changed from $self->{previous}->{info}->{heattempmax} to $self->{data}->{info}->{heattempmax}") if ($self->{loglevel});
	  $self->{previous}->{info}->{heattempmax} = $self->{data}->{info}->{heattempmax};
	}	
	
	if ($self->{previous}->{info}->{cooltemp} != $self->{data}->{info}->{cooltemp}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat cooling setpoint changed from $self->{previous}->{info}->{cooltemp} to $self->{data}->{info}->{cooltemp}") if ($self->{loglevel});
	  $self->{previous}->{info}->{cooltemp} = $self->{data}->{info}->{cooltemp};
	}	

	if ($self->{previous}->{info}->{cooltempmin} != $self->{data}->{info}->{cooltempmin}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat cooling setpoint minimum changed from $self->{previous}->{info}->{cooltempmin} to $self->{data}->{info}->{cooltempmin}") if ($self->{loglevel});
	  $self->{previous}->{info}->{cooltempmin} = $self->{data}->{info}->{cooltempmin};
	}	
	
	if ($self->{previous}->{info}->{cooltempmax} != $self->{data}->{info}->{cooltempmax}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat cooling setpoint maximum changed from $self->{previous}->{info}->{cooltempmax} to $self->{data}->{info}->{cooltempmax}") if ($self->{loglevel});
	...
	if ($self->{previous}->{info}->{setpointdelta} != $self->{data}->{info}->{setpointdelta}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat setpoint delta changed from $self->{previous}->{info}->{setpointdelta} to $self->{data}->{info}->{setpointdelta}") if ($self->{loglevel});
	...
	if ($self->{previous}->{sensors}->{sensors}[0]->{hum} != $self->{data}->{sensors}->{sensors}[0]->{hum}) {
	  main::print_log("[Venstar Colortouch:". $self->{data}->{name} . "] Thermostat Humidity Sensor changed from $self->{previous}->{sensors}->{sensors}[0]->{hum} to $self->{data}->{sensors}->{sensors}[0]->{hum}") if ($self->{loglevel});
	  $self->{previous}->{sensors}->{sensors}[0]->{hum} = $self->{data}->{sensors}->{sensors}[0]->{hum};
...
sub get_mode_humid {
  ...
  $value = 3 if (($self->{data}->{info}->{dehum_setpoint} != 99) and ($self->{data}->{info}->{hum_setpoint} != 99));
...
sub set_mode {
  ...
  my $num;
  if ((lc $value eq lc "auto") and ($self->{data}->{info}->{availablemodes} == 0)) {
    $num = 3;
  } elsif (((lc $value eq lc "cooling") or (lc $value eq lc "cool")) and ($self->{data}->{info}->{availablemodes} != 2)) {
    $num = 2;
  } elsif (((lc $value eq lc "heating") or (lc $value eq lc "heat")) and ($self->{data}->{info}->{availablemodes} != 3)) {
    $num = 1;
  } elsif (lc $value eq lc "off") {
    $num = 0;
  } else {
    main::print_log("Venstar Colortouch:". $self->{data}->{name} . "] Error, unknown mode $value or mismatch with Thermostat Modes" . $self->{data}->{info}->{availablemodes});
    return ('0');
  }
