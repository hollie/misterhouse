...
sub startup {
   ...
   my $BaudRate = ( defined $::config_parms{DSC_5401_baudrate} ) ? $main::config_parms{DSC_5401_baudrate} : 9600;
   if ( &main::serial_port_create( 'DSC5401', $port, $BaudRate, 'none', 'raw' ) ) {
      init( $::Serial_Ports{DSC5401}{object}, $port );
      &main::print_log("  DSC5401.pm initializing port $port at $BaudRate baud") if $main::config_parms{debug} eq 'DSC5401';
      &::MainLoop_pre_add_hook( \&DSC5401::check_for_data, 1 ) if $main::Serial_Ports{DSC5401}{object};
...
sub CheckCmd {
   ...
   if ( IsChecksumOK($CmdStr) ) {
      ...
      if ( $cmd == 500 ) {    # System Error
         ...
         LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} $Hour:$Min $MM/$DD/$YY" ) if $main::config_parms{DSC_5401_time_log};
         ...
         my $Name = $data;
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd}:" ) if $main::config_parms{DSC_5401_ring_log};
         $self->{ring_now} = 1;
      }
         ...
         $ZoneName = $main::config_parms{"DSC_5401_zone_$ZoneNum"}  if exists $main::config_parms{"DSC_5401_zone_$ZoneNum"};
         ...
         $ZoneName = $main::config_parms{"DSC_5401_zone_${data}"} if exists $main::config_parms{"DSC_5401_zone_${data}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} zone $ZoneName  ($ZoneNum)" ) if $main::config_parms{DSC_5401_zone_log};
         $ZoneNum =~ s/^0*//;
         $self->{zone_now_cmd}            = $cmd;
         ...
         $PartName = $main::config_parms{"DSC_5401_part_${data}"} if exists $main::config_parms{"DSC_5401_part_${data}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} partition $PartName  ($PartNum)" ) if $main::config_parms{DSC_5401_part_log};
         $self->{zone_now_cmd}         = $cmd;
         ...
         my $PartName = my $PartNum = $data;
         $PartName = $main::config_parms{"DSC_5401_part_${data}"} if exists $main::config_parms{"DSC_5401_part_${data}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} parition $PartName  ($PartNum)" ) if $main::config_parms{DSC_5401_part_log};
         ...
         my $PartName = my $PartNum = $data;
         $PartName = $main::config_parms{"DSC_5401_part_${data}"} if exists $main::config_parms{"DSC_5401_part_${data}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} partition $PartName ($PartNum) by user $self->{user_name} ($self->{user_id})" );
         set $self "disarmed";
         $self->{partition_now_cmd}    = $cmd;
         ...
         $PartName = $main::config_parms{"DSC_5401_part_${PartName}"} if exists $main::config_parms{"DSC_5401_part_${PartName}"};
         $UserName = $main::config_parms{"DSC_5401_user_${UserName}"} if exists $main::config_parms{"DSC_5401_user_${UserName}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} user $UserName ($UserNum) closing partition $PartName ($PartNum)" );
         ...
         $PartName = $main::config_parms{"DSC_5401_part_${PartName}"} if exists $main::config_parms{"DSC_5401_part_${PartName}"};
         $UserName = $main::config_parms{"DSC_5401_user_${UserName}"} if exists $main::config_parms{"DSC_5401_user_${UserName}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} User $UserName ($UserNum) opening partition $PartName ($PartNum)" );
         ...
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} Phone line 1 is in open or short condition" );
         ...
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} Phone line 2 is in open or short condition" );
         ...
         my $PartName = my $PartNum = $data;
         $PartName = $main::config_parms{"DSC_5401_part_${PartName}"} if exists $main::config_parms{"DSC_5401_part_${PartName}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} System report trouble on partition $PartName" );
         ...
         my $PartName = my $PartNum = $data;
         $PartName = $main::config_parms{"DSC_5401_part_${PartName}"} if exists $main::config_parms{"DSC_5401_part_${PartName}"};
         &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", "$cmd $EventMsg{$cmd} System report no trouble on partition $PartName" );
...
sub cmd {
   ...
   if ( $cmd eq "033" || $cmd eq "040" ) {    # we don't display password
      ...
      &LocalLogit( "$main::config_parms{data_dir}/logs/DSC5401.$main::Year_Month_Now.log", ">>> Sending to DSC panel                      $CmdName ($cmd) with argument ($arg)  [$CmdStr]" );
      $main::Serial_Ports{DSC5401}{object}->write("$CmdStr\r\n");
...
sub zone_now_fault_restore {
   return $_[0]->{zone_now_fault_restore} if defined $_[0]->{zone_now_fault_restore};
}
...
sub zone_name {
   ...
   my $ZoneName = $main::config_parms{"DSC_5401_zone_$zone_num"} if exists $main::config_parms{"DSC_5401_zone_$zone_num"};
...
sub partition_name {
   my ( $class, $part_num ) = @_;
   my $PartName = $main::config_parms{"DSC_5401_part_$part_num"} if exists $main::config_parms{"DSC_5401_part_$part_num"};
...
sub IntTstat {
   my ( $class, $TstatNum ) = @_;
   return $_[0]->{IntTstatTemp}{$TstatNum}, $_[0]->{IntTstatTime}{$TstatNum}, $_[0]->{IntTstatEpoch}{$TstatNum} if defined $_[0]->{IntTstatTemp};
   return -999;
}

sub ExtTstat {
   my ( $class, $TstatNum ) = @_;
   return $_[0]->{ExtTstatTemp}{$TstatNum}, $_[0]->{ExtTstatTime}{$TstatNum}, $_[0]->{ExtTstatEpoch}{$TstatNum} if defined $_[0]->{ExtTstatTemp};
   return -999;
}
