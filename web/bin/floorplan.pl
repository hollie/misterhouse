...
return &html_page('FloorPlan', "No $object_name Group found to generate a floorplan from") unless $object;
					...
					if ($l_obj ne "") { #Only do if object is at coordinates
						if ( $l_rendered{$l_obj} eq '' ) {
							...
							if ($l_obj->isa('Group') ) { #recurse groups
								$l_html.= "\t<td bgcolor='$l_bcolor' width='" . $l_xscale*$l_w . "' height='" . $l_yscale * $l_h . "' colspan='$l_w' rowspan='$l_h'>";
								$l_html.= web_fp($l_obj);
							} else {
...
#	my $l_max=10*60;
	...
	if ($l_agreen > $l_bgreen) {
		$l_green = int ( $l_agreen - (($l_agreen-$l_bgreen) * ($l_percent)) );
	} else {
		$l_green = int ( $l_agreen + (($l_bgreen-$l_agreen) * ($l_percent)) );
	}
	if ($l_ablue > $l_bblue) {
