...
|;
$html .= qq|<script language="JavaScript">dateused=true</script>| if $parms{date};

if ($parms{jclock1} or $parms{jclock2}) {
    $html .= &file_read("$config_parms{html_dir}/bin/clock1.js") if $parms{jclock1};
    $html .= &file_read("$config_parms{html_dir}/bin/clock2.js") if $parms{jclock2};
    $html .= "</head>\n";
