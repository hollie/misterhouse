...
#  my $year = $cgi->param('debyear');
  ...
  if ($debepochtime <= $endepochtime) {
  	...
  	&create_rrdgraph_zoom($debepochtime,$endepochtime,$cgi->param('width'), $cgi->param('height'),
...
print $cgi->start_center;
print $cgi->start_table({-width=>"710", -border=>"0", -cellpading=>"0", -cellspacing=>"0"});
...
print $cgi->popup_menu(-name=>'sensor1', -value=>\@list_sensors, -size=>1, -default=>$cgi->param('sensor1'),-labels=>\%sensor_names);
print $cgi->br;
# Add null value
push @list_sensors, "nosensor";
$sensor_names{"nosensor"}="";
print $cgi->popup_menu(-name=>'sensor2', -value=>\@list_sensors, -size=>1, -default=>$cgi->param('sensor2'),-labels=>\%sensor_names);
print $cgi->end_td;
...
print $cgi->end_td;
print $cgi->start_td({-align=>'left',-colspan=>1,valign=>"center"});
print $cgi->popup_menu(-name=>'width', -size=>1, -maxlength=>4, -value=>([1..5000]) , -default=>$cgi->param('width'));
print $cgi->br;
print $cgi->popup_menu(-name=>'height', -size=>1, -maxlength=>4, -value=>([1..1500]) , -default=>$cgi->param('height'));
print $cgi->end_b;
print $cgi->end_td;
...
sub create_rrdgraph_zoom {
  ...
  if ($sensor1 eq 'temp' or $sensor1 eq 'dew' or $sensor1 eq 'intemp' or $sensor1 eq 'indew' or
      ...
      $sensor1 eq 'dewspare1' or $sensor1 eq 'dewspare2' or $sensor1 eq 'dewspare3' or $sensor1 eq 'chill')
  	{
         ...
         $strvar=($config_parms{weather_uom_temp} eq 'C' ? "\"CDEF:fvar=var,32,-,5,9,/,*\"," : "\"CDEF:fvar=var\",");
	 $strminvar=($config_parms{weather_uom_temp} eq 'C' ? "\"CDEF:fmindata=mindata,32,-,5,9,/,*\"," : "\"CDEF:fmindata=mindata\",");
	 $strmaxvar=($config_parms{weather_uom_temp} eq 'C' ? "\"CDEF:fmaxdata=maxdata,32,-,5,9,/,*\"," : "\"CDEF:fmaxdata=maxdata\",");
  	}
  if ($sensor1 eq 'press')
  	{
	 $libuom=($config_parms{weather_uom_baro} eq 'mb' ? "\"Millibars (mb)\"," : "\"inch mercury (inHg)\",");
         $strvar=($config_parms{weather_uom_baro} eq 'mb' ? "\"CDEF:fvar=var,0.029529987508,/\"," : "\"CDEF:fvar=var\",");
	 $strminvar=($config_parms{weather_uom_baro} eq 'mb' ? "\"CDEF:fmindata=mindata,0.029529987508,/\"," : "\"CDEF:fmindata=mindata\",");
	 $strmaxvar=($config_parms{weather_uom_baro} eq 'mb'? "\"CDEF:fmaxdata=maxdata,0.029529987508,/\"," : "\"CDEF:fmaxdata=maxdata\",");
  	}
  ...
  if ($sensor1 eq 'speed' or $sensor1 eq 'avgspeed')
  	{
	 $libuom=($config_parms{weather_uom_wind} eq 'kph' ? "\"Killometers per hour (kph)\"," : "\"Miles per hour (mph)\",");
         $strvar=($config_parms{weather_uom_wind} eq 'kph' ? "\"CDEF:fvar=var,1.609344,*\"," : "\"CDEF:fvar=var\",");
	 $strminvar=($config_parms{weather_uom_wind} eq 'kph' ? "\"CDEF:fmindata=mindata,1.609344,*\"," : "\"CDEF:fmindata=mindata\",");
	 $strmaxvar=($config_parms{weather_uom_wind} eq 'kph'? "\"CDEF:fmaxdata=maxdata,1.609344,*\"," : "\"CDEF:fmaxdata=maxdata\",");
  	}
  if ($sensor1 eq 'rate')
  	{
	 $libuom=($config_parms{weather_uom_rainrate} eq 'mm/hr' ? "\"Millimeters per hour (mm/hr)\"," : "\"Inches per hour (in/hr)\",");
         $strvar=($config_parms{weather_uom_rainrate} eq 'mm/hr' ? "\"CDEF:fvar=var,0.0393700787402,/\"," : "\"CDEF:fvar=var\",");
	 $strminvar=($config_parms{weather_uom_rainrate} eq 'mm/hr' ? "\"CDEF:fmindata=mindata,0.0393700787402,/\"," : "\"CDEF:fmindata=mindata\",");
	 $strmaxvar=($config_parms{weather_uom_rainrate} eq 'mm/hr'? "\"CDEF:fmaxdata=maxdata,0.0393700787402,/\"," : "\"CDEF:fmaxdata=maxdata\",");
  	}
  if ($sensor1 eq 'rain')
  	{
	 $libuom=($config_parms{weather_uom_rain} eq 'mm' ? "\"Millimeters (mm)\"," : "\"Inches (in)\",");
         $strvar=($config_parms{weather_uom_rain} eq 'mm' ? "\"CDEF:fvar=var,0.0393700787402,/\"," : "\"CDEF:fvar=var\",");
	 $strminvar=($config_parms{weather_uom_rain} eq 'mm' ? "\"CDEF:fmindata=mindata,0.0393700787402,/\"," : "\"CDEF:fmindata=mindata\",");
	 $strmaxvar=($config_parms{weather_uom_rain} eq 'mm'? "\"CDEF:fmaxdata=maxdata,0.0393700787402,/\"," : "\"CDEF:fmaxdata=maxdata\",");
  	}
  if ($sensor1 eq 'humid' or $sensor1 eq 'inhumid' or
      $sensor1 eq 'humidspare1' or $sensor1 eq 'humidspare2' or $sensor1 eq 'humidspare3' or $sensor1 eq 'humidspare4' or
      $sensor1 eq 'humidspare5' or $sensor1 eq 'humidspare6' or $sensor1 eq 'humidspare7' or $sensor1 eq 'humidspare8' or
      $sensor1 eq 'humidspare9' or $sensor1 eq 'humidspare10')
  	{
         $libuom="\"Percent %\",";
  	}
