...
#     print_log "Error in backup up parms file $private_parms: $!";
    ...
    foreach my $key (@order) {
        if ((! defined $dparms{$key} and $cparms{$key}) or
          $cparms{$key} ne $dparms{$key}) {
...
sub localize {
    ...
    foreach my $file (sort @files) {
        next if $file =~ /location_specimen|example/; # Ignore mh.location_specimen.ini and mh.example.ini
...
sub select {
    ...
    file_write("$Pgm_Root/bin/mh.testing.ini", $data) if $testing;  # For testing only
