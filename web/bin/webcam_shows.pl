...
@files = sort(@files);
	...
	$html .= "<TR><TD WIDTH=50>  </TD>\n";
        $html .= "<TD BGCOLOR='$bgcolor' ALIGN='center'>" . $href . 
	    ...
	    if ($last_image) {
	        $jnk = substr($last_image,0,4) . "/" .substr($last_image,4,2) . "/" .substr($last_image,6,2) . " " .substr($last_image,8,2) . ":" . 
    		 substr($last_image,10,2) . ":" .substr($last_image,12,2) . " " .substr($last_image,14) ;
