#!/usr/bin/perl
# MrWiki - modified from UseModWiki V0.92 (www.usemod.com)
# Version 1.0 - 30 Aug 2003 - Richard Phillips
#
# Requires the following settings in mh.ini or mh.private.ini
# wiki_data=[location of wiki database] eg wiki_data=/home/mrwiki
# wiki_diff=[location of "diff" utility] eg wiki_diff=/usr/bin/diff
# win32 users can get a version if diff at http://sources.redhat.com/cygwin/ or http://unxutils.sourceforge.net/
...
sub BrowsePage {
    ...
    if ( $revision ne '' ) {
        ...
        if ( $goodRevision ne '' ) {
            ...
            $fullHtml .=
              '<b>' . Ts( 'Revision %s not available', $revision ) . ' (' . T('showing current revision instead') . ')</b><br>';
        }
    }
...
sub DoEdit {
    ...
    print STDERR "db1 id=$id nt=$newText\n";
...
sub DoEditPrefs {
    ...
    if ($EmailNotify) {
        print "<br>";
        print &GetFormCheck( 'notify', 1, T('Include this address in the site email list.') ),
          ' ',
    ...
    print '<br>', T('Edit area rows:'), ' ', &GetFormText( 'editrows', 20, 4, 4 ),
...
sub SaveUserData {
    ...
    print STDERR "db uf=$userFile d=$data\n";
