...
sub confirm_files {
    ...
    $html .=
      "<tr><td bgcolor='9999CC'>Directory</td><td><input name=dir size=" . ( length($dir) * 1.5 ) . " value=\"$dir\"></td></tr>\n";
...
sub track_edit {
    ...
    $html .= &do_select_row( '', 'Genre', 'genre', &remove_dups( @{ $combined{'genre'} } ),
    ...
    foreach ( my $i = 1; $i <= ( $#{@$track_numbers} + 1 ); $i++ ) {
        $html .= &do_select_row( 'track', "$i ($track_lengths->[$i-1])",
            "track${i}title", &remove_dups( &fix_caps( @{ $combined{'tracks'}->[$i] } ) ) );
    }
