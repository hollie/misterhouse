...
sub PrintDefault {
    ...
    print "<a href='$scriptName?vsSD=$showDayDetails&vsMonth=$month&vsYear="
      ...
      . "'>&lt;&lt;</a>\n";
    print
      "&nbsp;<a href='$scriptName?vsSD=$showDayDetails&vsMonth=$pmonth&vsYear=$pyear'>&lt;</a>\n";
    print
    ...
    print
      "<option value='vsMonth=$pmonth&vsYear=$pyear'>$pmonth / $pyear</option>\n";
    print
      "<option value='vsMonth=$month&vsYear=$year' selected>$month / $year</option>\n";
    my ($nM) = $month;
    my ($nY) = $year;
    ...
    print "</select>\n";
    print
      " <a href='$scriptName?vsSD=$showDayDetails&vsMonth=$nmonth&vsYear=$nyear'>&gt;</a>\n";
    print
...
sub PrintMonth {
    ...
    if ($showDayDetails) {
        print
          " <font size='1'>[<a href='$scriptName?vsSD=0&vsMA=$showforAudrey&vsMonth=$month&vsYear=$year'>Hide Details</a>]</font>\n";
    }
    else {
        print
          " <font size='1'>[<a href='$scriptName?vsSD=1&vsMA=$showforAudrey&vsMonth=$month&vsYear=$year'>Show Details</a>]</font>\n";
    }
    ...
    for ( my $cellCount = 1; $cellCount <= $numWeeks; $cellCount++ ) {
        ...
        foreach $temp (@days) {
            if ( ( $dayCount > $firstDay - 1 ) && ( $weekDayCount < $numDays ) )
            {
                ...
                if ( $objMyDb->EOF ) {
                    print
                      "<a $style href='$scriptName?vsSD=$showDayDetails&vsMA=$showforAudrey&vsMonth=$month&vsYear=$year&vsDay=$weekDayCount'>$weekDayCount</a><br>";
                }
                else {
                    #$style = "style=\"color:$dataHolidayColor\"" if ($objMyDb->FieldValue("HOLIDAY") eq "on");
                    print
                      "<b><a $style href='$scriptName?vsSD=$showDayDetails&vsMA=$showforAudrey&vsMonth=$month&vsYear=$year&vsDay=$weekDayCount'>$weekDayCount</a></b><br>";
                }
...
sub GetMonthInfo {
    ...
    $numDays = (
        localtime(
            Time::Local::timelocal( 0, 0, 0, 1, $nmonth, $nyear ) - 60 * 60 * 24
        )
    )[3];
