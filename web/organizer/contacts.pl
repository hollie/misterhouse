...
sub PrintAllRecords {
    ...
    if ( !$showforAudrey ) {
        if ($showAll) {
            print
              "<input type='button' onclick=\"window.location='$scriptName?vsALL=0';\" value='Show $pageSize Per Page'>";
        }
        else {
            print
              "<input type='button' onclick=\"window.location='$scriptName?vsALL=1';\" value='Show All'>";
        }
    }
    print
      "&nbsp;<input type='button' onclick=\"window.location='$scriptName?vsSORT=$sortField&vsMA=$showforAudrey&vsAP=$activePage&vsCOM=ADD';\" value='New Contact'>\n";
    print "</td><td>\n";
    ...
    while ( !$objMyDB->EOF && $count < $visiblePageSize ) {
        ...
        print "<td><font face='arial' size='1'><a href='"
          ...
          . "'><img src='$detailIcon' alt='Details' border='0'></a></font></td>\n";
        foreach $fieldName (@showFields) {
    ...
    if ( $activePage > 1 ) {
        print
          ...
          . ( $activePage - 1 )
          . "&vsFilterField=$filterField&vsFilterValue=$filterValue'>Previous</a>";
    }
    if ( $activePage < $pageCount ) {
        print
          " <a href='?vsALL=$showAll&vsMA=$showforAudrey&vsSORT=$sortField&vsAP="
          . ( $activePage + 1 )
          . "&vsFilterField=$filterField&vsFilterValue=$filterValue'>Next</a>";
    }
