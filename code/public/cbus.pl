...
# Voice Commands
$v_cbus_builder = new  Voice_Cmd("CBus Builder ".
                                 "[Test,Run,Scan CGate,List Devices]");
...
if (my $data = said $v_cbus_builder) {
    if ($data eq 'Run') {
        ...
        if (not defined $cbus_def) {
            # Their was no cbus def file to load.
            # Help out a new user, by auto-building the def file.
            # Otherwise, their will be nothing to build.
...
sub load_def_file {
    ...
    if (not -e $cbus_def_filename) {
        print_log "CBus: Definition file $cbus_def_filename does not exist";
        return;
    }

    print_log "CBus: Builder - Loading CBus config from file ".
                $cbus_def_filename;
...
sub build_cbus_file {
    ...
    foreach my $address (sort keys %{ $cbus_def->{group} }  ) {
        ...
        if ($type eq 'watchdog') {
            ...
            $ramptime = $config_parms{cbus_watchdog_timer} unless defined $ramptime;
...
if (my $cbus_data = said $cbus_talker) { 
    ...
    if ($msg_code == 320) {
        ...
        if ($cbus_scanning_cgate) {
            if ($cbus_data =~ /(\/\/.+\/\d+\/[a-z\d]+\/\d+)\/TagName=(.+)/) {
                my ($addr, $name) = ($1, $2);
        ...
        if (not defined $cbus_project_name) {
            ...
            my $cmd =  "print_log  'CBus: project load $cbus_project_name'; ".
                        "set \$cbus_talker 'project load $cbus_project_name'; ".
...
if (active $cbus_talker and $cbus_scanning_cgate) {
    if (not $cbus_scanning_tree) {
        ...
        if ($cbus_group_idx < @cbus_group_list) {
            ...
            if ($cbus_scan_last_addr_seen eq 
                            $cbus_unit_list[$#cbus_unit_list]) {
