...
if ( my $caller_id_data = said $NCID ) {
    my ( $caller, $cid_number, $cid_name, $time ) = &Caller_ID::make_speakable( $caller_id_data, 5 );

    print_log "NCID data=$caller_id_data";
    print "phone number=$cid_number name=$cid_name\n";

    &::logit( "$::config_parms{data_dir}/phone/logs/callerid.$::Year_Month_Now.log", "$cid_number name=$cid_name line=1 type=in" );
