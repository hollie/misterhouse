...
#    	print_log $vvLine;
    	if ($vvLine !~ /^\#/) {
    		($vvLan[$count], $vvLanName[$count], $URLList[$count]) = (split(',', $vvLine))[0,1,2];	# Split each line
            $vvNbr = $count;
...
sub process {
	...
	if ($myraw !~ /Läge:Namn:Typ:/ and $myLage !~ /^Lv/) {
		...
		$query_select = "SELECT raw FROM Roadwork WHERE raw = \'$myLage $myNamn $myTyp\'";
  		$sth = $dbh->prepare($query_select) or print "Can't prepare $query_select: $dbh->errstr\n";
...
#    	print "==> @row\n";
    	...
    	if ($row[0] eq "") {
	   		$query_insert = "INSERT into Roadwork (raw,Lan,Lage,Namn,Typ,Ber,Start,Stopp,Res,Besk,Last,Add) values (\'$myLage $myNamn $myTyp\',\'$myLan\',\'$myLage\',\'$myNamn\',\'$myTyp\',\'$myBer\',\'$BerStart\',\'$BerStopp\',\'$myRes\',\'$myBesk\',\'$myTime\',\'$myTime\')"; 
