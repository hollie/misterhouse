...
#		print_msg "$Time $myTime";
		$query_select2 = "SELECT raw,LatLon,ID,Typ,Lan,Namn FROM Roadwork WHERE Sent < $myTime and LatLon <> Null and Send = Yes ORDER BY Sent ";
  		$sth2 = $dbh2->prepare($query_select2) or print "Can't prepare $query_select2: $dbh2->errstr\n";
...
#				set	$tnc_output3	$aprs_vv;
				...
				$sth3 = $dbh2->prepare($query_update2) or print "Can't prepare $query_update2: $dbh2->errstr\n";
...
#		$query_delete = "SELECT * FROM Roadwork WHERE Stopp < #$myTime2#";
 		$query_delete = "DELETE * FROM Roadwork WHERE Stopp < #$myTime2#";
  		$sth4 = $dbh4->prepare($query_delete) or print "Can't prepare $query_delete: $dbh4->errstr\n";
...
if (new_minute 15 or said $vv_mail eq 'Mail') {
	...
	$query_select = "SELECT Roadwork.ID,Roadwork.Namn,Roadwork.LatLon,Location.ID,Location.Namn,Location.LatLon FROM Roadwork,Location WHERE LocCheck = No and Roadwork.Namn = Location.Namn and (Roadwork.LatLon Is Null or Location.LatLon Is Null)";
  	$sth5 = $dbh5->prepare($query_select) or print "Can't prepare $query_select: $dbh5->errstr\n";
	...
	while (($RID, $RName, $RLatLon, $LID, $LName, $LLatLon)=$sth5->fetchrow_array) {
		...
		if ($RLatLon eq "" and $LLatLon ne "") {
			...
			$sth3 = $dbh5->prepare($query_update2) or print "Can't prepare $query_update2: $dbh5->errstr\n";
			$sth3->execute() or print "can't execute the query: $sth3->errstr\n";
			$sth3->finish();
		}
	...
	$sth3 = $dbh5->prepare($query_insert2) or print "Can't prepare $query_insert2: $dbh5->errstr\n";
 	$sth3->execute() or print "can't execute the query: $sth3->errstr\n";
	$sth3->finish();	
	$sth5->finish();
	...
	while (@row2=$sth5->fetchrow_array) {
		&net_mail_send(to => "roger.bille\@telia.com", subject => "APRS VV $row2[0]", text => "@row2");
