        ...
        if ( $text =~ /$AirportCode / ) {               # Check	if Airport is in text
            ...
            foreach $data (@data) {                     # Process each field
                ...
                if ( $data =~ /KT/ ) {                  # Check	if Wind	data (03010KT,VRB02KT)
                    ...
                    $aprs_windspeed =
                      sprintf( "%03d", $wind_speed * 1.852 / 1.609344 );    # Convert to mph with 3 characters	beginning with 0's
                    $gust_speed = 0;
                    if ( $data =~ /G/ ) {
                        $gust_speed = substr( $data, 6, 2 );
                        $aprs_gustspeed =
                          sprintf( "%03d", $gust_speed * 1.852 / 1.609344 );   # Convert to mph with 3 characters	beginning with 0's
                    }
                }
