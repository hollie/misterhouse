...
use	vars '@AHubUP','@AHubUsers','@AHubUsersX','@AHubBps','@AHubPkts','@AHubVer','@AHubVerX','@AHubTime','@AHubTimeX','@AHubName','@AHubRaw','@AHubRawX';
...
# 	$APDNbr	= 0;
	foreach	$AHubLine (@AHub) {			
		($AHubName[$count],	$AHubWWW[$count], $AHubLoc[$count])	= (split(',', $AHubLine))[0,1,2];	# Split	each line
		$AHubNbr = $count;
		$AHubMail[$count] =	0;
		$AHubMailX[$count] =	0;
...
if ($New_Minute) {
	for	($count	= 0;$count <= $AHubNbr;$count++) {
		...
		$AHubCon60X[$count] = 0;
		for	($x	= 0;$x <= 59;$x++) {
			$AHubCon60[$count] = $AHubCon60[$count] + $AHubCon[$count*60+$x];
			$AHubCon60X[$count] = $AHubCon60X[$count] + $AHubConX[$count*60+$x];
		}
	}
	for	($count	= 1;$count <= 3;$count++) {
		...
		$CoreCon60[$count] = 0;
		for	($x	= 0;$x <= 59;$x++) {
			$CoreCon60[$count] = $CoreCon60[$count] + $CoreCon[$count*60+$x];
		}
	}
}
...
#			print_msg "APD=: $APDList[$key]";
			...
			if ($APDName[$key] eq "APRS-1") {
				...
				if ($APRSString	=~ /.*[Vv]erified*/) { $CoreCon[1*60+$Minute]++ };
			}
...
#				$ThirdUsers = $APDUsers[$key];		# comment since third does not send users
				...
				if ($APRSString	=~ /.*[Vv]erified*/) { $CoreCon[3*60+$Minute]++ };
			}
...
if ($New_Minute) {
	...
	for	($count	= 0;$count <= $AHubNbr;$count++) {
		for	($key = 0;$key <= $MailCount;$key++) {
			...
			$Time2 = ($MailStart[$key] + $AHubMail[$count] * $MailRem[$key]) * 60;
##			print_msg "Mail1: $AHubName[$count] $MailName[$key] $AHubTime[$count] $MailStart[$key] $AHubMailX[$count] $MailRem[$key] Time1=$Time1 Time2=$Time2";
			if ($AHubName[$count] eq $MailName[$key] and 
					(time() - $AHubTime[$count]) > ($MailStart[$key] + $AHubMail[$count] * $MailRem[$key]) * 60 and
					$AHubTime[$count] != "") {
...
#				$MailTo = "$MailAddr[$key]";
				...
				foreach $MailTo (@List) {
					&net_mail_send(to => "$MailTo",	subject	=> "Warning $AHubMail[$count] for AHub on $AHubName[$count] $Time1.",
						text => "$MailText", from => 'APRSstat@ahubswe.net');
				}
			}
		}
		for	($key = 0;$key <= $MailCount;$key++) {
			$Time1 = time() - $AHubTimeX[$count];
			$Time2 = ($MailStart[$key] + $AHubMailX[$count] * $MailRem[$key]) * 60;
##			print_msg "Mail2: $AHubName[$count] $MailName[$key] $AHubTimeX[$count] $MailStart[$key] $AHubMailX[$count] $MailRem[$key] Time1=$Time1 Time2=$Time2";
			if ($AHubName[$count] eq $MailName[$key] and 
					(time() - $AHubTimeX[$count]) > ($MailStart[$key] + $AHubMailX[$count] * $MailRem[$key]) * 60 and
					$AHubTimeX[$count] != "") {
#				$Time1 = &utc($AHubTime[$count]);
				$Time4 = &utc($AHubTimeX[$count]);
				$Time2 = &utc(time());
...
#				$MailTo = "$MailAddr[$key]";
				@List = split(/ /,$MailAddr[$key]);
				foreach $MailTo (@List) {
					&net_mail_send(to => "$MailTo",	subject	=> "Warning $AHubMailX[$count] for AFilterX on $AHubName[$count] $Time4.",
						 text => "$MailText", from => 'APRSstat@ahubswe.net');
				}
			}
		}
...
#	open(HTML, ">\\\\server\\d\$\\wwwroot\\ahub_table.html");
	...
	for	($count	= 0;$count <= $AHubNbr;$count++) {
		...
		if (&up($AHubUP[$count]) < 14400 and $AHubUP[$count] !=	"") { $bgUp = $bgYel };
	...
	$BordCol = $BordCol	. "	style=\"border-top-style: solid; border-top-color: #000000\"";
		...
		print HTML "</tr>\n";
	print HTML "</table>\n\n";
	print HTML "<p><font size=\"2\">Time in <span style=\"background-color: #FF0000\">red</span> if no update last 25 minutes. Uptime in <span style=\"background-color: #FFFF00\">yellow</span> if less than 4 hours. Connect is number of connections to the server last 60 minutes</font></p>";
#	close HTML;
}
...
sub core_table {
	...
	print HTML "</table>\n\n";
	print HTML "<p><font size=\"2\">Time in <span style=\"background-color: #FF0000\">red</span> if no update last 25 minutes. Uptime in <span style=\"background-color: #FFFF00\">yellow</span> if less than 4 hours. Third's users is updated once every 30 minutes</font></p>";
}
...
sub core_test_table {
	...
	if (time() - $FirstTime >	1500 and $FirstTime != "") { print HTML "	bgcolor=\"#FF0000\""};
	...
	print HTML "<td	width=\"14%\"$BordCol><font	size=\"2\">$SecondLoc</font></td>\n";
	print HTML "<td	width=\"15%\"$BordCol";
	if (time() - $SecondTime >	1500 and $SecondTime != "") { print HTML "	bgcolor=\"#FF0000\""};
	print HTML "><font size=\"2\">$Time1</font></td>\n";
	print HTML "<td	width=\"8%\"$BordCol><font size=\"2\">$SecondVer</font></td>\n";
	...
	print HTML "<td	width=\"14%\"$BordCol><font	size=\"2\">$ThirdLoc</font></td>\n";
	print HTML "<td	width=\"15%\"$BordCol";
	if (time() - $ThirdTime >	1500 and $ThirdTime != "") { print HTML "	bgcolor=\"#FF0000\""};
	print HTML "><font size=\"2\">$Time1</font></td>\n";
	print HTML "<td	width=\"8%\"$BordCol><font size=\"2\">$ThirdVer</font></td>\n";
...
sub aprsd_table {
	...
	for	($count	= 0;$count <= $APDNbr;$count++)	{
		...
		print HTML "<td	width=\"15%\"$BordCol";
		if (time() - $APDTime[$count] >	1500 and $APDTime[$count] != "") { print HTML "	bgcolor=\"#FF0000\""};
		print HTML "><font size=\"2\">$Time1</font></td>\n";
