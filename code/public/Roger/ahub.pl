...
if ($Reload) {
    ...
    $count = 0;
    foreach $AHubLine (@AHub) {
        ( $MailName[$count], $MailStart[$count], $MailRem[$count], $MailAddr[$count] ) =
...
sub second {
    ...
    if ( $AH2ndRestarts != 0 ) {
        ...
        print HTML "second $AH2ndRestarts restarts since $Time4. Last restart $Time3 (Current uptime = $Diff minutes)."
