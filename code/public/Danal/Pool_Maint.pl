...
if (state_now $Pool_Maint_age) {
   ...
   speak (volume => 90, text => "Djeeni says: Good Job swimmer, you maintained the pool and spa.");
...
if (time_cron '3,18,33,48 * * * 0,6') {
   my ($date, $time, $state) = (state_log $Pool_Maint_age)[0] =~ /(\S+) (\S+) *(.*)/;
   use Time::ParseDate;
   ...
   if ($days > 5) {
      ...
      my $state = state $Pool_Maint_age;
	speak (volume => 70, text => "Djeeni says: Time to maintain the pool and spa, please shock the pool with $state.");
...
if (time_cron '33 6,7,8,9,10 * * 1,2,3,4,5') {
   ...
   if ($days > 8) {
      print_log "Speaking Pool Maint nag message";
	speak (volume => 50, text => "Djeeni says: You skipped a weekend, Time to catch up on pool maintenance");
