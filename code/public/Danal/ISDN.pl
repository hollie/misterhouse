...
if ( my $packet = said $ISDN) {
    ...
    if (    ( $current{1}{callflag} == 4 )
        and ( $current{1}{destname} ne $data1_call ) )
    {    # New data call?
    ...
    if (    ( $current{2}{callflag} == 4 )
        and ( $current{2}{destname} ne $data2_call ) )
    {    # New data call?
...
sub callerid {
    ...
    my ($cid_nmbr) = @_;
    my $cid_speak_nmbr =
      substr( $cid_nmbr, 0, 3 ) . "." . substr( $cid_nmbr, 3, 3 ) . "." . substr( $cid_nmbr, 6, 4 );    # Pauses for speak
