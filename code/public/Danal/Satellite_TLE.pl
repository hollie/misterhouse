...
$v_get_Satellite_TLE->set_info("Get Two Line Elements for 100 Brightest Satellites");
...
$p_Satellite_TLE = new Process_Item("get_url $u_Satellite_TLE $f_Satellite_TLE");

if ( my $state = state_now $v_get_Satellite_TLE) {
    start $p_Satellite_TLE;
    print_log "Getting Visual 100 Satellite TLEs from $u_Satellite_TLE; placing in $f_Satellite_TLE";
}
