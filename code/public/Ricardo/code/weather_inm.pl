...
#@ The script parses the weather pages from www.inm.es
#@ and places the data into the %Weather hash for use by other parts of
#@ misterhouse.
#
# Check the file in http://www.inm.es/web/infmet/tobsr/bin/nombres.js
#
# Unmodified, you'll get the weather for Madrid (Retiro) in Spain
#
...
# A separate global for the 4-day forecast.
...
# get_weather_sp; I'll fix that later.
...
sub fetch_ec_weather {
    ...
    #                              $Weather{TempIndoor}, $Weather{TempOutdoor}, $Weather{WindChill},
    #                              $Weather{HumidIndoor}, $Weather{HumidOutdoor});
    $Weather{Summary} = sprintf( "out/chill: %2d/%2d Humid:%3d%%", $Weather{TempOutdoor}, $Weather{WindChill}, $Weather{HumidOutdoor} );
