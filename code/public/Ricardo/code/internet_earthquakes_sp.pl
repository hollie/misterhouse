...
if (($state eq 'Lee') or ((state $mode_mh eq 'normal') and time_cron("0,30 * * * *")) ) {
...
sub calc_age {
    ...
    return "hace " . int($diff/(60*60)) . " horas " if ($qtime > $midnight);
    my $hour;
    ...
    if (my ($qevent, $qdate, $qtime, $qlatd, $qnoso, $qlong, $qeawe, $qmagn, $qpercv, $qmec, $qdept, $qloca) =
      ...
      if ( $clatd && $clong && $ccity) { 
	      # reference to a diferent place
              $distance = sprintf "%d kilómetros de %s,", calc_distance($clatd,
                 $clong, $qlatd, $qlong) + .5, $ccity;
      }
      else {
	      # reference distance to mh home city
              $distance = sprintf "%d kilómetros", calc_distance($config_parms{latitude},
