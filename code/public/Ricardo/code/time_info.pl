...
speak "Hoy es el cumpleaños de $Birthday" if $Birthday and ($Weekend or $Holiday) and time_random('* 11-22 * * *',120);
speak "Hoy es el cumpleaños de $Birthday" if $Birthday and ($Weekday and !$Holiday) and time_random('* 20-22 * * *',90);
...
speak "Hoy es el santo de $Nameday" if $Nameday and ($Weekend or $Holiday) and time_random('* 11-22 * * *',120);
speak "Hoy es el santo de $Nameday" if $Nameday and ($Weekday and !$Holiday) and time_random('* 20-22 * * *',90);

$v_sun_set = new  Voice_Cmd('Cuando es la puesta de sol', 0);
$v_sun_set-> set_info("Calcula la puesta de sol para la latitud=$config_parms{latitude}, y longitud=$config_parms{longitude}");
$v_sun_set-> set_authority('anyone');
respond "El sol sale hoy a las $Time_Sunrise, la puesta de sol es a las $Time_Sunset." if said $v_sun_set;

speak "Aviso, el sol está saliendo ahora a las " . say_time($Time_Sunrise)  if time_now $Time_Sunrise and !$Save{sleeping_parents};
speak "app=notice Aviso, el sol se está poniendo ahora a las " . say_time($Time_Sunset)  if time_now $Time_Sunset;
...
$v_moon_info3-> set_info('fase será: Nueva, Cuarto creciente, Llena, Cuarto menguante, ...r');
...
if ($state = said $v_moon_info2) {
    ...
    respond qq[La última luna $state fué hace $days, el $Moon{"${state_en}_prev"}];
}

if ($state = said $v_moon_info3) {
    respond qq[la fase de la luna es $Moon{phase}, con un brillo del $Moon{brightness}%, y $Moon{age} dias];
}

$full_moon = new File_Item("$config_parms{data_dir}/remarks/full_moon.txt");
if ($Moon{phase} eq 'Llena' and time_random('* 8-22 * * *', 240)) {
    respond "app=notice Aviso, está noche es luna llena.  " . (read_next $full_moon);
}
