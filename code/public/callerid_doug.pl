...
if ($PhoneModemString = said $phone_modem)
{
	...
	speak("Phone call is from $cid_name");
        ...
        my $query_insert = "INSERT into calls set name=\"$cid_name\", number=\"$cid_number\", local_datetime=unix_timestamp(\"$Year-$Month-$Mday $Hour:$Minute:$Second\");"; 
...
{
	my ($dbh) = DBI->connect("DBI:mysql:callerid:localhost", "USER", "PASS");
	my $query = "SELECT name, number, from_unixtime(local_datetime) as when from calls where week(from_unixtime(unix_timestamp(), \"%Y-%m-%d\")) = week(from_unixtime(local_datetime, \"%Y-%m-%d\")) order by when desc;";
