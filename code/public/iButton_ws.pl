...
if ( time_cron('* * * * *') ) {
    ...
    $weather{Summary} =
      sprintf( "In/out/freezer: %3.1f/%3.1f/%3.1f ", $weather{Temp_inside}, $weather{Temp_outside}, $weather{Temp_freezer} );
    ...
    $weather{SummaryRain} =
      sprintf( "RainTotal/RainRecent/IsRaining: %3.2f/%3.2f/%d ", $weather{RainTotal}, $weather{RainRecent}, $weather{IsRaining} );
