...
if (said $v_what_time) {
    ...
    speak "voice=Whispering-Male Es ist $Time_Now on $Date_Now_Speakable. $temp";
}
...
$v_sun_set-> set_info("Berechnet Sonnen-aufgang und -untergang für latitude=$config_parms{latitude}, longitude=$config_parms{longitude}");
$v_sun_set-> set_authority('anyone');
#speak "Sunrise today is at $Time_Sunrise, sunset is at $Time_Sunset." if said $v_sun_set;
speak "Gender=male Voice=8 Sonnenaufgang ist heute $Time_Sunrise, Sonnenuntergang ist $Time_Sunset." if said $v_sun_set;

#speak "Notice, the sun is now rising at $Time_Sunrise" if time_now $Time_Sunrise and !$Save{sleeping_parents};
speak "Gender=male Voice=8 Mitteilung, Sonnenaufgang ist jetzt $Time_Sunrise" if time_now $Time_Sunrise and !$Save{sleeping_parents};
#speak "rooms=all Notice, the sun is now seting at $Time_Sunset"  if time_now $Time_Sunset;
speak "rooms=all Gender=male Voice=3 Mitteilung, die Sonne geht jetzt unter $Time_Sunset"  if time_now $Time_Sunset;
...
$v_moon_info3-> set_info('Phase wird sein: Neu, Ein Viertel wachsend, Hälfte wachsend, Drei-Viertel wachsend, Voll, etc for Waning');
...
#    speak qq[The next $state moon is in $days, on $Moon{$state}];
    speak qq[Gender=female Voice=2  Der naechste $state Mond ist in $days, am $Moon{$state}];
}
