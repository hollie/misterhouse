...
if ( $state = said $v_webcam_light) {
    ...
    logit( "$config_parms{data_dir}/logs/webcam.$Year_Month_Now.log", "ip=$Socket_Ports{http}{client_ip_address} domain=$domain_name" );
...
if ( $Save{web_text1} ) {
    ...
    my $domain_name =
      &net_domain_name( $Socket_Ports{http}{client_ip_address} );
    logit( "$config_parms{data_dir}/logs/webtext.$Year_Month_Now.log", "ip=$Socket_Ports{http}{client_ip_address} $Save{web_nick1} said $Save{web_text1}" );
