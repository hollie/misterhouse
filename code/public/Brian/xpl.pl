...
$xpl_jabber_resp   = new xPL_Item('doghouse-blabber.archestar20','control.basic' => {device => 'brian.klier@gmail.com/BlackBerry4BF62086', current => '$state'});
...
if ($state = state_now $xpl_jabber) {
        ...
        if (($xpljabberdevicecon eq 'brian.klier') and ($xpljabbertype eq 'message') and ($xpljabbercommand ne $xpllastcommand)) {
