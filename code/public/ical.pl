...
$p_ical_read  = new Process_Item("ical_load -days 1 -calendar $config_parms{calendar_file} -pl_file $ical_out_file");
...
$v_ical_read -> set_info('This will check create mh events to announce calendar entries');
if (said  $v_ical_read or ($New_Minute and !($Minute % 5) and changed $f_ical_data)) {
    start $p_ical_read;
