...
my $hvac = { HeatingDefaults => { Unoccupied => 50,
				  Occupied => 71,
				  Sleeping => 68,
				  Maximum => 75,
				},
	     CoolingDefaults => { Unoccupied => 90,
				  Occupied => 71,
				  Sleeping => 75,
				  Maximum => 67,
				},
...
if ( $Authorized ) {
	...
	foreach my $j ( qw( Name Controller Unoccupied Occupied Sleeping Maximum ) ) {
	    ...
	    if ( $i =~ /Heating_Tie_${j}_(\d+)_(\d+)/ ) {
	        my ( $zone, $num ) = ( $1, $2 );
		$hvac->{ heatingzones }->[$zone-1]->{ Ties }->[$num-1]->{ $j } = $ARGS{ $i };
	    }
        }
    }
	...
	if ( $i =~ /MoveHeatingZoneDown_(\d+)/ ) {
	    my $num = $1;
	    @{$hvac->{ heatingzones }}[$num-1,$num] = @{$hvac->{ heatingzones }}[$num,$num-1];
	}
	if ( $i =~ /MoveHeatingZoneUp_(\d+)/ ) {
	    my $num = $1;
	    @{$hvac->{ heatingzones }}[$num-2,$num-1] = @{$hvac->{ heatingzones }}[$num-1,$num-2];
	}
    }

    if ( $ARGS{ AddHeatingZone } ) {
	push @{$hvac->{ heatingzones }}, { Name => "Name this zone",
					   Rooms => [],
					   Ties => [],    
	...
	if ( $ARGS{ SETUP } =~ /Heating_(\d+)/ ) {
	    my $zone = $1 - 1;
	    push @{$hvac->{ heatingzones }->[ $zone ]->{ Rooms }}, { Name => "Name this room",
...
sub setupHeatingZone {
    ...
    my $select;
    my $selected = $hvac->{ heatingzones }->[ $zone ]->{ Controller } || "No Controller";
    ...
    $hvac->{ heatingzones }->[ $zone ]->{ Ties } = [ {} ] if !$hvac->{ heatingzones }->[ $zone ]->{ Ties } ||
...
sub showZone {
	...
	if ( $object ) {
	    ...
	    $str .= "Zone is currently " . $object->state . " and has been since " . 
...
sub showAllHeatingZones {
	...
	if ( $Save{ hvac_statistics }->[0]->{ heattime }
	    ...
	    my $time = showTime( $Save{ hvac_statistics }->[0]->{ heattime }->{ furnace } );
    ...
    if ( $object ) {
	$str .= "Furnace is currently " . $object->state . " and has been since " . 
	  scalar( localtime( $Time - $object->get_idle_time ) );
	$str .= "<br />\n";
    }
...
#		$code .= "   set \$hvac_${name}_desired $j->{ $k };\n";
		$code .= "   set \$hvac_${name}_desired " . '$hvac->{ heatingzones }->[ ' .
		  $zonenum . ' ]->{ Rooms }->[ ' . $roomnum . ' ]->{ ' . $k . " };\n";
		$code .= "}\n";
		$code .= "\n";
	    }
	    $roomnum++;
