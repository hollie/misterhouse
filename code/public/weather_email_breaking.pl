...
if (done_now $p_get_email and -e $get_email_scan_file) {
	...
	&net_mail_send(to => '$person3email', subject => "Weather Alert", text => "$subject", debug => 1);
	}
    }
}
...
if (time_cron '0,15,30,45 * * * *'){#speaks every 15 minutes
	...
	open (weatherwariningfile, "$config_parms{data_dir}/logs/weatherwarning.txt") or die "No Warnings available.\n";
