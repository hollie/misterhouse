...
my ( $ph_id, $ph_date, $ph_time, $cid_number, $ph_duration, $ph_line, $ph_extn, $ph_account, $ph_type );

# get the data if it's there
if ( my $data = said $merlin_data) {

    # Split it up into it's parts
    ( $ph_id, $ph_date, $ph_time, $cid_number, $ph_duration, $ph_line, $ph_extn, $ph_account ) = split( /\s+/, $data );

    # All the outgoing calls are Id'd wit a C
    if ( $ph_id eq "C" and $cid_number ne "IN" ) {
        $cid_number = substr( $cid_number, 0, 11 );
        my $ph_nm = "Ln " + $ph_line + " ext " + $ph_extn + " for " + $ph_duration;
        ...
        logit( "$config_parms{data_dir}/phone/logs/phone.$Year_Month_Now.log", "$cid_number name=$ph_duration ext=$ph_extn line=$ph_line type=$ph_type" );

        #		&::logit("$::config_parms{data_dir}/phone/logs/phone.$::Year_Month_Now.log",
        #		" $l_number name=$l_name line=$ph_line type=$tempsource");

        logit_dbm( "$config_parms{data_dir}/phone/phone.dbm", $cid_number, "$Time_Now $Date_Now $Year", "name=$ph_duration line=$ph_line" );
