...
sub im_status {
  ...
  if (lc $user eq 'aimscreenname1' or lc $user eq 'aimscreenname2') {
    ...
    if ($status eq 'on' and $Save{steve_at_work}==0) {
      ...
      &im_status_steve_changed unless active $timer_steve_leave_work; #Don't notify if I login within 5 minutes
...
sub im_status_steve_changed {
  if ($Save{steve_at_work}==1) {
    net_im_send(pgm=>'aol', to=>$Save{steve_work_aim}, text=>'Welcome to work!');
    play "room=kitchen aim/ring.wav";
    speak "Notice, Steve just logged on to the computer at work." if state $mode_occupied eq 'wife';
  } else {
    print_log "Steve is leaving work\n";
    if (time_cron '* 16,17,18,19 * * *' and state $mode_occupied eq 'wife') {
...
sub work_notify_steve {
  my $msg = "@_";
  &net_im_send(pgm => "AOL", to => $Save{steve_work_aim}, text => $msg) if $Save{steve_at_work}==1;
