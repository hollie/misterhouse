...
$v_concept_monitor = new Voice_Cmd("Concept Pannel Monitor [START,STOP,STATUS]");
...
if ($Startup) {
    ...
    $concept_monitor = new Socket_Item( undef, undef, $config_parms{concept_mon_address} );
    ...
    $concept_talker = new Socket_Item( undef, undef, $config_parms{concept_talk_address} );
...
sub load_concept_data {
    ...
    my $filename = $config_parms{code_dir} . "/" . $config_parms{concept_dat_file};
...
sub build_concept_file {
    ...
    print CF "# Created: $Time_Now, from concept_dat file: \"$config_parms{concept_dat_file}\"\n";
    print CF "# This file is automatically created with the Concept command RUN_BUILDER  -- DO NOT EDIT\n";
    print CF "#\n# -- DO NOT EDIT --\n";
    print CF "#\n#\n#\n";
    ...
    foreach my $category (@concept_categories) {
        ...
        print CF "# Category=" . $config_parms{concept_category_prefix} . "$category\n#\n";
    ...
    print CF "#\n# Create Event Ties\n#\n";
    foreach my $address ( sort num_sort keys %concept_data ) {
        if ( $concept_data{$address}{'type'} != 1 )    # Don't set a tie for Zone inputs, they are input only
        {
            $item = $concept_data{$address}{'label'};
            $item =~ s/ /_/g;
            ...
            printf CF ( "tie_event %-29s \'concept_set( \"%s\", \$state, $rstring)\';\n", $item, $address );
            $rows++;
        }
    }
    ...
    print CF "\t# *****************************************************************************************\n";
    print CF "\t# This subroutine is automatically generated by concept_builder.pl, do not edit !\n";
    print CF "\t# *****************************************************************************************\n\n";
...
sub concept_monitor_status {
    ...
    if ( active $concept_monitor) {
        print_log "Concept_Monitor: is active. Last event received was: $last_concept_event_state";
        speak("Concept Monitor is active. Last event received was $last_concept_event_state");
    }
    else {
...
if ( my $concept_msg = said $concept_monitor)    # See if any data has arrived
{
    ...
    if ( $cg_code == 203 )                       # Auxilary update
    {
        ...
        $co_setby = $cd[1];
        if ( $co_setby eq $config_parms{concept_interface} )    # Comms task 1 is MH
        {
            $co_setby = "MH";
        }
    }
    ...
    if ( $cg_type != 0 ) {    # We have something of interest
        ...
        if ( $co_setby eq 'MH' ) {
            ...
            print_log "Concept_Monitor: Recieved recursive instruction $concept_label $state_speak, ignored ";
        }
        else {
            concept_update( $cg_unit, $concept_state, 'concept' );    # The 'concept' is picked up
...
sub concept_set {
    ...
    if ( ( $level eq ON ) || ( $level eq 'ON' ) ) {
        ...
        print_log "Concept Talker: Unkown level \'$level\' passed to concept_set()";
        return;
    }
    ...
    {
        print_log "Concept Talker: Tryed to change state on a zone $device in concept_set()";
        return;
    }
...
sub concept_talker_status {
    ...
    if ( active $concept_talker) {
        print_log "Concept_Talker: is active. Last command sent was: $last_concept_control_state";
        speak("Concept Talker is active. Last command sent was $last_concept_control_state");
    }
    else {
