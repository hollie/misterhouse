...
#test moves
$v_move_er1client =
  new Voice_Cmd( "move: [forward 6 inches,forward 6 feet,backward 6 inches,backward 6 feet,turn left,turn left 45,turn right]" );
    ...
    if ( @er1data[0] eq 'sound' ) {
        ...
        { #true if position returned, !!! careful here, if you send a request for only 2 analog inputs, this will think they are x,y, really no need to do this
            ...
            if ( &net_socket_check("$strabo_ip:$strabo_port") ) {
                my $strabo_html =
                  get "http://$strabo_ip:$strabo_port/Setposition?" . $x . "," . $y;    #tells strabo to update position
