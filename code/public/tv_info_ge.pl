...
#    run qq[get_tv_info_ge -times $hour:$min -dates $mon/$mday -keyfile $config_parms{favorite_tv_shows_file}  -title_only];
    run qq[get_tv_info_ge -times "$hourNow:$minNow-$hour:$min" -dates $mon/$mday -keys "$config_parms{favorite_tv_shows}" -keyfile $config_parms{favorite_tv_shows_file}  -title_only];
...
if ($state = changed $f_tv_file or said $v_tv_results) {
    ...
    if ($state eq 'favorites today') {
        ...
        if ($show_count > 0) 
	{
        	speak address => &audrey_ip('Kitchen'), text => "Notice, $msg starting now.  @data" ;
	}
    }
    else {
