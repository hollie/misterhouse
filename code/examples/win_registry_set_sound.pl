...
if ( $state = said $sound_set) {
    $state = ( $state eq 'modem' ) ? 'Modem #0 Line Playback' : 'SB Live! Wave Device';
    print_log "Setting sound device to $state";
    registry_set 'HKEY_CURRENT_USER\Software\Microsoft\Multimedia\Sound Mapper', 'Playback', 1, $state;
