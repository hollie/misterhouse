...
if ( time_cron('* * * * *') ) {
    ...
    $Weather{SummaryWind} = sprintf(
        ...
        ( &convert_wind_dir_to_abbr( $Weather{WindDir} ) ),    $Weather{WindDir},
        ( &convert_wind_dir_to_abbr( $Weather{WindAvgDir} ) ), $Weather{WindAvgDir},
        $Weather{WindSpeed},                                   $main::config_parms{weather_uom_wind},
        $Weather{WindAvgSpeed},                                $main::config_parms{weather_uom_wind}
    );
...
sub setup_ds2450 {
    ...
    until ( $setupds2450 == 1 ) {
        ...
        $D{range}      = 5.12;
        #
        if ( $ib_wind_dir->Hardware::iButton::Device::DS2450::setup( $VCC, \%A, \%B, \%C, \%D ) ) {
            $setupds2450 = 1;
        }
        if ( $setupds2450 = 1 ) {
            ...
            print_log "Failed to initialise weather station DS2450, retrying...";
            sleep 1;
        }
    }
}
