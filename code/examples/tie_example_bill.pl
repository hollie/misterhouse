...
$Outside_Courtyard_Fountain->tie_filter( '$Windy->state() eq "1"',
    'on', 'log=fountain.log Fountain ON overridden due to high winds detected' );
$Outside_Courtyard_Fountain->tie_filter( '$Cold_outside->state() eq "1"',
    'on', 'log=fountain.log Fountain ON overridden due to cold outdoor temperatures' );
...
$Sprinklers->tie_event( '$Sprinklers->set_runtimes(1,1,1,1,1,1)',
    'complete', 'log=irrigation.log Zone cascade complete, setting default run times' );
