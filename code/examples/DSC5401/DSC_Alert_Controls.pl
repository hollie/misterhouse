...
sub alarm_page
{
   ...
   net_mail_send(from => "a\@videotron.ca", to => "a\@txt.bellmobilite.ca",
         text => $text2, subject => $text2);

   net_mail_send(from => "a\@videotron.ca", to => "a\@cgi.com",
         text => $text2, subject => $text2);

   net_mail_send(from => "a\@videotron.ca", to => "a\@videotron.ca",
         text => $text2, subject => $text2);

   speak(mode=>'unmuted', volume=>100, rooms=>'all', text=>"DSC Alarm Panel Said $text2");
