...
#   return                         if $from =~ /Mail Delivery Subsystem/i;
    ...
    return                         if $to =~ /listserv.activestate.com/i or $from_full =~ /listserv.activestate.com/i;
    return                         if $to   =~ /perl-/i;
    return                         if $from_full =~ /perl-/i;
    return 'The phone guys'        if $to =~ /ktx/ or $subject =~ /kx-t/i;
    return 'filtered - klug'       if $to =~ /klug/i;          # filtered -> will not be spoken (like blank)
