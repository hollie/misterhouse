...
$test_x10_rf2 -> tie_filter('$set_by eq "rf"', , 'Ignoring x10 rf data') if $Reload;
...
if ($state = state_now $test_x10_rf2) {
    ...
    print_log "test_x10_rf2 b set to $state by $test_x10_rf2->{set_by}" if get_set_by $test_x10_rf2 eq "rf";
}
