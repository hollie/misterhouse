...
if (0 and $New_Msecond_250) {
    my $is_dj_speaking = &Voice_Text::is_speaking();
    if ((!$is_dj_speaking_flag and  $is_dj_speaking) or
        ( $is_dj_speaking_flag and !$is_dj_speaking)) {
...
#  speak voice => 'mary',    text => read_next $dj_tagline;
    ...
    my $text = (rand(10) > 4) ? (read_next $dj_tagline) : (read_next $dj_thought);

    $Respond_Target = 'none';   # So common/speak_chime does not chime

#   speak volume => 200, card => '1/2', room => 'living', voice => 'Audrey', nolog => 1,  text => $text;
    speak volume => 200, card => '1/2', voice => 'Audrey', nolog => 1,  text => $text, mode => 'unmuted';

#    speak volume => 200, card => 3, voice => 'next', nolog => 1,  text => $text;
#   speak volume => 200, voice => 'mary', card => 3,
