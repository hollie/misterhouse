...
#   $text =~ s/\q&nbsp\;/ /g;
 ...
 $log_html = $header.$log_html if $data !~ /table/;
    ...
    file_write($f_weather_short, "And here is the weather at $update_time. The Temperature is $temp_F Degrees Fahrenheit. That\'s $temp_C Degrees Centigrade.  The Wind Direction is $wind_dir at $wind_mph Miles Per Hour. The Conditions are $conditions. The forecast for today is $today\r\n");
