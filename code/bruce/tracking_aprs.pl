...
if ( my $APRSString = said $tnc_output) {
        ...
        and substr( $data, 0, 6 ) eq '$GPRMC' )
    {
        ...
        my $GPSDistance =
          &great_circle_distance( $GPSLatitude, $GPSLongitude, $config_parms{latitude}, abs $config_parms{longitude} );

        # Skip if the car has not moved since last time or its been a while (unless distant internet data)
        if (
               ( $Debug{'aprs'} )
            or ( abs( $callsign_data{$callsign}{distance} - $GPSDistance ) > .1 )
          ...
          )
        {
            ...
            for my $GPSTempCompLine (@gps_complines) {
                ...
                if ( $GPSTempCompDist < $GPSCompDist ) {
                    ...
                    # Calculate direction
                    $GPSCompCourse =
                      int( atan2( $GPSTempCompLong - $GPSLongitude, $GPSLatitude - $GPSTempCompLat ) * 180 / 3.14159265 );
            ...
            $msg2 =
              ( $GPSCompDist < 1 )
              ? " near $GPSCompPlace"
              : &plural( $GPSCompDist, 'mile' ) . " $GPSCompCourse of $GPSCompPlace";
            if ($GPSSpeed) {
            ...
            if (
                ...
                or ( $gps_names{$callsign}{group} eq $config_parms{tracking_printflag} )
            ...
            my $track_flag = ( $gps_names{$callsign}{group} eq $config_parms{tracking_speakflag} )
                ...
                or $config_parms{tracking_speakflag} eq 'GPS' )
            {
                ...
                $html = qq[<FORM ACTION='/SET:last_response' target='speech';\n];
...
##              $html .= qq[&GC=X:$x|Y:$y|LT:$y|LN:$x|LS:8000|&IC=$y:$x:100:$callsign2&CMD=MAP\">];
            ...
            if ($track_flag) {
                ...
                $msg3 =~ s/ is / was /;
                $Save{ 'aprs_whereis_' . lc $callsign2 } =
                  'On ' . &time_date_stamp(15) . ' at ' . &time_date_stamp(5) . ', ' . $msg3;
...
sub great_circle_distance {
    ...
    my $d = ( sin( ( $lat2 - $lat1 ) / 2 ) )**2 + cos($lat1) * cos($lat2) * ( sin( ( $lon2 - $lon1 ) / 2 ) )**2;
...
$aprs_whereis3->set_info( 'This will find a vehicle using internet gateway data when it is away from home' );
...
if ( $state = said $aprs_whereis3) {
    ...
    set $get_aprs_p qq[get_url "http://www.findu.com/cgi-bin/rawposit.cgi?call=$vehicle&start=12&length=12" "$get_aprs_f"];
    start $get_aprs_p;
}
...
if ( done_now $get_aprs_p) {
    ...
    my ($record) = $html =~ /.+^(.+)<br>/sm;    # s -> . matches newline  m -> ^ matches newline
    set_data $tnc_output $record unless $record eq $Save{aprs_track_data};
    $Save{aprs_track_data} = $record;           # Save this, so we can see when data changes
...
if ($New_Week) {
    ...
    rename "$config_parms{tracking_dir}/week1.html", "$config_parms{tracking_dir}/week2.html"
      or print_log "Error in aprs rename 2: $!";
    rename "$config_parms{tracking_dir}/week1.txt", "$config_parms{tracking_dir}/week2.txt"
      or print_log "Error in aprs rename 2: $!";
...
sub menu_vehicle_log {
    my ($name) = @_;
    my ( @data, $count );
    for ( grep /The $name/, &file_read("$config_parms{tracking_dir}/week1.txt"),
        &file_read("$config_parms{tracking_dir}/week2.txt") )
    {
        my ( $day, $date, $time, $text ) = $_ =~ /(\S+) (\S+) (\S+) The $name + is +(.+)/;
