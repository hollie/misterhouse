...
##              $html .= qq[&GC=X:$x|Y:$y|LT:$y|LN:$x|LS:8000|&IC=$y:$x:100:$callsign2&CMD=MAP\">];
            ...
            if ($track_flag) {
                ...
                $msg3 =~ s/ is / was /;
                $Save{ 'aprs_whereis_' . lc $callsign2 } = 'On ' . &time_date_stamp(15) . ' at ' . &time_date_stamp(5) . ', ' . $msg3;
...
sub menu_vehicle_log {
    my ($name) = @_;
    my ( @data, $count );
    for ( grep /The $name/, &file_read("$config_parms{tracking_dir}/week1.txt"), &file_read("$config_parms{tracking_dir}/week2.txt") ) {
