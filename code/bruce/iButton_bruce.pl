...
speak voice => 'next', text => read_random $remark_bad if ON  eq state_now $ib_nick;
speak voice => 'next', text => read_next $f_deep_thoughts_ib if ON  eq state_now $ib_zack;
speak voice => 'next', text => read_next $remark_good  if ON  eq state_now $ib_bruce;
