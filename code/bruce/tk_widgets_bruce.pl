...
# Category=MisterHouse
    ...
    &tk_entry('Code Search', $search_code_string, 'Command Search', $search_command_string    ) if $Run_Members{mh_control};
    &tk_entry("Volume", $mh_volume, 'Debug flag', \$config_parms{debug}) if $Run_Members{mh_sound};

# Category=Music
    &tk_entry('MP3 Search', \$Save{mp3_search}, 'MP3 Genre', \$Save{mp3_Genre}) if $Run_Members{mp3_playlist};

# Category=Phone
    &tk_entry('Phone Search', \$Save{phone_search}, 'Photo Search', \$Save{photo_search});
...
# Category=Timed Events
    ...
    &tk_checkbutton('Sleeping Parents', \$Save{sleeping_parents}, 'Sleeping Nick', \$Save{sleeping_nick}, 'Sleeping Zack', \$Save{sleeping_zack});
    &tk_radiobutton('Wakeup Time',  \$Save{wakeup_time}, ['6:20 am', '6:30 am', '6:40 am', '7:00 am', 'none']) if $Run_Members{wakeup};
...
# Category=Internet
    ...
    &tk_radiobutton('Internet Speak', \$config_parms{internet_speak_flag}, ['none', 'local', 'some', 'all']) if $Run_Members{speak_server};
