...
if ($Reload) {
    ...
    &tk_button(
        'Reload (F1)', \&read_code,    'Pause (F2)', \&toggle_pause, ' Exit (F3) ', \&sig_handler,
    ...
    &tk_checkbutton(
        'Sleeping Parents', \$Save{sleeping_parents}, 'Sleeping Nick', \$Save{sleeping_nick},
    ...
    &tk_radiobutton( 'Internet Speak', \$config_parms{internet_speak_flag}, [ 'none', 'local', 'some', 'all' ] )
