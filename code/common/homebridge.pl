...
my $units = "C";
$units = $config_parms{homebridge_temp_units} if (defined $config_parms{homebridge_temp_units});
...
if (said $v_generate_hb_config) {
	...
	$config_json .= "\t\t}\n\t]\n}\n";
	print_log "[Homebridge]: Writing configuration to $filepath...";
...
sub add_group {
	...
	for my $member (list $group) {
		...
		if ($type eq "thermostat") {
			...
			if ($type eq "thermostat") {
				$text .= "\t\t\"mode_url\": \"http://" . $Info{IPAddress_Local} . ":" . $config_parms{http_port} . "/SUB?hb_thermo_set_state%28" . $member->{object_name} .",%VALUE%%29\",\n";			
				...
				$text .= "\t\t\"status_url\": \"http://" . $Info{IPAddress_local} . ":" . $config_parms{http_port} . "/SUB?hb_status%28" . $obj_name ."%29\",\n";
			}
...
sub hb_status {
     ...
     unless (defined $object) {
     	...
     	if (($state eq "on") or ($state =~ /^lock/i) or ($state =~ /^close/)) {
     		$data = "1";
     	} elsif (($state eq "off") or ($state =~ /^unlock/i) or ($state =~ /^open/) or ($state =~ /^down/)) {
     		$data = "0";
     	} elsif ($state =~ /^low/) {
     		$data = "20";
     	} elsif ($state =~ /^med/) {
     	...
     	print_log "[Homebridge]: Warning, no state data to return!" if ($data eq "");
     	print_log "[Homebridge]: Status request: item=$item state=$state status=[$data]\n" if ($hb_debug);
     }
     return <<eof;
HTTP/1.0 200 OK
Server: MisterHouse
...
sub hb_thermo_get_state {
	...
	my $object = &get_object_by_name($item);
	unless (defined $object) {
     	print_log "[Homebridge: hb_thermo_state]: Error, unknown object $object";
    } else {
...
sub hb_thermo_get_setpoint {
	...
	unless (defined $object) {
		...
		if (UNIVERSAL::isa($object,'Venstar_Colortouch')) {
			...
			print_log "[Homebridge]: Thermostat Venstar Colortouch with mode " . $mode . " found";
			if ($mode =~ /^cool/) {
				$data = $object -> get_cool_sp();
			} else {
				$data = $object -> get_heat_sp();
			}   
		}
	}
	print_log "[Homebridge]: Thermostat State request: item=$item mode=$mode status=[$data]\n" if ($hb_debug);
