...
if (($Misc{insult}{flag} == 1 and !&Voice_Text::is_speaking('any')) or said $v_insult1) {
    $Misc{insult}{flag}  = 2 if $Misc{insult}{flag} == 1;
    $Misc{insult}{robot} = ($Misc{insult}{robot} eq 'insult1') ? 'insult2' : 'insult1';
    my $text = read_random $f_insult;
    speak app => $Misc{insult}{robot}, text => $text;
    display window_name => 'insults',  text => "$Misc{insult}{robot}: $text\n\n",
            time => 0, font => 'Times 25 bold', append => 'top',
            geometry => '+0+0', width => 80, height => 25;
}
