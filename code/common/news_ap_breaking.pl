...
if ( done_now $p_ap_news) {
    ...
    foreach my $item ( @{ $rss->{'items'} } ) {
        ...
        $html .=
          qq|<li><a href="| . &recompose_uri( $item->{'link'} ) . qq|" title=$description>| . $item->{'title'} . qq|</a></li>|;
    ...
    if ( $v_get_ap_news->{state} eq 'Mail' ) {
        ...
        $v_get_ap_news->respond(
            "connected=0 image=mail Sending AP News to " . ( ($to) ? $to : $config_parms{net_mail_send_account} ) . '.' );
        &net_mail_send(
