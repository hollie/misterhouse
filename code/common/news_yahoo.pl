...
if ($Reload) {
    &trigger_set("time_cron('13 6,12,18 * * *')", "run_voice_cmd('Get daily news')", 'NoExpire', 'get daily news')
...
if (said $v_news) {
    ...
    $v_news->respond($news_files{"Entertainment"}) if $state eq 'Entertainment';
