...
if ($MW and $Reload) {
    ...
    if (!$Startup and !file_change("$config_parms{code_dir_common}/tk_frames.pl") and !$Invalidate_Window) {
	...
	&configure_element('frame', \$Tk_objects{fb4});
        ...
        file_change("$config_parms{code_dir_common}/tk_frames.pl") if $Startup; # Set file change time stamp *** Why? display_alpha doesn't do this for bitmaps
