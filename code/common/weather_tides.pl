...
if ((new_minute 10 and ($Weather{'Next High Tide'} eq '' or $Weather{'Next Low Tide'} eq '' or
  $Weather{'Next Moonrise'} eq '' or $Weather{'Next Moonset'} eq '')) or said $v_get_tides) {
	$v_get_tides->respond("app=tides Retrieving tide information for $tide_site...") if said $v_get_tides;
	unlink $f_tides;
	$p_get_tides -> start;
}
	...
	for my $html (file_read $f_tides, '') {
		if (my ($year, $mnth, $date, $hour, $minu, $size, $units, $event) = $html =~ 
		  /^(\d\d\d\d)-(\d\d)-(\d\d)  (\d\d):(\d\d) \w\w\w\s+(-?\d+\.\d+ (feet|meters))?\s+?(Low Tide|High Tide|Sunrise|Sunset|Moonrise|Moonset)$/) {
			...
			$Weather{'Previous High Tide'} = $time_str if $event eq 'High Tide' and $timediff < 0;
			$nexth = 1, $Weather{'Next High Tide'} = $time_str if $event eq 'High Tide' and $timediff > 0 and not $nexth;
			$Weather{'Previous Low Tide'} = $time_str if $event eq 'Low Tide' and $timediff < 0;
			$nextl = 1, $Weather{'Next Low Tide'} = $time_str if $event eq 'Low Tide' and $timediff > 0 and not $nextl;
			$Weather{'Previous Moonrise'} = $time_str if $event eq 'Moonrise' and $timediff < 0;
			$nextr = 1, $Weather{'Next Moonrise'} = $time_str if $event eq 'Moonrise' and $timediff > 0 and not $nextr;
			$Weather{'Previous Moonset'} = $time_str if $event eq 'Moonset' and $timediff < 0;
			$nexts = 1, $Weather{'Next Moonset'} = $time_str if $event eq 'Moonset' and $timediff > 0 and not $nexts;
		}
	}
