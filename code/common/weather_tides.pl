        ...
        $v_read_tides = new Voice_Cmd
          'When is the next [High Tide,Low Tide,Moonrise,Moonset]?';
...
=cut
        ...
        if ( done_now $p_get_tides) {
            ...
            for my $html ( file_read $f_tides, '' ) {
                if (
                    my (
                        $year, $mnth, $date, $hour, $minu, $size, $units,
                        $event
                    )
                    = $html =~
                ...
                {
                    my $timediff = timelocal( 0, $minu, $hour, $date, $mnth - 1,
                        $year - 1900 ) - $Time;
