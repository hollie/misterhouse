...
if ($Reload) {
	...
	if($config_parms{wunderground_apikey} eq '') {
		...
		if($wunderground_stationid ne '') {
			...
			print_log "[WUnderground] WARNING: wunderground_stationid and zip_core are both blank in mh.private.ini. Leaving it to Weather underground to guess your location by IP address..." if $Debug{weather};
