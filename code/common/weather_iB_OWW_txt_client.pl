...
# noloop=start
$config_parms{owwserver_host_deadman_minutes} = 2 unless defined $config_parms{owwserver_host_deadman_minutes};
$config_parms{owwserver_host_txt_port} = "localhost:8891" unless defined $config_parms{owwserver_host_txt_port};
$ibwstxt   = new  Socket_Item(undef, undef, $config_parms{owwserver_host_txt_port}, 'ibwstxt', 'tcp', 'raw');
# noloop=stop

# Initialisation
if ($Reload) {
    $config_parms{temperature_sensor_keys} = "outdoor => TempOutdoor, indoor => TempIndoor" unless defined $config_parms{temperature_sensor_keys};
}
...
PARSE: 
	...
	$temp_unit = $data[$i+2];
	print_log "Temperature at $key ( $name ) is $Weather{$name} $temp_unit" if $debug;
	$i+=2;
	last PARSE;
      };
      /^Humid\[/  && do {
	...
	$Weather{$name} = $data[$i+1];
	print_log "Humidity at $key ( $name ) is $Weather{$name}" if $debug;
	$i+=1;
	last PARSE;
      };
      /^Dew\[/  && do {
	...
	$Weather{$name} = $data[$i+1];
	print_log "Dewpoint at $key ( $name ) is $Weather{$name}" if $debug;
	$i+=1;
	last PARSE;
      };
      /^WindSpeed/  && do {
	...
	s/\[//; s/]//;
	print_log "Wind is $data[$i+1] degrees $data[$i+2] vane $data[$i+3]" if $debug;
