...
#@ Data feed from Weatherunderground. To use it you need to sign up for a weatherundeground key
...
# - sub getConditionsData  didn't work on Clear?? the chkcond array isn't checked yet
# - l771 times is kinda wonky seems to be a 1? $time-1 for array reference. Haven't seen anything spanning two timeslots
# - make the results output a string and email the results if email_notify is set
...
$p_wu_forecast = new Process_Item qq[get_url -quiet "http://api.wunderground.com/api/$config_parms{wu_key}/astronomy/yesterday/conditions/forecast/q/$config_parms{eto_location}.json" "$config_parms{data_dir}/wuData/wu_data.json"];
...
if ($Startup) {
	...
	unless (defined $config_parms{wu_key}) {
...
if (done_now $p_wu_forecast) {
	...
	if ($program_data) {
		if (defined $config_parms{eto_irrigation}) {
			...
			#$osp_program->set_program($Day,$run_times,$run_seconds);
			}	
		}
	} else {
...
#HP TODO: what does the any(x in current['weather'].lower() for x in chkcond): mean???
		if (defined $conditions->{$current->{weather}} ) {
	    	...
	    	print_log '[calc_eto] INFO not found current ' . $current->{weather};
			$cWeather = 5;
		}
	}
...
#        #
#HP TODO This should be an array.
	$startTime[$times - 1] = int($availTimes[$times - 1]) if ($times); 
...
############################## Print Results ###################################
    ...
    $msg = "[calc_eto] RESULTS We will not water because: $whyNot" if ($noWater);
	print_log $msg;
	$msg_string .= $msg . "\n";
