...
#       print_log $rc;
	...
	if (!defined $config_parms{net_connect_entry} and $config_parms{net_connect_entry}) { #name and password optional
		$v_logon_to_net->respond("app=error Dial-up networking is not configured.");
        	...
        	set $net_connect 'connected', $v_logon_to_net if &ras_connect(); # *** BIG assumption!
	}
    }
}
