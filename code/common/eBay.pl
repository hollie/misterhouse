...
my (
    %last_ebay_watching, %last_ebay_bidding, %last_ebay_won, %last_ebay_selling,
    %last_ebay_sold,     %last_ebay_unsold,  %last_ebay_lost
);
...
if ( done_now $p_ebay_sold) {
    ...
    foreach my $item ( keys %ebay_sold ) {
        ...
        $count++;
        foreach (
            'buyer',         'quantity',          'sale_price',         'title', 'paid',
            'feedback_left', 'feedback_received', 'completed_checkout', 'shipped'
          )
        {
