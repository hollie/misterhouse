...
sub Search_Header_Allow {
	...
	if (($Authorized eq 'admin') or ($config_parms{Use_Header_Control}=0)){
		$AllowHeaderPassage = 1;
		#print_log "$Authorized is authorized";
	}else{
        	open (MYFILE, $header_file) or die "Error, could not open file $header_file: $!\n";
        	while (<MYFILE>) {
	...
	if (($AllowHeaderPassage eq "") and ($config_parms{Use_Header_Control}=1)){
