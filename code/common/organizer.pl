...
ical2vsdb_<name>_options = ical processing options
...
ical2vsdb_account1_options = holiday, speak_cal
...
#print_log "i=$ical, icals->{$ical}\n";
    ...
    if ($data) {
    	$data = "#\n# Autogenerated code at" . $Time_Now . "\n#\n" . $data;
	$data .= "\n#Options\n";
...
sub write_i2v {
     ...
     open (FILE,">$filename") || print_log "Organizer.pl Error: Cannot write i2v.cfg file $filename!";
     print FILE $data;
     close (FILE);
