...
if ($Reload) {
    ...
    my @_upd_cal =
      ( 'DATE', 'TIME', 'EVENT', 'CATEGORY', 'DETAILS', 'HOLIDAY', 'VACATION', 'SOURCE', 'REMINDER', 'ENDTIME', 'CONTROL' );
    $calOk = &update_vsdb( 'Calendar', $_organizer_cal->name, @_upd_cal );

    my @_upd_todo =
      ( 'Complete', 'Description', 'DueDate', 'AssignedTo', 'Notes', 'SPEAK', 'SOURCE', 'REMINDER', 'STARTDATE', 'CATEGORY' );
    ...
    if ( $config_parms{ical_read_interval} ) {
        ...
        if ($data) {
            $data = "#\n# Autogenerated code at " . $Date_Now . " " . $Time_Now . "\n#\n" . $data;
            $data .= "\n#Options\n";
    ...
    while ( !$objDB->EOF ) {
        ...
        eval {
            ...
            my $evaldt =
              ...
              : $data{date} . ' 12:00 am';
            next
              unless time_less_than("$evaldt + 23:59");    # Skip past and invalid events
                                                           # protect against bad dates
...
sub get_textmsg_code {
    ...
    for ( my $index = 1; $index <= $data{name_count}; $index++ ) {
        $email .= "net_mail_send to => '$_organizer_emails{lc $data{name}[$index]}', subject => q~$subject~, text => q~$notes~; ";
    }
    return $email;
}
...
sub generate_code {
    ...
    if ( $data{reminder}
        and !( time_greater_than( $data{time_date} ) or $data{reminder} eq 'none' ) )
    {
        ...
        for my $reminder_info (@reminders) {
            ...
            if ($reminder_time) {
                ...
                print $fh "   if (time_now '$data{time_date} - $reminder_diff ') {" . &get_speak_code(%data) . "};\n";
            }
        }
    }
    if ( $data{type} =~ /task/i ) {
        if ( ($speak_tasks) and ( $data{speak} =~ /^y/i ) ) {
            if ( $data{time} and ( $data{time} !~ /12:00 am/i ) ) {
                print MYCODE "   if (time_now '$data{date} $data{time}') {" . &get_speak_code(%data) . "};\n";
            }
            else {
                foreach my $announce_time (@_organizer_announce_day_times) {
                    print MYCODE "   if (time_now '$data{date}  $announce_time') {" . &get_speak_code(%data) . "};\n";
                }
            }
        }
