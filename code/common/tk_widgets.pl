...
if ($Reload and $MW) {
    ...
    if ($config_parms{tk_system_widgets}) {
	    &tk_label_new(2, \$Tk_objects{label_time}) if $config_parms{tk_clock};
    ...
    $Tk_objects{sliders}{x10_errata} = &tk_scalebar(\$config_parms{x10_errata}, 1, 'X10 Logging', 1, 4, 0);
...
#   &tk_entry('Code Search', $search_code_string,    'Debug flag', \$config_parms{debug});
    &tk_entry('Code Search', $search_code_string) if $config_parms{tk_system_widgets};
