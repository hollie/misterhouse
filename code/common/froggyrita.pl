    ...
    if ( $command_waiting and $state ne 'status' ) {
        if ( defined $temperature ) {
            $command_waiting->respond(
                'app=frog connected=0 Indoor temperature is ' . $temperature . ' degrees fahrenheit. Humidity is ' . $Froggy->humidity() . '%' );
        }
        else {
...
if ($Reload) {
    &trigger_set( "new_minute 5", "&get_froggy_status()", 'NoExpire', 'get frog status' ) unless &trigger_get('get frog status');
}
