...
if ( expired $timer_speed_check) {
    ...
    $v_what_speed->respond(
        "app=pc $Info{cpu_used}% of cpu, $Info{loop_speed} loops/sec, and " . ( int $Info{memory_real} ) . " megabytes" );
}
...
if ( said $v_speed_benchmark) {
    ...
    if ( $v_speed_benchmark->{state} eq 'Stop the' ) {
        ...
        $msg = 'Speed benchmark started by ' . ( ( $v_speed_benchmark->{state} =~ /name/i ) ? 'name' : 'speed' ) . '.';
    }
...
if ( $speed_benchmark_count and ( new_second 5 or said $v_speed_benchmark) ) {
    ...
    my @log;
    for my $member ( sort { $by_speed and ( $Benchmark_Members{$b} <=> $Benchmark_Members{$a} ) or $a cmp $b } ' OTHER',
        ' USER', keys %Run_Members )
