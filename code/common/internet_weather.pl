...
if ( said $v_get_internet_weather_data) {
    if (&net_connect_check) {
        set $p_weather_data qq|get_weather -state $config_parms{state} -city "$city" -zone "$config_parms{zone}"|;
        start $p_weather_data;
        $v_get_internet_weather_data->respond( "app=weather Weather data requested for $city, $config_parms{state}"
...
if ( said $v_get_internet_weather_conditions) {
    if (&net_connect_check) {
        ...
        $v_get_internet_weather_conditions->respond( "app=weather Weather conditions requested for $city, $config_parms{state}"
              . ( ( $config_parms{zone} ) ? " Zone $config_parms{zone}" : '' ) );
    }
    else {
...
if ( said $v_get_internet_weather_forecast) {
    if (&net_connect_check) {
        ...
        $v_get_internet_weather_forecast->respond( "app=weather Weather forecast requested for $city, $config_parms{state}"
              . ( ( $config_parms{zone} ) ? " Zone $config_parms{zone}" : '' ) );
    }
    else {
...
if ( done_now $p_weather_data or done_now $p_weather_conditions) {
    ...
    if ( $conditions =~ /No data available/ ) {
        ...
        $w{DewOutdoor} =
          &Weather_Common::convert_humidity_to_dewpoint( $w{HumidOutdoor}, convert_f2c( $w{TempOutdoor} ) )
...
if ( done_now $p_weather_data) {
    if ( $v_get_internet_weather_data->{state} eq 'Check' ) {
        ...
        $v_get_internet_weather_data->respond( "connected=0 app=weather image=mail Sending Internet weather data to "
