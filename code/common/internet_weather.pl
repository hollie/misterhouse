...
if (said $v_get_internet_weather_data) {
	...
	my $data = "all";
	$data = $config_parms{nws_data} if defined $config_parms{nws_data};
        $city = $config_parms{nws_city} if defined $config_parms{nws_city};
        set $p_weather_forecast qq|get_weather -state $config_parms{state} -city "$city" -zone "$config_parms{zone}" -data $data|;
        start $p_weather_forecast;
        print_log "$data weather data requested for $city, $config_parms{state}" . (($config_parms{zone})?"zone=$config_parms{zone}":"");
    }
    else {
...
if ($state = said $v_show_internet_weather_forecast) { 
    if ($config_parms{nws_data} eq "conditions") {
	print_log "The internet_weather.pl module is not configured for forecasts. Set the nws_data configuration parameter to 'forecast' or 'all'.";
    }
    else {
	if ($state eq "Read") {
...
if ($state = said $v_show_internet_weather_conditions) {
    if ($config_parms{nws_data} eq "forecast") {
	print_log "The internet_weather.pl module is not configured for current conditions. Set the nws_data configuration parameter to 'conditions' or 'all'.";
    }
    else {
	if ($state eq "Read") {
...
if (done_now $p_weather_forecast) {
    ...
    if (lc $config_parms{nws_data} ne "forecast") {
