...
=cut
    ...
    foreach my $day ( split /\|/, $Weather{"Forecast Days"} ) {
        my $chance = 0;
        $chance = 20
          if $forecasts{$day} =~ /(slight|a) chance (of|for) *\S* *(rain|showers|snow|thunderstorms)/i;
        $chance = 60
          if $forecasts{$day} =~ /(rain|showers|snow|thunderstorms) (becoming )?likely/i;
        $chance = $1
          if $forecasts{$day} =~ /(\d+) percent chance (of|for) *\S* *(rain|showers|snow|thunderstorms)/i;
        $chance = $4
          if $forecasts{$day} =~ /chance (of|for) *\S* *(rain|showers|snow|thunderstorms)( near)? (\d+) percent/i;
        $chance = $3
          if $forecasts{$day} =~ /chance (of|for) *\S* *(rain|showers|snow|thunderstorms) increasing to (\d+) percent/i;
    ...
    unless ($text) {
        ...
        $Weather{ "ChanceOf" . ucfirst($current_precip) . "Percent" } = $current_chance
        ...
        $v_get_chance_of_rain->respond("app=rain Precipitation forecast prepared.")
          if said $v_get_chance_of_rain;
    }
}
...
if ($Reload) {
    &trigger_set( "time_cron '5 8,11,16,19 * * *'", "run_voice_cmd 'Read the forecasted chance of rain or snow'", 'NoExpire', 'read chance of rain' )
