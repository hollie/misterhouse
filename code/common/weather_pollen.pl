...
$p_pollen_forecast = new Process_Item(
    "get_url http://www.claritin.com/weatherpollenservice/weatherpollenservice.svc/getforecast/$config_parms{zip_code} $pollen_file"
);

&parse_pollen_forecast if ( ($Reload) && ( -e $pollen_file ) );

sub parse_pollen_forecast {
    my @pollen_data = file_read($pollen_file)
    ...
    my $json = decode_json(@pollen_data)
      || warn "Error parsing pollen info from file.";
