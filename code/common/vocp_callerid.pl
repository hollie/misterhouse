...
if ( file_changed $config_parms{vocp_logfile} ) {
    ...
    if ( $ctype eq 'incoming call' ) {
        ...
        if ( ( $cname eq '0' ) or ( $cname eq 'O' ) ) {
            my %callerid_by_number = dbm_read("$config_parms{data_dir}/phone/callerid.dbm")
              unless %callerid_by_number;
            my $cid_data = $callerid_by_number{$fcnumber};
            my ( $cid_calls, $cid_time, $cid_date, $cid_name ) = $cid_data =~ /^(\d+) +(.+), (.+) name=(.+)/
              if $cid_data;
            $cname = $cid_name;
...
if ( ( ( $state = said $v_audrey_top_led_off ) or ( $state = said $v_audrey_both_leds_off ) )
    and $config_parms{vocp_audrey} )
{
...
if ( $state = said $v_ciddb_upd) {
    ...
    for ( file_read "$config_parms{data_dir}/organizer/contacts.tab" ) {
        my (
            $ID,        $FirstName,   $LastName,     $Title,        $Company,      $WorkAddress1,   $WorkAddress2, $WorkCity,
            $WorkState, $WorkZip,     $HomeAddress1, $HomeAddress2, $HomeCity,     $HomeState,      $HomeZip,      $WorkPhone,
            $HomePhone, $MobilePhone, $Fax,          $Pager,        $PrimaryEmail, $SecondaryEmail, $WebSiteURL,   $Notes
        ) = split( /\t/, $_ );
        $WorkPhone =~ s/\D//g;
        $HomePhone =~ s/\D//g;
        $MobilePhone =~ s/\D//g;
