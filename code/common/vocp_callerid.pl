...
if ( $state = said $v_ciddb_upd) {
    ...
    for ( file_read "$config_parms{data_dir}/organizer/contacts.tab" ) {
        my (
            $ID,           $FirstName, $LastName,     $Title,          $Company,      $WorkAddress1,
            $WorkAddress2, $WorkCity,  $WorkState,    $WorkZip,        $HomeAddress1, $HomeAddress2,
            $HomeCity,     $HomeState, $HomeZip,      $WorkPhone,      $HomePhone,    $MobilePhone,
            $Fax,          $Pager,     $PrimaryEmail, $SecondaryEmail, $WebSiteURL,   $Notes
        ) = split( /\t/, $_ );
        $WorkPhone =~ s/\D//g;
        $HomePhone =~ s/\D//g;
        $MobilePhone =~ s/\D//g;
        if ( $WorkPhone and ( length($WorkPhone) == 10 ) ) {
            ...
            logit_dbm( "$config_parms{data_dir}/phone/callerid.dbm",
                $tmp, "$Time_Now $Date_Now $Year name=$LastName $FirstName (Work)" );
        ...
        if ( $MobilePhone and ( length($MobilePhone) == 10 ) ) {
            ...
            logit_dbm( "$config_parms{data_dir}/phone/callerid.dbm",
                $tmp, "$Time_Now $Date_Now $Year name=$LastName $FirstName (Cell)" );
