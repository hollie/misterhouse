...
if ($Reload) {
    ...
    &trigger_set( $command, 'speak "force_chime=1 app=sunrise Notice, the sun is now rising at $Time_Sunrise"',
        'NoExpire', 'announce sunrise' )
      unless &trigger_get('announce sunrise');

    $command = 'time_now $Time_Sunset';
    &trigger_set( $command, 'speak "force_chime=1 app=sunset Notice, the sun is now setting at $Time_Sunset"',
        'NoExpire', 'announce sunset' )
