...
#        <META HTTP-EQUIV="REFRESH" CONTENT="6; url=' . "'" . 'SUB;referer?uirt2_update_html()' . "'" . '">
    ...
    $html .= '
      ...
      ';
    @functions = ();
    @functions = UIRT2::list_functions($current_device) if $current_device ne '';
    ...
    $html .= '
      ...
      Frequency
      <input type="radio" value="36" name=$uirt2_function_frequency' . ($frequency == 36 ? ' checked' : '') . '>36
      <input type="radio" value="38" name=$uirt2_function_frequency' . ($frequency == 38 ? ' checked' : '') . '>38
      <input type="radio" value="40" name=$uirt2_function_frequency' . ($frequency == 40 ? ' checked' : '') . '>40 kHz 
...
if (state_now $uirt2_device_rename) {
	my $dev = state $uirt2_device_list;
	my $device = uc state $uirt2_device_text;
	UIRT2::rename_device($devices[$dev], $device) if $dev =~ /\d+/ and $device;
	$current_device = $device;
	$current_function = '';
}
...
if (state_now $uirt2_gen_commit) {
	...
	UIRT2::set_ir_code($device, $function, UIRT2::raw_to_struct(UIRT2::pronto_to_raw(
