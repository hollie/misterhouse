...
sub mp3Rip_process_log_entry {
    ...
    if ( $line eq 'Rip Completed' ) {
        ...
        if ( $ripper_data{$cddbid}->{'rip_done_count'} == $ripper_data{$cddbid}->{'total_tracks'} ) {
            ...
            $ripper_data{$cddbid}->{'rip_predicted_total'} = int( $ripper_data{$cddbid}->{'total_length'} *
                  ( $ripper_data{$cddbid}->{'rip_time'} / $ripper_data{$cddbid}->{'rip_done_length'} ) );
            $ripper_data{$cddbid}->{'rip_predicted_remaining'} =
              $ripper_data{$cddbid}->{'rip_predicted_total'} - $ripper_data{$cddbid}->{'rip_time'};
        }
    }
        ...
        if ( $ripper_data{$cddbid}->{'compress_done_count'} == $ripper_data{$cddbid}->{'total_tracks'} ) {
            ...
            $ripper_data{$cddbid}->{'compress_predicted_total'} = int( $ripper_data{$cddbid}->{'total_length'} *
                  ( $ripper_data{$cddbid}->{'compress_time'} / $ripper_data{$cddbid}->{'compress_done_length'} ) );
