...
if ( $state = said $android_server) {
    ...
    # Older legacy method
        ...
        &print_log( "android_server:: legacy_login_request:: pass: $pass device: $device, port: $port, room: $room" )
          if $Debug{android};
    ...
    if ( my $user = password_check $pass, 'server_android' ) {
        ...
        &print_log( "android_server:: login_accepted:: user: $user room: $room device: $device ip: $client_ip client: $client " )
          if $Debug{android};
