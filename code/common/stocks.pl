...
my @stock_keys = (
    'SName',     'LName',     'Last',          'Date', 'Time',      'Change',
    'PChange',   'Volume',    'Avg Volume',    'Bid',  'Ask',       'Prev Close',
    'Open',      'Day Range', '52-Week Range', 'EPS',  'P/E Ratio', 'Div Pay Date',
...
if ( done_now $p_stock_quote) {
    ...
    if ( $v_stock_quote->{state} eq 'Check' ) {
        ...
        $v_stock_quote->respond( "app=stocks connected=0 image=mail Sending stock quotes to "
