...
if ($Reload) { 
	$versalink_host = $config_parms{'versalink_host'} if $config_parms{'versalink_host'}; 
...
if (done_now $p_get_versalink) {
	...
	my @cell = $te->rows;
	$versalink_download = $cell[7][0]  > $versalink_in  ? $cell[7][0]  - $versalink_in  : 0;
	$versalink_upload   = $cell[13][0] > $versalink_out ? $cell[13][0] - $versalink_out : 0;
