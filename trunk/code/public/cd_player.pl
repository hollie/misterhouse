...
if ($state = said $v_cd_player) {
    my %cmd_table = (on => 'on', off => 'off', load => 'disk load', eject => 'disk eject', 
                     next => 'track next', prev => 'track prev', play => 'play', stop => 'stop', 
                     pause => 'pause', 'volume up' => 'volume 8', 'volume down' => 'volume -8');
