...
#if (($New_Minute) or
if ((state $mode_mh eq 'normal') and time_cron('0,5,10,15,20,25,30,35,40,45,50,55 * * * *')) {
...
if ($state = changed $f_tv_file or said $v_tv_results) {
	...
	$title ='';
        if ( (($title, $ch_name, $ch_key, $ch_num, $start, $end) = 
          $line =~ /^\d+\|\s+(.+)\|\s*Canal (.+)\|\s*(.+)\|\s+Dial\s+(\d+)\|\s*De (\d+:\d+) hasta (\d+:\d+)\|/) or 
...
my $display;
    ...
    open(MYCODE, ">$tv_shows_e") or print_log "Error in writing to $tv_shows_e";
