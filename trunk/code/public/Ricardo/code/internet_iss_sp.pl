...
my $iss_check_u = "http://www.heavens-above.com/PassSummary.asp?" . 
...
if (done_now $iss_check_p or $state eq 'lista') {
    my ($display, $time_s, $azimuth_s, $time_m, $azimuth_m, $time_e, $azimuth_e, $sec_s, $sec_m, $sec_e, $time_start, $time_max, $time_end);
...
#   print "ISS_TEXT:\n $text \n";
    ...
    open(MYCODE, ">$iss_check_e") or print_log "Error in writing to $iss_check_e";
...
#           print "db testing time: $a[1]/$a[0] $a[3]\n";
            $time_s = my_str2time($config_parms{date_format} =~ /ddmm/ ? 
                                "$a[0]/$a[1] $a[3]" : "$a[1]/$a[0] $a[3]") +
                                  3600*$config_parms{time_zone};
            $time_s += 3600 if (localtime)[8]; # Adjust for daylight savings time
            $time_m = my_str2time($config_parms{date_format} =~ /ddmm/ ? 
                                "$a[0]/$a[1] $a[6]" : "$a[1]/$a[0] $a[6]") +
                                  3600*$config_parms{time_zone};
            $time_m += 3600 if (localtime)[8]; # Adjust for daylight savings time
            $time_e = my_str2time($config_parms{date_format} =~ /ddmm/ ? 
                                "$a[0]/$a[1] $a[9]" : "$a[1]/$a[0] $a[9]") +
                                  3600*$config_parms{time_zone};
            $time_e += 3600 if (localtime)[8]; # Adjust for daylight savings time
