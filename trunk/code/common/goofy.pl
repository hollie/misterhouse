...
$v_why_here->respond("app=goofy Well, why are you here? Why are any of us here? I mean when you get down to it, it is all so meaningless. You know what I mean?") if said $v_why_here;

$v_go_away->respond("I would go away if I could.") if said $v_go_away;
$v_personal_remark_good->respond("app=goofy " . read_next $f_personal_remark_good) if said $v_personal_remark_good;
$v_personal_remark_bad->respond("app=goofy image=frown " . read_next $f_personal_remark_bad) if said $v_personal_remark_bad;
