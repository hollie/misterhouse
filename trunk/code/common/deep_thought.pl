...
$v_deep_thought-> set_info('The deep thoughts are creative, goofy, and fun sayings from SNL');
$v_deep_thought_next-> set_authority('anyone');

my $temp_deep_thought = join(',', &Voice_Text::list_voices);
$v_deep_thought_voice = new Voice_Cmd "Speak deep thought with voice [$temp_deep_thought,random,next]";
...
$v_deep_thought_voice->tie_event('&read_deep_thought($v_deep_thought_voice, $state)');
#noloop=stop
...
$v_house_tagline->respond("app=tagline" . read_next $house_tagline) if said $v_house_tagline;
