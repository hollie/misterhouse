...
#--- Execute Zone Motion with Light
	...
	#print_log "activite DSC motion zone 10";
	    $State_Light = state $Cinema_Lumiere_Sofa;
      	    if ($State_Light eq OFF)
            {
	      set $Cinema_Lumiere_Sofa -40 if Time_Schedule($Time_Sunset,"23:59");
	      set $Cinema_Lumiere_Sofa -50 if Time_Schedule("00:00",$Time_Sunrise);
	    }

	    $State_Light = state $Cinema_Lumiere_Foyer;
	    if ($State_Light eq OFF)
            {
	      set $Cinema_Lumiere_Foyer -40 if Time_Schedule($Time_Sunset,"23:59");
	      set $Cinema_Lumiere_Foyer -50 if Time_Schedule("00:00",$Time_Sunrise);
	    }
		...
		unset $IR_Salon_SS_Timer;
            ...
            set $IR_Salon_SS_Timer 180 if (Time_Schedule("00:00",$Time_Sunrise) && ($Weekday == 1));
            set $IR_Salon_SS_Timer 300 if (Time_Schedule("00:00",$Time_Sunrise) && ($Weekday == 0));
	}
...
if (expired $IR_Salon_SS_Timer && $Holiday eq 0)
{
	...
	set $Cinema_Lumiere_Sofa OFF if (Time_Schedule("23:31","23:59") && ($Weekday == 1));
	set $Cinema_Lumiere_Foyer OFF if (Time_Schedule("23:31","23:59") && ($Weekday == 1));

	set $Cinema_Lumiere_Sofa OFF if Time_Schedule("00:00",$Time_Sunrise);
	set $Cinema_Lumiere_Foyer OFF if Time_Schedule("00:00",$Time_Sunrise);
}
