...
my $tv_states = $tv_controls . ',' . $tv_local_channels . ',' . 
  join(',', (sort { $tv_cable_channels{$a} <=> $tv_cable_channels{$b} } keys %tv_cable_channels));
...
if ($state = said $v_vcr_control) {
    ...
    if ($state eq "video2" || $state eq "video3" || $state eq "mute" || $state eq "vol+" || $state eq "vol-") {
        set $TV $state;
    }
    else {
        set $VCR $state;
