...
if ($Weather{TempOutdoor} > ($Weather{TempIndoor} + .75) and
    state $weather_outside_flag ne 'warmer') {
    set $weather_outside_flag 'warmer';
}
elsif ($Weather{TempOutdoor} < ($Weather{TempIndoor} - .5) and
       state $weather_outside_flag ne 'cooler') {
