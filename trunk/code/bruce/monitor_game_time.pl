...
if ($state = said $check_game_time) {
    display text => scalar file_tail('//warp/c/mh/data/logs/eqtime_totals.log', 100), 
      time => 200, title => 'Total eq time log', font => 'fixed', height => 50
        unless $state eq 'daily';
    display text => scalar file_tail('//warp/c/mh/data/logs/eqtime.log', 50), 
      time => 50, title =>  'Daily eq time log', font => 'fixed', height => 50
