...
sub X10Lamp {
 ...
 $objState = $o->state;
 $icon = "<a href=\"/SET;&webpause?$arg1=off\"><font face=arial size=+3 color=990033><b>" . $objState . "</b></a></font>";
 if ($objState eq 'on') {$icon = "<a href=\"/SET;&webpause?$arg1=off\"><img border=0 src=$onIcon></a>";}
 if ($objState eq 'off') {$icon = "<a href=\"/SET;&webpause?$arg1=on\"><img border=0 src=$offIcon></a>";}
 return $icon;
}
...
sub housemode {
 ...
 if ($objState eq 'on') {$icon = "<a href=\"/SET;&housemodepause?$arg1=off\"><img border=0 src=$onIcon></a>";}
 if ($objState eq 'off') {$icon = "<a href=\"/SET;&housemodepause?$arg1=on\"><img border=0 src=$offIcon></a>";}
 return $icon;
}
...
$log_out = "";
    ...
    $NumofCalls = 0;
    $log_out = "<table width=100% columns=4 cellspacing=3><tr><font color=white size=2>";
    foreach $CallLogTempLine (@callloglines) {
        ...
        $log_out .= "<td>$PhoneDateLog</td>     <td>$PhoneTimeLog</td>     <td>$PhoneNameLog</td>      <td>$PhoneNumberLog</td>";
        $log_out .= "</tr>";
    }
  ...
  $log_out .= "<td align=left colspan=2 bgcolor=silver><font color=black size=3><b>NUMBER OF CALLS:  $NumofCalls</td>";
  $log_out .= "<td align=right colspan=2><a href=\"/SUB;phone.shtml?web_clearphonelog\"><img src=\"/graphics/icon_eraser.gif\"></a></td>";
  $log_out .= "</tr></table>";
