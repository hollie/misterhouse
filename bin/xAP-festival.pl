...
my $xap_send = new IO::Socket::INET->new(PeerPort => $XAP_PORT, Proto => 'udp', PeerAddr => inet_ntoa(INADDR_BROADCAST), Broadcast => 1 ) or
...
if ($FORCE_LOCAL_HUB) {
  ...
  for my $p (49152 .. 65535) {
    $XAP_PORT = $p;
    last if $xap_listen = new IO::Socket::INET->new(LocalAddr => 'localhost', LocalPort => $p, Proto => 'udp');
  } 
  die "Could not create xap listener\n" unless $xap_listen;
  print "Listening on relay socket ", $xap_listen->sockport(), "\n" if $debug;
} else {

  $xap_listen = new IO::Socket::INET->new(LocalAddr=>inet_ntoa(INADDR_ANY), LocalPort => $XAP_PORT, Proto => 'udp', Broadcast => 1 );
...
while (1) {
  ...
  if (select($rin, undef, undef, 0)) {
    ...
    if ($header =~ /\nclass\=tts.speak\n/i) {
      ...
      $target = lc $target;
      print "target=[$target]\n" if $debug;
      if ( ($target eq "") || ($target eq "*") || ($target eq "*.festival.*") || ($target eq $my_address)) {
...
sub handle_tts_speak {
  ...
  &speak($say,$volume,$voice) if ( ($rooms eq "") || ($rooms eq "all") || ($rooms eq $room) );
}
